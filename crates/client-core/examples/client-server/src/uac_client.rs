...
impl SimpleUacHandler {
    fn new(rtp_debug: bool, wav_file: Option<&str>, server_address: String) -> Self {
        // Load WAV file if provided (basic file loading without format validation)
        let wav_data = wav_file.and_then(|path| {
            if Path::new(path).exists() {
                match std::fs::read(path) {
                    Ok(data) => {
                        info!("📁 Loaded WAV file: {} ({} bytes)", path, data.len());
                        info!("🎵 WAV file loaded (format validation skipped - will use default test tone)");
                        Some(data)
                    }
                    Err(e) => {
    ...
    pub async fn run(
        ...
        // Store the client
        self.set_client_manager(client.clone()).await;
