...
impl DialogApiTestEnvironment {
    ...
    async fn new() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let (server_transaction_manager, _server_events) =
        ...
        let (client_transaction_manager, _client_events) =
            TransactionManager::with_transport_manager(
        ...
        let server =
            DialogServer::with_dependencies(Arc::new(server_transaction_manager), server_config)
                .await?;

        let client =
            DialogClient::with_dependencies(Arc::new(client_transaction_manager), client_config)
                .await?;
...
async fn test_basic_dialog_operations_real_transport() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let dialog_id = env
        .server
        .create_outgoing_dialog(local_uri.clone(), remote_uri.clone(), None)
        .await?;
    ...
    assert!(
        active_dialogs.contains(&dialog_id),
        "Created dialog should be in active list"
    );
...
async fn test_call_operations_real_transport() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let invite_request =
        env.create_sip_request(Method::Invite, &format!("sip:bob@{}", env.server_addr));
    ...
    let accept_result = env
        .server
        .accept_call(&dialog_id, Some("SDP answer".to_string()))
        .await;
...
async fn test_api_error_handling_real_transport() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        info_result.is_err(),
        "Should return error for invalid dialog ID"
    );
...
async fn test_concurrent_api_operations_real_transport() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..5 {
        // Test with 5 concurrent dialogs
    ...
    for handle in handles {
        match handle.await? {
            Ok(dialog_id) => {
                ...
                println!(
                    "✅ Created concurrent dialog: {}",
                    dialog_ids.last().unwrap()
                );
            }
    ...
    println!(
        "✅ Successfully created {} concurrent dialogs",
        dialog_ids.len()
    );
