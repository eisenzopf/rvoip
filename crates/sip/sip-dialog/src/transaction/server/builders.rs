...
/// # Example
/// ```
/// use rvoip_sip_dialog::transaction::server::builders::ResponseBuilder;
...
impl ResponseBuilder {
    ...
    pub fn build(self) -> Result<Response> {
        ...
        if let Some(call_id) = self.call_id {
            response
                .headers
                .push(TypedHeader::CallId(CallId::new(&call_id)));
        }
        ...
        if let Some(contact) = self.contact {
            let contact_uri: Uri = contact
                .parse()
                .map_err(|e| Error::Other(format!("Invalid contact URI: {}", e)))?;
        ...
        if let Some(sdp_content) = &self.sdp_content {
            if let Some(_content_type) = &self.content_type {
                response
                    .headers
                    .push(TypedHeader::ContentType(ContentType::from_type_subtype(
                        "application",
                        "sdp",
                    )));
            }
...
/// # Example
/// ```
/// use rvoip_sip_dialog::transaction::server::builders::InviteResponseBuilder;
/// use rvoip_sip_dialog::transaction::builders::client_quick;
/// use rvoip_sip_core::StatusCode;
/// use std::net::SocketAddr;
///
...
/// # Example
/// ```
/// use rvoip_sip_dialog::transaction::server::builders::RegisterResponseBuilder;
/// use rvoip_sip_dialog::transaction::builders::client_quick;
/// use rvoip_sip_core::StatusCode;
/// use std::net::SocketAddr;
///
...
pub mod quick {
    ...
    pub fn ringing(request: &Request, contact: Option<String>) -> Result<Response> {
        let mut builder = InviteResponseBuilder::new(StatusCode::Ringing).from_request(request);
    ...
    pub fn server_error(request: &Request, reason: Option<String>) -> Result<Response> {
        let mut builder =
            ResponseBuilder::new(StatusCode::ServerInternalError).from_request(request);
