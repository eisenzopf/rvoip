...
/// # use rvoip_sip_core::prelude::*;
/// # use rvoip_sip_dialog::transaction::method::update::create_update_request;
...
pub fn create_update_request(
    ...
    if dialog_request.to().is_none()
        || dialog_request.from().is_none()
        || dialog_request.call_id().is_none()
        || dialog_request.cseq().is_none()
    {
        return Err(Error::Other(
            "Invalid dialog request - missing required headers".to_string(),
        ));
    }
    ...
    if dialog_request.to().unwrap().tag().is_none() {
        return Err(Error::Other(
            "Cannot create UPDATE - not an in-dialog request (To tag missing)".to_string(),
        ));
    }
    ...
    update = update.with_header(TypedHeader::MaxForwards(
...
/// # use std::str::FromStr;
/// # use rvoip_sip_core::prelude::*;
/// # use rvoip_sip_dialog::transaction::method::update::validate_update_request;
/// #
...
pub fn validate_update_request(request: &Request) -> Result<()> {
    ...
    if request.call_id().is_none() {
        return Err(Error::Other(
            "UPDATE request missing Call-ID header".to_string(),
        ));
    }

    if request.from().is_none() {
        return Err(Error::Other(
            "UPDATE request missing From header".to_string(),
        ));
    }
    ...
    match request.to() {
        ...
        Some(to) => {
            ...
            if to_tag.is_none() || to_tag.unwrap().is_empty() {
                return Err(Error::Other(
                    "UPDATE request To header missing tag (must be in-dialog)".to_string(),
                ));
            }
        }
    }
    ...
    if request.cseq().is_none() {
        return Err(Error::Other(
            "UPDATE request missing CSeq header".to_string(),
        ));
    }
    ...
    let has_contact = request
        .headers
        .iter()
        .any(|h| matches!(h, TypedHeader::Contact(_)));
    if !has_contact {
...
mod tests {
    ...
    fn test_validate_update_request() {
        ...
        let mut request = Request::new(
            Method::Update,
            Uri::from_str("sip:alice@example.com").unwrap(),
        );
        ...
        assert!(
            validate_update_request(&method_request).is_err(),
        ...
        let mut no_tag_request = Request::new(
            Method::Update,
        ...
        assert!(
            result.is_err(),
            "UPDATE request without To tag should fail validation"
        );
        ...
        let mut empty_tag_request = Request::new(
            Method::Update,
            Uri::from_str("sip:bob@example.com").unwrap(),
        );
        ...
        assert!(
            result2.is_err(),
            "UPDATE request with empty To tag should fail validation"
        );
        ...
        request_no_contact
            .headers
            .retain(|h| !matches!(h, TypedHeader::Contact(_)));
        assert!(
            validate_update_request(&request_no_contact).is_ok(),
    ...
    fn test_create_update_invalid_input() {
        ...
        let out_of_dialog_invite = SimpleRequestBuilder::new(Method::Invite, "sip:bob@example.com")
            .unwrap()
            .from("Alice", "sip:alice@example.com", Some("alice-tag"))
