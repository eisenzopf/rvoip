...
use crate::transaction::client::builders::{ByeBuilder, InDialogRequestBuilder, InviteBuilder};
use crate::transaction::error::{Error, Result};
use crate::transaction::server::builders::{InviteResponseBuilder, ResponseBuilder};
...
pub use quick::{
    bye_for_dialog, info_for_dialog, info_for_dialog_with_extras, info_for_dialog_with_request_uri,
    message_for_dialog, message_for_dialog_with_extras, message_for_dialog_with_request_uri,
    message_out_of_dialog, message_out_of_dialog_with_extras, notify_for_dialog,
    notify_for_dialog_with_extras, notify_for_dialog_with_request_uri,
    options_out_of_dialog_with_extras, prack_for_dialog, prack_for_dialog_with_request_uri,
    refer_for_dialog, refer_for_dialog_with_contact, refer_for_dialog_with_extras,
    refer_for_dialog_with_request_uri, reinvite_for_dialog, reinvite_for_dialog_with_extras,
    reinvite_for_dialog_with_request_uri, response_for_dialog_transaction, subscribe_out_of_dialog,
    subscribe_out_of_dialog_with_extras, subscribe_out_of_dialog_with_identity,
    subscribe_out_of_dialog_with_identity_and_extras, update_for_dialog,
    update_for_dialog_with_contact, update_for_dialog_with_extras,
...
/// # Example
/// ```rust,no_run
/// use rvoip_sip_dialog::transaction::dialog::{DialogRequestTemplate, request_builder_from_dialog_template};
...
pub fn request_builder_from_dialog_template(
    ...
    let mut request = match method {
        ...
        Method::Bye => ByeBuilder::from_dialog_enhanced(
            &template.call_id,
            &template.from_uri,
            &template.from_tag,
            &template.to_uri,
        ...
        _ => {
            ...
            let mut builder = InDialogRequestBuilder::new(method).from_dialog_enhanced(
                &template.call_id,
                &template.from_uri,
                &template.from_tag,
                &template.to_uri,
...
/// # Example
/// ```rust,no_run
/// use rvoip_sip_dialog::transaction::dialog::{DialogTransactionContext, response_builder_for_dialog_transaction};
...
/// # Example
/// ```rust,no_run
/// use rvoip_sip_dialog::transaction::dialog::extract_dialog_template_from_request;
/// use rvoip_sip_dialog::transaction::client::builders::InviteBuilder;
/// use std::net::SocketAddr;
///
/// # fn main() -> Result<(), Box<dyn std::error::Error>> {
...
pub fn extract_dialog_template_from_request(
    ...
    let contact = request.header(&HeaderName::Contact).and_then(|header| {
        if let TypedHeader::Contact(contact_header) = header {
            let addresses: Vec<_> = contact_header.addresses().collect();
            addresses.first().map(|addr| addr.uri.to_string())
        } else {
            None
        }
    });
...
mod tests {
    ...
    async fn test_dialog_template_creation() {
        ...
        assert_eq!(
            bye_request.from().unwrap().tag().unwrap(),
            template.from_tag
        );
...
pub mod helpers {
    ...
    pub fn quick_notify_from_template(
        ...
        let builder = InDialogRequestBuilder::for_notify(event, body).from_dialog_enhanced(
            &template.call_id,
            &template.from_uri,
            &template.from_tag,
            &template.to_uri,
