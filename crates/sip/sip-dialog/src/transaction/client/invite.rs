...
impl TransactionLogic<ClientTransactionData, ClientInviteTimerHandles> for ClientInviteLogic {
    ...
    async fn handle_timer(
        ...
        match timer_name {
            ...
            _ => {
                warn!(id=%crate::transaction::safe_diagnostics::SafeTransactionKey::new(&tx_id), timer_name=%timer_name, "Unknown timer triggered for ClientInvite");
                Ok(None)
            }
        }
    }
                ...
                .await
            {
                Ok(_) => {
                    ...
                    if new_state != TransactionState::Calling {
                        tracing::trace!(
                            "WARNING: State didn't change to Calling, still: {:?}",
                            new_state
                        );
                    }
