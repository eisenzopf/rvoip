...
use rvoip_sip_core::builder::{
    ContentLengthBuilderExt, SimpleRequestBuilder, SimpleResponseBuilder,
};
...
impl SimplifiedTransactionCore {
    ...
    pub async fn create_udp_transport(
        ...
        let addr = self
            .transport
            .create_transport(TransportType::Udp, bind_addr)
            .await?;
        Ok(addr)
    }
            ...
            while let Some(event) = transport_rx.recv().await {
                match event {
                    TransportEvent::MessageReceived {
                        ...
                        match message {
                            Message::Request(request) => {
                                ...
                                let tx_event = TransactionEvent::NewRequest { request, source };
                            ...
                            Message::Response(response) => {
                                // Convert to a transaction event
                                let tx_event = TransactionEvent::NewResponse { response, source };
                    ...
                    TransportEvent::Error { error } => {
                        // Convert to a transaction event
                        let tx_event = TransactionEvent::Error { error };
    ...
    pub async fn send_request(
        ...
        self.transport
            .send_message(request.into(), destination)
            .await?;
        Ok(())
    }
    ...
    pub async fn send_response(
        ...
        self.transport
            .send_message(response.into(), destination)
            .await?;
        Ok(())
    }
...
async fn test_transport_with_transaction_core() {
    ...
    let server_addr = server_tx_core
        .create_udp_transport("127.0.0.1:0".parse().unwrap())
        .await
        .unwrap();
    ...
    client_tx_core
        .send_request(register_request.clone(), server_addr)
        .await
        .unwrap();
...
async fn test_transport_with_transaction_core_tcp() {
    ...
    // Send the request from client to server
    client_tx_core
        .send_request(invite_request.clone(), server_addr)
        .await
        .unwrap();
    ...
    // Check the received event
    if let Some(TransactionEvent::NewResponse {
        response,
        source: _,
    }) = client_event
    {
...
async fn test_transport_with_transaction_core_ws() {
    ...
    // Send the request from client to server
    client_tx_core
        .send_request(options_request.clone(), server_addr)
        .await
        .unwrap();
