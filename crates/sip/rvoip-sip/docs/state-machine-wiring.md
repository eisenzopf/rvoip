...
| BYE outbound | ByeBuilder::send / SessionHandle::hangup | n/a | SendOutboundBye / HangupCall | SendOutboundBye / HangupCall | SendBYEWithOptions, SendBYE | state-table | Local hangup sends BYE once the dialog is established; terminal confirmation or the retained exact-release fallback owns cleanup. |
...
| SUBSCRIBE | SubscribeBuilder::send / SubscribeRefreshBuilder::send | DialogToSessionEvent::NotifyReceived | n/a | n/a | UnifiedCoordinator::send_subscribe_oob_with_optional_auth | dialog-managed | Subscription sends are direct-wired with optional UAC auth retry; NOTIFY delivery is handled by dialog-core/session events. |
