...
pub const EVENT_WIRINGS: &[EventWiring] = &[
    ...
    EventWiring {
        ...
        kind: WiringKind::StateTable,
        notes: "Local hangup sends BYE once the dialog is established; the exact outbound transaction completion commits the terminal YAML transition before one retained release.",
    },
    EventWiring {
    ...
    EventWiring {
        sip_message: "NOTIFY inbound",
        caller: "rvoip-sip-dialog event_hub",
        infra_event: "DialogToSessionEvent::NotifyReceived",
        event_type: "ReceiveNOTIFY",
        yaml_event: "ReceiveNOTIFY",
        actions: &["ProcessNOTIFY"],
        kind: WiringKind::StateTable,
        notes: "Dialog-core validates and acknowledges the wire request; raw/dialog-package observations remain direct, while parsed REFER progress mutates exact session state through YAML before derived observations.",
    },
    EventWiring {
    ...
    EventWiring {
        ...
        actions: &["SendREGISTERWithOptions"],
        kind: WiringKind::StateTable,
        notes: "Registration owns lifecycle, auth, 423 retry, refresh, and unregister state through one retained options snapshot; legacy action variants are external-YAML compatibility facades only.",
    },
    EventWiring {
    ...
    EventWiring {
        ...
        event_type: "n/a",
        yaml_event: "n/a",
        actions: &["canonical 200", "publish Event::OptionsReceived"],
        kind: WiringKind::Direct,
        notes: "Dialog/transaction commits the canonical capability response before optional application observation; observer availability cannot affect the wire result.",
    },
    EventWiring {
        ...
        notes: "Subscription sends are direct-wired with optional UAC auth retry; inbound NOTIFY delivery is documented separately.",
    },
    EventWiring {
        sip_message: "PUBLISH",
        ...
        notes: "PUBLISH has no live rvoip-sip state-table row; the retained public dialog facade fails closed until a transaction-backed implementation is installed.",
    },
    EventWiring {
        sip_message: "Bridge",
