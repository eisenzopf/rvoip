...
use crate::types::{
    contact::Contact, from::From, sip_request::Request, sip_response::Response, to::To, uri::Uri,
};
#[cfg(test)]
...
pub fn add_audio_profile(builder: SdpBuilder, port: u16, codecs: Option<&[&str]>) -> SdpBuilder {
    ...
    for &codec in codecs {
        match codec {
            ...
            "9" => {
                media_builder = media_builder.rtpmap("9", "G722/8000");
            }
            "101" => {
...
pub fn add_video_profile(builder: SdpBuilder, port: u16, codecs: Option<&[&str]>) -> SdpBuilder {
    let codecs = codecs.unwrap_or(&["96", "97"]);

    let mut media_builder = builder
        .media_video(port, "RTP/AVP")
        .formats(codecs)
        .direction(MediaDirection::SendRecv);
...
mod tests {
    use super::*;
    use crate::types::{
        call_id::CallId, contact::ContactParamInfo, cseq::CSeq, headers::TypedHeader, Method,
        StatusCode, Uri,
    };
    use std::str::FromStr;
    ...
    fn test_sdp_from_request() {
        ...
        let invite = RequestBuilder::new(Method::Invite, "sip:bob@example.com")
            .unwrap()
            .header(TypedHeader::From(From::new(Address {
                display_name: Some("Alice".to_string()),
                uri: Uri::from_str("sip:alice@atlanta.com").unwrap(),
                params: vec![],
            })))
            .header(TypedHeader::To(To::new(Address {
                display_name: Some("Bob".to_string()),
                uri: Uri::from_str("sip:bob@example.com").unwrap(),
                params: vec![],
            })))
            .header(TypedHeader::CallId(CallId::new(
