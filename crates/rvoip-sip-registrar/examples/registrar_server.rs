...
use rvoip_sip_registrar::RegistrarService;
use std::sync::Arc;
use tracing::info;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config = rvoip_sip_registrar::types::RegistrarConfig::default();
    let registrar = Arc::new(
        RegistrarService::with_auth(rvoip_sip_registrar::api::ServiceMode::B2BUA, config, realm)
            .await?,
    );
