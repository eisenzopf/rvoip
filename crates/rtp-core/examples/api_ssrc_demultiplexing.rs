...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let example_timeout = tokio::time::timeout(Duration::from_secs(EXAMPLE_TIMEOUT_SECS), async {
        ...
        // Create server
        let server =
            rvoip_rtp_core::api::server::transport::DefaultMediaTransportServer::new(server_config)
                .await?;
        ...
        info!(
            "Server SSRC demultiplexing enabled: {}",
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                match tokio::time::timeout(Duration::from_millis(500), frame_receiver.recv()).await
                {
                    Ok(Ok((client_id, frame))) => {
                        info!(
                            "Received frame from client {} with SSRC={:08x}, PT={}, seq={}, ts={}",
                            client_id,
                            frame.ssrc,
                            frame.payload_type,
                            frame.sequence,
        ...
        // Create client
        let client =
            rvoip_rtp_core::api::client::transport::DefaultMediaTransportClient::new(client_config)
                .await?;
        ...
        info!(
            "Testing basic connectivity with default SSRC ({:08x})",
            session_ssrc
        );
