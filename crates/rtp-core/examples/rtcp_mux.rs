...
use rvoip_rtp_core::{
    packet::{
        rtcp::{NtpTimestamp, RtcpApplicationDefined, RtcpGoodbye, RtcpPacket, RtcpSenderReport},
        RtpPacket,
    },
    traits::RtpEvent,
            ...
            match events1.recv().await {
                Ok(RtpEvent::MediaReceived {
                    ...
                    if rtp_count >= 3 {
                        ...
                        let bye = RtcpGoodbye::new_with_reason(
                            0x12345678,
                            "Example test complete".to_string(),
                        );

                        // Send the RTCP packets using RTCP-MUX
                        let rtcp_packet = RtcpPacket::SenderReport(sr);
                        let serialized_sr = rtcp_packet
                            .serialize()
                            .expect("Failed to serialize RTCP SR");
                        println!(
                            "Endpoint 1 sending SR packet, first bytes: {:?}",
                            &serialized_sr[..4]
                        );
                        transport1_clone
                            .send_rtcp(&rtcp_packet, addr2)
                            .await
                            .expect("Failed to send RTCP SR");

                        let rtcp_packet = RtcpPacket::Goodbye(bye);
                        let serialized_bye = rtcp_packet
                            .serialize()
                            .expect("Failed to serialize RTCP BYE");
                        println!(
                            "Endpoint 1 sending BYE packet, first bytes: {:?}",
                            &serialized_bye[..4]
                        );
                        transport1_clone
                            .send_rtcp(&rtcp_packet, addr2)
                            .await
                            .expect("Failed to send RTCP BYE");
                ...
                Ok(RtpEvent::RtcpReceived { data, source }) => {
                    rtcp_count += 1;
                    println!(
                        "Endpoint 1 received RTCP packet: Size={}, From={}",
                        data.len(),
                        source
                    );
        ...
        loop {
            match events2.recv().await {
                ...
                Ok(RtpEvent::RtcpReceived { data, source }) => {
                    rtcp_count += 1;
                    println!(
                        "Endpoint 2 received RTCP packet: Size={}, From={}",
                        data.len(),
                        source
                    );
                    ...
                    match RtcpPacket::parse(&data) {
                        Ok(rtcp) => {
                            ...
                            if let RtcpPacket::Goodbye(_) = rtcp {
                                ...
                                transport2_clone
                                    .send_rtcp(&rtcp_packet, addr1)
                                    .await
                                    .expect("Failed to send RTCP APP");
