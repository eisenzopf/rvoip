...
use rvoip_rtp_core::{
    payload::{create_payload_format, PayloadFormat, PayloadFormatFactory, PayloadType},
    RtpHeader, RtpPacket, RtpTimestamp,
};
        ...
        if let Some(payload_enum) = enum_type {
            ...
            if let Some(format) = format_result {
                ...
                if actual_clock_rate == pt_info.clock_rate {
                    info!(
                        "✅ {} (PT={}) has correct clock rate: {} Hz",
                        pt_info.encoding_name, pt_info.pt, pt_info.clock_rate
                    );
                } else {
                    info!(
                        "❌ {} (PT={}) has incorrect clock rate: {} Hz, expected: {} Hz",
                ...
                if pt_info.media_type == "audio" && pt_info.channels.is_some() {
                    ...
                    if actual_channels == expected_channels {
                        info!(
                            "✅ {} (PT={}) has correct channel count: {}",
                            pt_info.encoding_name, pt_info.pt, expected_channels
                        );
                    } else {
                        info!(
                            "❌ {} (PT={}) has incorrect channel count: {}, expected: {}",
                ...
                info!(
                    "⚠️ {} (PT={}) format handler not implemented",
...
fn test_payload_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(pcmu_format) = create_payload_format(PayloadType::PCMU, None) {
        ...
        info!(
            "G.711 µ-law: Original size: {}, Packed size: {}, Unpacked size: {}",
            pcmu_data.len(),
            pcmu_packed.len(),
            pcmu_unpacked.len()
        );
    ...
    if let Some(g722_format) = create_payload_format(PayloadType::G722, None) {
        ...
        info!(
            "G.722: Original size: {}, Packed size: {}, Unpacked size: {}",
            g722_data.len(),
            g722_packed.len(),
            g722_unpacked.len()
        );
    ...
    info!(
        "RTP packet with PCMU: Successfully serialized ({} bytes) and deserialized",
        serialized.len()
    );
    info!(
        "RTP packet header: PT={}, seq={}, timestamp={}",
...
fn test_timestamp_calculations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(pcmu_format) = create_payload_format(PayloadType::PCMU, None) {
        ...
        let pcmu_ts_increment =
            ((pcmu_format.clock_rate() as f64) * (pcmu_duration as f64) / 1000.0) as u32;

        info!(
            "G.711 µ-law: {} samples ({}ms) = {} timestamp increment",
    ...
    if let Some(g722_format) = create_payload_format(PayloadType::G722, None) {
        ...
        // Calculate timestamp increment manually
        let g722_ts_increment =
            ((g722_format.clock_rate() as f64) * (g722_duration as f64) / 1000.0) as u32;

        info!(
            "G.722: {} samples ({}ms) = {} timestamp increment",
    ...
    if let Some(opus_format) = create_payload_format(PayloadType::Opus, Some(2)) {
        ...
        // Calculate timestamp increment manually
        let opus_ts_increment =
            ((opus_format.clock_rate() as f64) * (opus_duration as f64) / 1000.0) as u32;

        info!(
            "Opus: {} samples ({}ms) = {} timestamp increment",
