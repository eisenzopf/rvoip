...
use rvoip_rtp_core::{
    buffer::{
        AdaptiveJitterBuffer, BufferLimits, BufferPool, GlobalBufferManager, JitterBufferConfig,
        PacketPriority, SharedPools, TransmitBuffer, TransmitBufferConfig,
    },
...
impl StreamReceiver {
    ...
    async fn run_receiver(&mut self) {
            ...
            if self.jitter_buffer.add_packet(packet).await {
                ...
                while let Some(packet) = self.jitter_buffer.get_next_packet().await {
                    ...
                    if packets_played % 100 == 0 {
                        debug!(
                            "Stream {:08x}: Played {} packets",
                            self.ssrc, packets_played
                        );
                    }
                }
            }
        ...
        info!(
            ...
            self.ssrc, packets_received, packets_played, stats.jitter_ms, stats.discontinuities
        );
    }
}
...
fn create_stream_pair(
    ...
    let jitter_buffer =
        AdaptiveJitterBuffer::with_buffer_manager(jitter_config, buffer_manager.clone());
    let transmit_buffer =
        TransmitBuffer::with_buffer_manager(ssrc, transmit_config, buffer_manager, packet_pool);
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!(
        "Simulating {} streams with {} packets each",
        NUM_STREAMS, PACKETS_PER_STREAM
    );
