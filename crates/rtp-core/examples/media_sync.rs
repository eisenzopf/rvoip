...
use bytes::Bytes;
use rvoip_rtp_core::{
    packet::rtcp::{NtpTimestamp, RtcpPacket, RtcpSenderReport},
    MediaClock, MediaSync, RtpSession, RtpSessionConfig, RtpSessionEvent, TimestampMapper,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let audio_wallclock = mapper
        .rtp_to_wallclock(AUDIO_SSRC, audio_capture_timestamp)
        .expect("Failed to convert audio timestamp to wallclock time");

    let video_wallclock = mapper
        .rtp_to_wallclock(VIDEO_SSRC, video_capture_timestamp)
        .expect("Failed to convert video timestamp to wallclock time");

    // Calculate offset between streams
    let audio_to_video_offset = mapper
        .get_sync_offset(AUDIO_SSRC, VIDEO_SSRC)
        .expect("Failed to calculate sync offset");

    info!(
        "Audio timestamp: {}, maps to wallclock {:?}",
        audio_capture_timestamp, audio_wallclock
    );

    info!(
        "Video timestamp: {}, maps to wallclock {:?}",
        video_capture_timestamp, video_wallclock
    );

    // The offset will be approximately 100ms
    info!(
        "Calculated synchronization offset: {:.2}ms",
        audio_to_video_offset
    );
    ...
    if play_immediately {
        ...
        let corrected_video_ts = mapper
            .map_timestamp(AUDIO_SSRC, VIDEO_SSRC, audio_capture_timestamp)
    ...
    info!(
        "Video timestamp with drift compensation: {}",
        video_ts_with_drift
    );
    info!(
        "Video timestamp without compensation: {}",
        video_ts_without_drift
    );
    ...
    let video_from_audio = media_sync.convert_timestamp(AUDIO_SSRC, VIDEO_SSRC, AUDIO_CLOCK_RATE);

    if let Some(video_ts) = video_from_audio {
        info!(
            "1s audio timestamp ({}) maps to video timestamp: {}",
    ...
    if let Some(ntp) = ntp_from_rtp {
        info!(
            "1s audio timestamp ({}) maps to NTP timestamp: {:?}",
            rtp_1sec, ntp
        );
    }
