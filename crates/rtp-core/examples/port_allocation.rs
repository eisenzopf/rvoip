...
async fn test_individual_port_allocation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let sequential_allocator =
        rvoip_rtp_core::transport::PortAllocator::with_config(config.clone());

    // Allocate a few ports with Sequential strategy
    info!("Allocating ports with Sequential strategy:");
    for i in 0..3 {
        let port = sequential_allocator
            .allocate_port(IpAddr::V4(Ipv4Addr::LOCALHOST))
            .await?;
        info!("  Port {}: {}", i + 1, port);
    }
    ...
    // Release all ports
    let total_ports =
        sequential_allocator.allocated_count().await + random_allocator.allocated_count().await;
...
async fn test_adjacent_pair_allocation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..3 {
        let (rtp_addr, rtcp_addr) = allocator
            .allocate_port_pair(&format!("test-adjacent-{}", i), None)
            .await?;
        info!(
            "  Pair {}: RTP: {}, RTCP: {:?}",
            i + 1,
            rtp_addr.port(),
            rtcp_addr.map(|a| a.port())
        );
    }
...
async fn test_rtcp_mux_allocation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..3 {
        let (rtp_addr, rtcp_addr) = allocator
            .allocate_port_pair(&format!("test-mux-{}", i), None)
            .await?;
        info!(
            "  Port {}: RTP: {}, RTCP: {:?}",
            i + 1,
...
async fn test_port_reuse() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Release the port
    allocator
        .release_port(IpAddr::V4(Ipv4Addr::LOCALHOST), port1)
        .await;
    ...
    // Release the port
    allocator
        .release_port(IpAddr::V4(Ipv4Addr::LOCALHOST), port2)
        .await;
