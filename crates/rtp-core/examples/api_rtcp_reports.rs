...
async fn run_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "    - Packet loss: {:.2}%",
        client_stats.packet_loss_percent
    );
    ...
    println!(
        "    - Packet loss: {:.2}%",
        server_stats.packet_loss_percent
    );
    ...
    if !clients.is_empty() {
        ...
        println!(
            "12. Sending RTCP Receiver Report to specific client ({})",
            first_client_id
        );
        server
            .send_rtcp_receiver_report_to_client(first_client_id)
            .await?;
    }
...
) -> Result<impl MediaTransportClient, Box<dyn std::error::Error>> {
...
async fn setup_server_events(
    server: &impl MediaTransportServer,
    ...
    server
        .on_event(Box::new(|event| match event {
            ...
            MediaTransportEvent::FrameReceived(frame) => {
                println!(
                    "[SERVER EVENT] Received frame: seq={}, size={} bytes",
                    frame.sequence,
                    frame.data.len()
                );
            }
            MediaTransportEvent::RtcpReport {
                jitter,
                packet_loss,
                round_trip_time,
            } => {
                println!(
                    "[SERVER EVENT] RTCP Report - Jitter: {:.2}ms, Loss: {:.2}%, RTT: {:?}ms",
                    jitter,
                    packet_loss * 100.0,
                    round_trip_time.map(|rtt| rtt.as_millis())
                );
            }
...
async fn setup_client_events(
    client: &impl MediaTransportClient,
) -> Result<(), Box<dyn std::error::Error>> {
    client
        .on_event(Box::new(|event| match event {
            MediaTransportEvent::Connected => {
            ...
            MediaTransportEvent::FrameReceived(frame) => {
                println!(
                    "[CLIENT EVENT] Received frame: seq={}, size={} bytes",
                    frame.sequence,
                    frame.data.len()
                );
            }
            MediaTransportEvent::RtcpReport {
                ...
                println!(
                    "[CLIENT EVENT] RTCP Report - Jitter: {:.2}ms, Loss: {:.2}%, RTT: {:?}ms",
                    jitter,
                    packet_loss * 100.0,
                    round_trip_time.map(|rtt| rtt.as_millis())
                );
