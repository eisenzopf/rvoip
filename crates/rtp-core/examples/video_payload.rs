...
use bytes::Bytes;
use rvoip_rtp_core::{
    create_payload_format, PayloadFormat, PayloadType, Vp8PayloadFormat, Vp9PayloadFormat,
};
...
fn main() {
    ...
    info!(
        "VP8: PT={}, Clock Rate={} Hz, Frame Rate={} fps, Bitrate={} bps",
    ...
    info!(
        "VP9: PT={}, Clock Rate={} Hz, Frame Rate={} fps, Bitrate={} bps",
    ...
    let timestamp_increment_vp8 =
        vp8_format.samples_from_duration(vp8_format.preferred_packet_duration());
    info!(
        "VP8: {} ms at 90kHz = {} samples for RTP timestamp increment",
        vp8_format.preferred_packet_duration(),
        timestamp_increment_vp8
    );

    // Calculate frame size information for VP9
    let timestamp_increment_vp9 =
        vp9_format.samples_from_duration(vp9_format.preferred_packet_duration());
    info!(
        "VP9: {} ms at 90kHz = {} samples for RTP timestamp increment",
    ...
    info!(
        "VP9 with advanced features has {} byte descriptor overhead",
        advanced_descriptor_size
    );
    ...
    if let Some(format) = create_payload_format(PayloadType::Dynamic(98), Some(90000)) {
        info!(
            "Created format for PT=98 with clock rate 90kHz: {}",
            format.payload_type()
        );
        ...
        info!(
            "A packet size of {} bytes represents ~{} ms of video at default bitrate",
            mtu, duration
        );
    }
    ...
    if let Some(format) = create_payload_format(PayloadType::Dynamic(99), Some(90000)) {
        info!(
            "Created format for PT=99 with clock rate 90kHz: {}",
            format.payload_type()
        );
    }
}
