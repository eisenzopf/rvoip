...
use bytes::Bytes;
use rvoip_rtp_core::{create_payload_format, G722PayloadFormat, PayloadFormat, PayloadType};
...
fn main() {
    ...
    info!(
        "  - {} ms of audio = {} samples at 16kHz",
        ms_duration, actual_samples
    );
    info!(
        "  - RTP timestamp increases by only {} (8kHz rate)",
        rtp_timestamp_inc
    );
