...
use rvoip_rtp_core::api::{
    ...
    server::{transport::MediaTransportServer, ServerConfigBuilder, ServerFactory},
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    server
        .configure_header_extension(1, "urn:ietf:params:rtp-hdrext:ssrc-audio-level".to_string())
        .await?;
