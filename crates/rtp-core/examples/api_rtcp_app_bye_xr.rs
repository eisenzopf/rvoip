...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let example_timeout = tokio::time::timeout(
        ...
        async {
            ...
            if !clients.is_empty() {
                server
                    .send_rtcp_app_to_client(&clients[0].id, "SVRT", server_app_data)
                    .await?;
            } else {
            ...
            if !clients.is_empty() {
                server
                    .send_rtcp_xr_voip_metrics_to_client(&clients[0].id, server_metrics)
                    .await?;
            } else {
            ...
            client
                .send_rtcp_bye(Some("Example completed".to_string()))
                .await?;
