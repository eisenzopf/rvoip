...
use bytes::Bytes;
use rvoip_rtp_core::{
    create_payload_format, OpusBandwidth, OpusPayloadFormat, PayloadFormat, PayloadType,
};
...
fn main() {
    ...
    info!(
        "Mono Opus: PT={}, Channels={}, Bitrate={} bit/s, Bandwidth={}",
    ...
    info!(
        "Stereo Opus: PT={}, Channels={}, Bitrate={} bit/s, Bandwidth={}",
    ...
    let mono_encoded = simulate_opus_encoding(
        &pcm_data,
        opus_mono.max_bitrate(),
        opus_mono.preferred_packet_duration(),
    );
    info!(
        "Encoded to {} bytes of Opus data (mono)",
        mono_encoded.len()
    );
    ...
    info!(
        "20ms at 48kHz = {} samples for RTP timestamp increment",
        timestamp_increment
    );
    ...
    if let Some(PayloadType::Dynamic(pt)) = Some(PayloadType::Dynamic(101)) {
        match create_payload_format(PayloadType::Dynamic(pt), None) {
            Some(format) => {
                ...
                info!(
                    "20ms of Opus at default bitrate = {} bytes (max)",
                    packet_size
                );
            }
            None => {
