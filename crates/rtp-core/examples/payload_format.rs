...
use bytes::Bytes;
use rvoip_rtp_core::{
    create_payload_format, G711APayloadFormat, G711UPayloadFormat, PayloadFormat, PayloadType,
};
...
fn main() {
    ...
    info!(
        "Packed {} bytes of audio data into {} bytes of payload",
        pcm_data.len(),
        payload.len()
    );
    ...
    info!(
        "Unpacked payload back to {} bytes of audio data",
        unpacked.len()
    );
    ...
    info!(
        "20ms at 8kHz = {} samples, which is {}ms",
        samples, duration
    );
    ...
    match create_payload_format(payload_type, None) {
        Some(format) => {
            ...
            info!(
                "20ms packet size = {} bytes, which is {}ms",
                packet_size, calculated_duration
            );
        }
        None => {
    ...
    info!(
        "G.711 A-law format: PT={}, rate={}Hz, channels={}",
