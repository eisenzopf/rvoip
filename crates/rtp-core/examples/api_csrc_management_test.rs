...
async fn run_test() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!(
        "Sending dummy frame from client to server to establish connection with SSRC={:08x}",
        client_ssrc
    );
    ...
    info!(
        "CSRC management is {} on server",
        if server_csrc_enabled {
            "enabled"
        } else {
            "disabled"
        }
    ...
    info!(
        "CSRC management is {} on client",
        if client_csrc_enabled {
            "enabled"
        } else {
            "disabled"
        }
    ...
    for &(name, ssrc) in &participants {
        ...
        server
            .update_csrc_cname(ssrc, format!("{}@example.com", name.to_lowercase()))
            .await?;
        server
            .update_csrc_display_name(ssrc, name.to_string())
            .await?;
        info!(
            "Added CSRC mapping to server for {}: SSRC={:08x}",
            name, ssrc
        );
        ...
        client
            ...
            .await?;
        info!(
            "Added CSRC mapping to client for {}: SSRC={:08x}",
            name, ssrc
        );
    }
    ...
    let receive_task = tokio::spawn(async move {
        ...
        while !shutdown_clone.load(Ordering::SeqCst) {
            if let Ok(Some(frame)) = client_clone.receive_frame(Duration::from_millis(100)).await {
                info!(
                    "Received frame: SSRC={:08x}, PT={}, SEQ={}, Size={} bytes",
                    frame.ssrc,
                    frame.payload_type,
                    frame.sequence,
                    frame.data.len()
    ...
    info!(
        "Running CSRC test with {} active sources",
        mixer.get_active_sources().len()
    );
    ...
    if !clients.is_empty() {
        for client_info in &clients {
            ...
            info!(
                "Test: Server sending mixed packet to client ID: {}",
                client_id
            );
            ...
            info!(
                "Frame will include {} active CSRCs: {:?}",
                active_csrcs.len(),
                active_csrcs
            );
    ...
    for i in 1..=3 {
        ...
        info!(
            "Client frame will include {} active CSRCs: {:?}",
            active_csrcs.len(),
            active_csrcs
        );
    ...
    info!(
        "Received packet with CSRCs: {}",
        packet_received.load(Ordering::SeqCst)
    );
