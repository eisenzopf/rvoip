...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        while let Ok(event) = sender_events.recv().await {
            match event {
                RtpSessionEvent::PacketReceived(packet) => {
                    info!(
                        "Sender received packet: PT={}, SEQ={}, SSRC={:08x}",
    ...
    let receiver_monitor = tokio::spawn(async move {
        while let Ok(event) = receiver_events.recv().await {
            match event {
                RtpSessionEvent::PacketReceived(packet) => {
                    info!(
                        "Receiver received packet: PT={}, SEQ={}, SSRC={:08x}",
                        packet.header.payload_type,
                        packet.header.sequence_number,
                        packet.header.ssrc
                    );
    ...
    for i in 0..10 {
        ...
        // Send packet
        sender_session
            .send_packet(1000 + i * 160, payload, i == 0)
            .await?;
    ...
    info!(
        "Sender stats: Sent={}, Received={}, Jitter={}ms",
        sender_stats.packets_sent, sender_stats.packets_received, sender_stats.jitter_ms
    );

    info!(
        "Receiver stats: Sent={}, Received={}, Jitter={}ms, Lost={}",
