...
impl RtpMixer {
    ...
    fn mix_output(&mut self, sample_count: usize) -> RtpPacket {
        ...
        let active_sources: Vec<&SourceBuffer> =
            self.sources.values().filter(|s| s.is_active()).collect();

        info!(
            "Mixing {} active sources out of {} total",
            active_sources.len(),
            self.sources.len()
        );
        ...
        let active_ssrcs: Vec<RtpSsrc> = active_sources
            .iter()
            .map(|s| s.ssrc)
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 1..=20 {
        ...
        if !csrcs.is_empty() {
            let active_names: Vec<String> = csrcs
                .iter()
