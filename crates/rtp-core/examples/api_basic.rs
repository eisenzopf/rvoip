...
async fn main() -> Result<(), ExampleError> {
    ...
    std::thread::spawn(|| {
        ...
        std::thread::sleep(Duration::from_secs(
            MAX_RUNTIME_SECONDS + FORCE_KILL_AFTER_SECONDS,
        ));
    ...
    while !connected && retry_count < CONNECTION_RETRY_COUNT {
        ...
        .await
        {
            ...
            Err(_) => {
                warn!(
                    "Client connection attempt {} timed out after {} seconds",
                    retry_count + 1,
    ...
    match time::timeout(Duration::from_secs(1), client.is_connected()).await {
        ...
        Err(_) => {
            ...
            return Err(ExampleError(
                "Connection verification timed out".to_string(),
            ));
        }
    }
    ...
    for attempt in 0..3 {
        match time::timeout(
            ...
            client.send_frame(test_frame.clone()),
        )
        .await
        {
            Ok(result) => match result {
