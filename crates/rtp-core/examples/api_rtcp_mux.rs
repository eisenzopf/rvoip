...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match tokio::time::timeout(Duration::from_secs(15), async {
        ...
        println!(
            "Server without RTCP-MUX is bound to: {}",
            server_no_mux_addr
        );
        ...
        match tokio::time::timeout(Duration::from_millis(250), server_with_mux.receive_frame())
            .await
        {
            Ok(Ok((client_id, received_frame))) => {
                println!("Server received frame from client {}", client_id);
                println!(
                    "  Frame info: type={:?}, seq={}, size={} bytes",
        ...
        match client_without_mux.connect().await {
            Ok(_) => {
                ...
                println!(
                    "Client without RTCP-MUX is bound to: {}",
                    client_no_mux_addr
                );
...
async fn create_server(
    rtcp_mux: bool,
) -> Result<impl MediaTransportServer, Box<dyn std::error::Error>> {
