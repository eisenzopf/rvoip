...
fn generate_master_key() -> Vec<u8> {
    ...
    vec![
        ...
        0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
        0x88, // Salt material
...
async fn demo_key_rotation() -> Result<(), ExampleError> {
    ...
    for (name, policy) in policies {
        ...
        info!(
            "  Initial state: generation={}, elapsed={:?}",
        ...
        info!(
            "  After rotation: generation={}, elapsed={:?}",
...
async fn demo_multi_stream_syndication() -> Result<(), ExampleError> {
    ...
    for (name, config) in configs {
        ...
        key_manager
            .initialize(generate_master_key())
            .await
        ...
        let sessions = vec![
            ...
            (
                "charlie-dave-video",
                vec![StreamType::Audio, StreamType::Video],
            ),
            (
                "enterprise-conference",
                vec![StreamType::Audio, StreamType::Video, StreamType::Data],
            ),
        ];
        ...
        for (session_id, additional_streams) in sessions {
            ...
            // Create session
            syndication
                .create_session(session_id.to_string(), generate_master_key())
            ...
            info!(
                "  Created session '{}' with {} streams",
                session_id,
                additional_streams.len()
            );
        }
    ...
    for (name, config) in configs {
        ...
        let test_failures = vec![
            (
                ...
                SecurityError::Network("Connection timeout".to_string()),
            ),
            (
                KeyExchangeMethod::DtlsSrtp,
                SecurityError::CryptoError("Certificate validation failed".to_string()),
            ),
            (
                KeyExchangeMethod::PreSharedKey,
                SecurityError::Configuration("Invalid key format".to_string()),
            ),
        ];

        for (method, error) in test_failures {
            info!(
                "  Simulating failure: {:?} method with {:?}",
                method,
                FailureType::from_error(&error)
            );
            ...
            match recovery_manager.handle_failure(method, error).await {
                Ok(action) => {
                    ...
                    info!(
                        "    Recovery state: {:?}",
                        recovery_manager.get_state().await
                    );
                }
                Err(e) => {
        ...
        if let Some((method, count)) = stats.most_problematic_method() {
            info!(
                "    Most problematic method: {:?} ({} failures)",
                method, count
            );
        }
...
async fn demo_security_policy() -> Result<(), ExampleError> {
    ...
    for (name, policy) in policies {
        ...
        let test_configs = vec![
            ...
            (
                "Valid PSK",
                SecurityConfig::srtp_with_key(generate_master_key()),
            ),
        ...
        for (config_name, config) in test_configs {
            match policy.validate_config(&config) {
                Ok(()) => {
                    info!(
                        "    ✅ {} configuration passed policy validation",
                        config_name
                    );
                }
                Err(e) => {
        ...
        let rotation_policies = vec![
            (
                "Quick rotation",
                KeyRotationPolicy::TimeInterval(Duration::from_secs(300)),
            ),
            (
                "Standard rotation",
                KeyRotationPolicy::enterprise_standard(),
            ),
...
async fn demo_production_scenario() -> Result<(), ExampleError> {
    ...
    info!(
        "  Syndication: {:?} streams",
        syndication_config.stream_types
    );
    ...
    let conference_sessions = vec![
        (
            "board-meeting-room-a",
            vec![StreamType::Audio, StreamType::Video, StreamType::Control],
        ),
        (
            "team-standup-room-b",
            vec![StreamType::Audio, StreamType::Video],
    ...
    match recovery_manager
        .handle_failure(KeyExchangeMethod::Mikey, incident_error)
        .await
    {
        Ok(action) => {
    ...
    info!(
        "  ✅ Key rotation completed (generation {})",
