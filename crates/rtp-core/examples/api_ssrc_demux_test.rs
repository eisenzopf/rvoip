...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = tokio::time::timeout(Duration::from_secs(TEST_TIMEOUT_SECS), run_test()).await;
...
async fn run_test() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!(
        "Server SSRC demultiplexing initially enabled: {}",
        server_demux_enabled
    );
    ...
    let server_demux_enabled = server.is_ssrc_demultiplexing_enabled().await?;
    info!(
        "Server SSRC demultiplexing now enabled: {}",
        server_demux_enabled
    );
    ...
    info!(
        "Client SSRC demultiplexing enabled: {}",
        client_demux_enabled
    );

    // Pre-register the audio SSRC
    info!(
        "Pre-registering AUDIO_SSRC {:08x} for demultiplexing",
        AUDIO_SSRC
    );
    ...
    info!(
        "Pre-registering VIDEO_SSRC {:08x} for demultiplexing",
        VIDEO_SSRC
    );
    ...
    tokio::spawn(async move {
        ...
        loop {
            match tokio::time::timeout(Duration::from_millis(100), server_clone.receive_frame())
                .await
            {
                Ok(Ok((client_id, frame))) => {
                    info!(
                        "Received frame from client {}: SSRC={:08x}, PT={}, seq={}, len={}",
                        client_id,
                        frame.ssrc,
                        frame.payload_type,
                        frame.sequence,
        ...
        && attempts < 50
    {
        ...
        if attempts % 10 == 0 {
            ...
            info!(
                ...
                audio_packet_received.load(Ordering::SeqCst)
            );
            info!(
    ...
    for (i, (client_id, frame)) in frames.iter().enumerate() {
        if let Ok(text) = std::str::from_utf8(&frame.data) {
            ...
            info!(
                "Frame #{}: client={}, SSRC={:08x}, binary data, len={}",
                i + 1,
                client_id,
                frame.ssrc,
                frame.data.len()
    ...
    if audio_received && video_received {
        ...
        let msg = format!(
            "FAILURE: Not all test packets were received. AUDIO={}, VIDEO={}",
