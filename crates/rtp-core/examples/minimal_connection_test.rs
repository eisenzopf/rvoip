...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    server
        .on_client_disconnected(Box::new(|client_info| {
            println!(
    ...
    let monitor_task = tokio::spawn(async move {
        ...
        for i in 0..20 {
            match tokio::time::timeout(Duration::from_millis(100), frame_receiver.recv()).await {
                ...
                Ok(Err(e)) => {
                    if i % 5 == 0 {
                        // Only print every 5th error to reduce noise
                ...
                Err(_) => {
                    ...
                    if i % 5 == 0 {
                        // Only print every 5th timeout to reduce noise
            ...
            match server_clone.get_clients().await {
                Ok(clients) => {
                    if i % 5 == 0 || !clients.is_empty() {
                        // Print every 5th update or whenever clients exist
                        println!("[MONITOR] Server has {} clients", clients.len());
                        for client in &clients {
                            println!(
                                "[MONITOR]   - Client: {}, Address: {}",
    ...
    println!(
        "\n[CLIENT] Creating raw UDP client socket on {}",
        client_addr
    );
    ...
    println!(
        "[CLIENT] Sending test packets to server at {}",
        actual_bind_address
    );
    ...
    client_socket
        .send_to(&rtp_packet, actual_bind_address)
        .await?;
    println!(
        "[CLIENT] Sent {} bytes of proper RTP data",
        rtp_packet.len()
    );
    ...
    client_socket
        .send_to(&malformed_rtp, actual_bind_address)
        .await?;
    println!(
        "[CLIENT] Sent {} bytes of malformed RTP data",
        malformed_rtp.len()
    );
    ...
    client_socket
        .send_to(&random_data, actual_bind_address)
        .await?;
