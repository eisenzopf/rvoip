...
use rvoip_rtp_core::{
    packet::extension::{
        ids::{AUDIO_LEVEL, TRANSPORT_CC, VIDEO_ORIENTATION},
...
fn create_packet_with_two_byte_extensions() -> Result<RtpPacket, Box<dyn std::error::Error>> {
    ...
    let voi_data = vec![
        (if is_front_camera { 0x40 } else { 0x00 })
    ...
    info!(
        "  - Video orientation: front={}, flipped={}, rotation={}°",
        is_front_camera,
        is_flipped,
        rotation * 90
    );
...
fn parse_and_display_extensions(
    packet: &RtpPacket,
    format_name: &str,
        ...
        for element in &extensions.elements {
            ...
            match element.id {
                ...
                _ => {
                    ...
                    let hex_data: Vec<String> = element
                        .data
                        .iter()
                        .take(8)
                        .map(|b| format!("{:02x}", b))
...
fn demonstrate_header_convenience_methods() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Check what's left
    info!(
        "  - Extensions after removal: {}",
        if let Some(exts) = &header.extensions {
            exts.len()
        } else {
            0
        }
    ...
    info!(
        "  - Changed format to: {:?}",
        header.extension_format().unwrap()
    );

    // Clear extensions
    header.clear_extensions();
    info!(
        "  - Extensions after clearing: {}",
        if let Some(exts) = &header.extensions {
            exts.len()
        } else {
            0
    ...
    for (ssrc, name, is_speaking, level) in participants {
        ...
        if let Some(audio_level) = received.header.get_extension(AUDIO_LEVEL) {
            if audio_level.len() >= 1 {
                ...
                info!(
                    "  Participant {}: SSRC=0x{:08x}, speaking={}, level={}dB",
