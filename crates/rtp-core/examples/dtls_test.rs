...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let server_task = tokio::spawn(async move {
        ...
        let (initial_packet, client_addr) = {
            ...
            println!(
                "Server received initial packet: {} bytes from {}",
                packet.len(),
                addr
            );
            ...
            match rvoip_rtp_core::dtls::record::Record::parse_multiple(&packet) {
                Ok(records) => {
                    ...
                    for record in records {
                        ...
                        if record.header.content_type
                            == rvoip_rtp_core::dtls::record::ContentType::Handshake
                        {
                            // Parse the handshake message
                            if let Ok((header, _)) =
                                rvoip_rtp_core::dtls::message::handshake::HandshakeHeader::parse(
                                    &record.data,
                                )
                            {
