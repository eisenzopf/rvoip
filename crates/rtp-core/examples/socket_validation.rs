...
use rvoip_rtp_core::transport::{PlatformSocketStrategy, PlatformType, RtpSocketValidator};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!(
        "Default socket strategy for this platform: {:#?}",
        default_strategy
    );
    ...
    match &validation_result {
        Ok(ref strategy) => {
            ...
            if strategy.use_reuse_addr != default_strategy.use_reuse_addr {
                info!(
                    "Use SO_REUSEADDR changed from {} to {}",
            ...
            if strategy.use_reuse_port != default_strategy.use_reuse_port {
                info!(
                    "Use SO_REUSEPORT changed from {} to {}",
            ...
            if strategy.buffer_size != default_strategy.buffer_size {
                info!(
                    "Buffer size changed from {} to {} bytes",
            ...
            if strategy.rebind_wait_time_ms != default_strategy.rebind_wait_time_ms {
                info!(
                    "Rebind wait time changed from {} to {} ms",
    ...
    info!(
        "Waiting {}ms before rebinding...",
        strategy.rebind_wait_time_ms
    );
