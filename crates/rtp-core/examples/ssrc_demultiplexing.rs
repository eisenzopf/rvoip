...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for _ in 0..10 {
        ...
        info!(
            "Currently detected {} out of {} expected SSRCs",
            current_count, expected_ssrc_count
        );
    ...
    for &ssrc in &ssrcs_to_check {
        if let Some(stream_stats) = receiver_guard.get_stream(ssrc).await {
            info!(
                "Stream with SSRC={:08x} has {} packets received",
    ...
    info!(
        "Listing all {} streams:",
        receiver_guard.stream_count().await
    );
    let all_streams = receiver_guard.get_all_streams().await;
    for stream in all_streams {
        info!(
            "Stream SSRC={:08x}: packets={}, lost={}, jitter={}ms",
            stream.ssrc, stream.packets_received, stream.packets_lost, stream.jitter
        );
    }
    ...
    if total_streams_found == ssrcs_to_check.len() {
        info!(
            "✅ Success! All {} expected streams were demultiplexed correctly",
            ssrcs_to_check.len()
        );
    } else {
        info!(
            "⚠️ Warning: Only found {} out of {} expected streams",
            total_streams_found,
            ssrcs_to_check.len()
        );
    }
