...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!(
        "Generated VoIP metrics with R-factor={} (MOS-LQ={}, MOS-CQ={})",
    ...
    info!(
        "Serialized compound packet of {} bytes containing SR, XR, and BYE",
        compound_bytes.len()
    );
    ...
    info!(
        "Successfully parsed compound packet with {} RTCP packets",
    ...
    for (i, packet) in parsed_compound.packets.iter().enumerate() {
        match packet {
            ...
            RtcpPacket::ExtendedReport(xr) => {
                info!(
                    "Packet {}: Extended Report from SSRC 0x{:08x} with {} blocks",
                    i,
                    xr.ssrc,
                    xr.blocks.len()
                );
            ...
            RtcpPacket::Goodbye(bye) => {
                info!(
                    "Packet {}: Goodbye from {} sources (Reason: {:?})",
                    i,
                    bye.sources.len(),
                    bye.reason
                );
