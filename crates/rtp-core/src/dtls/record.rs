...
impl RecordHeader {
    ...
    pub fn parse(data: &[u8]) -> Result<(Self, usize)> {
        ...
        let version = match version_raw {
            ...
            _ => {
                return Err(crate::error::Error::InvalidProtocolVersion(format!(
                    "Invalid DTLS version: {:#x}",
                    version_raw
                )));
            }
        };
...
impl Record {
    ...
    pub fn parse(data: &[u8]) -> Result<(Self, usize)> {
        ...
        if data.len() < 13 + length {
            println!(
                "Record data too short: {} bytes, need {}",
                data.len(),
                13 + length
            );
            return Err(crate::error::Error::PacketTooShort);
        ...
        println!(
            "Successfully parsed DTLS record: type={:?}, epoch={}, seq={}, len={}",
