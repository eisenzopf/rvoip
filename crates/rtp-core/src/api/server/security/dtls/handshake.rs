...
pub async fn process_handshake_step(
    ...
    match step {
        ...
        HandshakeStep::ReceivedClientKeyExchange => {
            ...
            if let Err(e) = conn.continue_handshake().await {
                warn!(
                    "Failed to continue handshake after ClientKeyExchange: {}",
                    e
                );
            } else {
                ...
                *completed = true;
                ...
                match keys::extract_srtp_keys(conn, address, false).await {
                    Ok(ctx) => {
                        ...
                        *srtp_guard = Some(ctx);
                        debug!(
                            "Server successfully extracted SRTP keys for client {}",
                            address
                        );
                    }
...
pub async fn wait_for_handshake(
    ...
    debug!(
        "Waiting for DTLS handshake completion for client {}",
        address
    );
        ...
        Ok(_) => {
            ...
            if let Some(conn) = conn_guard.as_ref() {
                ...
                Err(SecurityError::NotInitialized(
                    "DTLS connection not available".to_string(),
                ))
            }
        }
        Err(e) => {
...
pub async fn process_dtls_packet(
    ...
    debug!(
        "Server processing DTLS packet of {} bytes from client {}",
        data.len(),
        address
    );
    ...
    match conn.process_packet(data).await {
        Ok(_) => {
            ...
            if let Some(step) = conn.handshake_step() {
                process_handshake_step(conn, step, address, handshake_completed, srtp_context)
                    .await?;
            }
