...
pub async fn send_rtcp_app_to_client(
    ...
    // Send to client
    transport
        .send_rtcp_bytes(&rtcp_data, client_addr)
        .await
        .map_err(|e| MediaTransportError::RtcpError(format!("Failed to send APP packet: {}", e)))?;

    debug!(
        "Sent RTCP APP packet to client {}: name={}, data_len={}",
        client_id,
        name,
        data.len()
    );
...
pub async fn send_rtcp_bye_to_client(
    ...
    debug!(
        "Sent RTCP BYE packet to client {}: reason={:?}",
        client_id, reason_clone
    );
...
pub async fn send_rtcp_xr_voip_metrics_to_client(
    ...
    debug!(
        "Sent RTCP XR VoIP metrics to client {}: loss_rate={}%, r_factor={}",
