...
pub async fn process_packet(
    ...
    debug!(
        "Client received packet of {} bytes - delegating to DTLS library",
        data.len()
    );
    ...
    if let Some(conn) = conn_guard.as_mut() {
        ...
        if let Err(e) = conn.process_packet(data).await {
            ...
            return Err(SecurityError::HandshakeError(format!(
    ...
    if let Some(conn) = conn_guard.as_mut() {
        ...
        if data.len() >= 14 && data[0] == 22 && data[13] == 3 {
            ...
            if let Err(e) = conn.process_packet(data).await {
                ...
                return Err(SecurityError::HandshakeError(format!(
                    "Failed to process HelloVerifyRequest: {}",
                    e
                )));
            }
            ...
            if let Some(addr) = *remote_addr_guard {
                ...
                if conn.handshake_step()
                    == Some(crate::dtls::handshake::HandshakeStep::ReceivedHelloVerifyRequest)
                {
                    ...
                    if let Err(e) = conn.continue_handshake().await {
                        ...
                        return Err(SecurityError::HandshakeError(format!(
                            "Failed to continue handshake with cookie: {}",
                            e
                        )));
                    } else {
                    ...
                    if let Err(e) = conn.start_handshake(addr).await {
                        ...
                        return Err(SecurityError::HandshakeError(format!(
                            "Failed to restart handshake with cookie: {}",
                            e
                        )));
                    } else {
                    ...
                    if !*completed {
                        *completed = true;
                        ...
                        if conn.state() == crate::dtls::connection::ConnectionState::Connected {
                            ...
                            // Extract keys
                            if let Err(e) = keys::extract_srtp_keys(
                                connection,
                                srtp_context,
                                handshake_completed,
                            )
    ...
    let addr = match remote_addr {
        ...
        None => {
            return Err(SecurityError::Configuration(
                "Remote address not set".to_string(),
            ))
        }
    };
    ...
    if let Some(conn) = conn_guard.as_mut() {
            ...
            == Some(crate::dtls::handshake::HandshakeStep::ReceivedHelloVerifyRequest)
        {
            ...
            Err(SecurityError::HandshakeError(
                "Not in ReceivedHelloVerifyRequest state".to_string(),
            ))
        }
    } else {
        Err(SecurityError::NotInitialized(
