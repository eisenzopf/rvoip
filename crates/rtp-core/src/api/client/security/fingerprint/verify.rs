    ...
    if let Some(conn) = conn_guard.as_ref() {
        ...
        if let Some(cert) = conn.remote_certificate() {
            ...
            let algorithm = match remote_fingerprint_algorithm.to_lowercase().as_str() {
                ...
                "sha-512" => "SHA-512",
                _ => {
                    return Err(SecurityError::HandshakeVerification(format!(
            ...
            match cert_copy.fingerprint(algorithm) {
                Ok(actual_fingerprint) => {
                    ...
                    if matches {
                        ...
                        warn!(
                            "Fingerprint verification failed! Expected: {}, Actual: {}",
                ...
                Err(e) => Err(SecurityError::HandshakeVerification(format!(
                    "Failed to generate fingerprint for verification: {}",
                    e
                ))),
            }
        } else {
            Err(SecurityError::HandshakeVerification(
                "No remote certificate available for verification".to_string(),
            ))
        }
    } else {
        Err(SecurityError::NotInitialized(
