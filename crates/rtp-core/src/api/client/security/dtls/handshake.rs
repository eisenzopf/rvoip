...
pub async fn start_handshake_monitor(
    ...
    let remote_addr = match remote_addr_guard.as_ref() {
        ...
        None => {
            ...
            return Err(SecurityError::Configuration(
                "Remote address not set for handshake monitor".to_string(),
            ));
        }
    };
    ...
    let socket = match socket_guard.as_ref() {
        ...
        None => {
            handshake_monitor_running.store(false, Ordering::SeqCst);
            return Err(SecurityError::Configuration(
                "Socket not set for handshake monitor".to_string(),
            ));
        }
    };
    drop(socket_guard);
    ...
    tokio::spawn(async move {
        ...
        while attempt <= max_attempts {
            ...
            for _ in 0..30 {
                // Check status every 100ms for up to 3 seconds
            ...
            warn!(
                "Creating new DTLS connection for retry (attempt #{})",
                attempt + 1
            );
