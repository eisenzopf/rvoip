...
impl PlatformSocketStrategy {
    ...
    pub async fn apply_to_socket(&self, socket: &UdpSocket) -> io::Result<()> {
        ...
        #[cfg(unix)]
        {
            ...
            if unsafe { libc::setsockopt(fd, libc::SOL_SOCKET, libc::SO_RCVBUF, optval, optlen) }
                < 0
            {
            ...
            if unsafe { libc::setsockopt(fd, libc::SOL_SOCKET, libc::SO_SNDBUF, optval, optlen) }
                < 0
            {
                return Err(io::Error::last_os_error());
            }
            ...
            if self.use_reuse_addr {
                ...
                if unsafe {
                    libc::setsockopt(fd, libc::SOL_SOCKET, libc::SO_REUSEADDR, optval, optlen)
                } < 0
                {
                    return Err(io::Error::last_os_error());
                }
            ...
            if self.set_ipv6_only && socket.local_addr()?.is_ipv6() {
                let optval =
                    if self.ipv6_only { &1 } else { &0 } as *const i32 as *const libc::c_void;
                if unsafe {
                    libc::setsockopt(fd, libc::IPPROTO_IPV6, libc::IPV6_V6ONLY, optval, optlen)
                } < 0
                {
                    return Err(io::Error::last_os_error());
                }
        ...
        #[cfg(windows)]
        {
            ...
            if unsafe {
                ws2::setsockopt(raw_socket, ws2::SOL_SOCKET, ws2::SO_RCVBUF, optval, optlen)
            } != 0
            {
                return Err(io::Error::last_os_error());
            }
            ...
            // Set send buffer size
            if unsafe {
                ws2::setsockopt(raw_socket, ws2::SOL_SOCKET, ws2::SO_SNDBUF, optval, optlen)
            } != 0
            {
                return Err(io::Error::last_os_error());
            }
            ...
            if self.set_ipv6_only && socket.local_addr()?.is_ipv6() {
                let optval =
                    if self.ipv6_only { &1 } else { &0 } as *const i32 as *const libc::c_char;
                const IPV6_V6ONLY: i32 = 27; // Defined in ws2ipdef.h but not in winapi crate
                if unsafe {
                    ws2::setsockopt(
                        raw_socket,
                        ws2def::IPPROTO_IPV6 as i32,
                        IPV6_V6ONLY,
                        optval,
                        optlen,
                    )
...
impl RtpSocketValidator {
    ...
    async fn test_binding_pair_with_strategy(strategy: &PlatformSocketStrategy) -> Result<()> {
        ...
        let rtcp_result = tokio::time::timeout(tokio::time::Duration::from_secs(1), async {
        ...
        match rtcp_result {
            Ok(Ok(())) => {
                debug!(
                    "Successfully bound and tested RTP/RTCP socket pair on ports {}/{}",
                    rtp_port, rtcp_port
                );
                Ok(())
            }
            Ok(Err(e)) => Err(Error::Transport(format!(
                "RTP/RTCP pair test failed: {}",
                e
            ))),
            Err(_) => Err(Error::Transport("RTP/RTCP pair test timed out".to_string())),
        }
    }
    ...
    async fn test_rtcp_mux_binding_with_strategy(strategy: &PlatformSocketStrategy) -> Result<()> {
        ...
        // Use a separate task to handle timeouts more gracefully
        let mux_result = tokio::time::timeout(tokio::time::Duration::from_secs(1), async {
        ...
        match mux_result {
            ...
            Ok(Err(e)) => Err(Error::Transport(format!("RTCP-MUX test failed: {}", e))),
            Err(_) => Err(Error::Transport("RTCP-MUX test timed out".to_string())),
        }
    }
...
mod tests {
    ...
    fn test_socket_validation() {
        ...
        let result = rt.block_on(async { RtpSocketValidator::validate().await });
