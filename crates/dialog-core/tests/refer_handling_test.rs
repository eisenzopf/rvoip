...
use rvoip_dialog_core::{api::unified::UnifiedDialogApi, config::DialogManagerConfig};
use rvoip_sip_core::{
...
async fn create_test_dialog_api(port: u16) -> Arc<UnifiedDialogApi> {
    ...
    let config = DialogManagerConfig::hybrid(format!("127.0.0.1:{}", port).parse().unwrap())
...
async fn test_refer_creates_transfer_request_event() {
    ...
    // Create dialog API. Dialog-level event coordination lives in the
    // GlobalEventCoordinator bus now; this test only verifies REFER request
    // structure, so no per-test session-coordinator wiring is needed.
    ...
    dialog_api
        .start()
        .await
        .expect("Failed to start dialog API");

    // Create an established dialog first (simulate active call)
    let _call_handle = dialog_api
        .make_call("sip:alice@127.0.0.1:40001", "sip:bob@127.0.0.1:40002", None)
        .await
        .expect("Failed to create call");

    // Create REFER request for blind transfer using SimpleRequestBuilder
    let refer_request = SimpleRequestBuilder::new(Method::Refer, "sip:alice@127.0.0.1:40001")
        .unwrap()
    ...
    assert!(
        ...
        "ReferTo header MUST be present as a header"
    );
    assert_eq!(
...
async fn test_refer_without_dialog_returns_481() {
    ...
    let refer_request = SimpleRequestBuilder::new(Method::Refer, "sip:alice@127.0.0.1:40010")
        .unwrap()
    ...
    assert_eq!(refer_request.body().len(), 0, "REFER body should be empty");
...
async fn test_refer_with_replaces_header() {
    ...
    let _call_handle = dialog_api
        .make_call("sip:alice@127.0.0.1:40020", "sip:bob@127.0.0.1:40021", None)
        .await
        .expect("Failed to create call");

    // Create REFER with Replaces for attended transfer
    let refer_request = SimpleRequestBuilder::new(Method::Refer, "sip:alice@127.0.0.1:40020")
        .unwrap()
    ...
    assert!(
        refer_to_header.is_some(),
        "ReferTo header should be present for attended transfer"
    );
    ...
    if let Some(refer_to) = refer_to_header {
        ...
        assert!(
            uri_str.contains("Replaces="),
            "Refer-To URI should contain Replaces parameter"
        );
        assert!(
            uri_str.contains("other-call-id"),
            "Refer-To URI should contain the call ID to replace"
        );
    }
