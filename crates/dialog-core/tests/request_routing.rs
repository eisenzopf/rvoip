...
fn test_dialog_tuple_extraction_real() {
    ...
    println!(
        "✅ Real dialog tuple extraction working: ({}, {}, {})",
...
fn test_request_dialog_matching_real() {
    ...
    println!(
        "✅ Real SIP request Call-ID matches dialog: {}",
        request_tuple.0
    );
...
fn test_request_classification_real() {
    ...
    let in_dialog_methods = vec![Method::Bye, Method::Ack, Method::Info, Method::Update];

    for method in in_dialog_methods {
        assert!(
            is_in_dialog_method(&method),
            "Method {:?} should be in-dialog",
            method
        );
    ...
    for method in new_dialog_methods {
        assert!(
            !is_in_dialog_method(&method),
            "Method {:?} should create new dialog",
            method
        );
...
fn test_route_set_handling_real() {
    ...
    println!(
        "✅ Real request created with correct remote target: {}",
        request.target_uri
    );
    ...
    println!(
        "✅ Real route set properly maintained: {} routes",
...
fn test_dialog_perspective_matching_real() {
    ...
    println!(
        "✅ Real dialogs share same Call-ID: {}",
        initiator_dialog.call_id
    );
    ...
    println!(
        "   Initiator: {} -> {}",
        initiator_dialog.local_uri, initiator_dialog.remote_uri
    );
    println!(
        "   Recipient: {} -> {}",
        recipient_dialog.local_uri, recipient_dialog.remote_uri
    );
}
...
fn test_sequence_number_validation_real() {
    ...
    println!(
        "✅ Initial sequence numbers: local={}, remote={}",
        dialog.local_cseq, dialog.remote_cseq
    );
    ...
    println!(
        "✅ INFO request incremented sequence to: {}",
        dialog.local_cseq
    );
    ...
    println!(
        "✅ BYE request incremented sequence to: {}",
        dialog.local_cseq
    );
    ...
    println!(
        "✅ ACK request correctly did NOT increment sequence: {}",
        dialog.local_cseq
    );
}
...
fn extract_dialog_tuple_from_request(request: &Request) -> (String, String, String) {
    ...
    let from_tag = match request.header(&HeaderName::From) {
        Some(TypedHeader::From(from)) => from.tag().unwrap_or("unknown").to_string(),
        _ => "unknown".to_string(),
    };

    let to_tag = match request.header(&HeaderName::To) {
        Some(TypedHeader::To(to)) => to.tag().unwrap_or("unknown").to_string(),
        _ => "unknown".to_string(),
    };
...
fn is_in_dialog_method(method: &Method) -> bool {
    matches!(
        method,
        Method::Bye | Method::Ack | Method::Info | Method::Update | Method::Prack
    )
}
