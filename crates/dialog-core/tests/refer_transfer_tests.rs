...
fn test_transfer_request_with_replaces() {
    ...
    match event {
        SessionCoordinationEvent::TransferRequest { replaces, .. } => {
            assert_eq!(
                replaces,
                Some("call-id=abc123;to-tag=456;from-tag=789".to_string())
            );
        }
        _ => panic!("Expected TransferRequest event"),
    }
...
mod integration_tests {
    ...
    fn test_refer_request_parsing() {
        ...
        let referred_by_header = request
            .headers
            .iter()
            .find(|h| {
                matches!(
                    h,
                    rvoip_sip_core::types::headers::TypedHeader::ReferredBy(_)
                )
            })
            .expect("Should have Referred-By header");

        if let rvoip_sip_core::types::headers::TypedHeader::ReferredBy(referred_by) =
            referred_by_header
        {
            assert!(
                referred_by
    ...
    fn test_blind_transfer_headers() {
        ...
        assert!(
            replaces.is_none(),
            "Should NOT have Replaces header for blind transfer"
        );
    }
}
