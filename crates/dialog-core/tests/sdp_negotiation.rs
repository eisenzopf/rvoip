...
fn test_real_reinvite_sdp_negotiation() {
    ...
    println!(
        "✅ Real re-INVITE created with incremented sequence: {}",
        dialog.local_cseq
    );
...
fn test_real_sdp_session_modification() {
    ...
    println!(
        "✅ Real codec change re-INVITE created (seq={})",
        dialog.local_cseq
    );
...
fn test_real_concurrent_sdp_negotiations() {
    ...
    println!(
        "   Dialog1: {} -> {}",
        dialog1.local_uri, dialog1.remote_uri
    );
    println!(
        "   Dialog2: {} -> {}",
        dialog2.local_uri, dialog2.remote_uri
    );
...
fn create_real_200_ok_response_with_sdp() -> Response {
    ...
    let response = SimpleResponseBuilder::new(StatusCode::Ok, Some("OK")).build();
