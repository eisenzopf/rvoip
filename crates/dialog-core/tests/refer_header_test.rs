...
async fn test_incoming_refer_with_proper_header() {
    ...
    let refer_request = SimpleRequestBuilder::new(Method::Refer, "sip:bob@127.0.0.1:45010")
        .unwrap()
...
async fn test_refer_request_structure() {
    ...
    assert_eq!(
        refer_request.method(),
        Method::Refer,
        "Method must be REFER"
    );
...
async fn test_refer_processing_end_to_end() {
    ...
    let refer_request = SimpleRequestBuilder::new(Method::Refer, "sip:bob@127.0.0.1:45030")
        .unwrap()
