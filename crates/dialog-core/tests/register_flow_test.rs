...
async fn create_test_dialog_api(port: u16) -> Arc<UnifiedDialogApi> {
    ...
    let (transaction_manager, global_rx) =
        TransactionManager::with_transport_manager(transport, transport_rx, Some(100))
            .await
...
async fn test_send_register_basic() {
    ...
    match result {
        Err(_) => {
            ...
            tracing::info!(
                "✅ send_register() sent request (timed out waiting for response - expected)"
            );
        }
        Ok(Ok(response)) => {
            // Got a response somehow - that's also OK
            tracing::info!(
                "✅ send_register() got response: {}",
        ...
        Ok(Err(e)) => {
            ...
            panic!(
                "send_register() should not fail to build/send request: {}",
                e
            );
        }
    }
...
async fn test_send_register_with_authorization() {
    ...
    match result {
        ...
        Ok(Ok(response)) => {
            tracing::info!(
                "✅ send_register() with Authorization got response: {}",
                response.status_code()
            );
        }
        Ok(Err(e)) => {
...
async fn test_register_with_real_server() {
    ...
    let response = dialog_api
        .send_register(registrar_uri, from_uri, contact_uri, expires, None)
        .await
    ...
    let www_auth = response
        .raw_header_value(&rvoip_sip_core::types::header::HeaderName::WwwAuthenticate)
...
async fn test_send_register_validates_uri() {
    ...
    let result = dialog_api
        .send_register(invalid_uri, from_uri, contact_uri, 3600, None)
        .await;
...
async fn test_send_register_concurrent() {
    ...
    assert!(
        ...
        "All concurrent REGISTERs should succeed"
    );
