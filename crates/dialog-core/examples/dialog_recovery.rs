...
impl DialogRecoveryExample {
    ...
    async fn run_dialog_state_recovery(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let dialog1 = self
            .primary_api
            .create_dialog(local_uri, remote_uri)
            .await?;
        let dialog2 = self
            .primary_api
            .create_dialog(local_uri, "sip:test2@example.com")
            .await?;

        info!(
            "✅ Created test dialogs: {} and {}",
            dialog1.id(),
            dialog2.id()
        );
        ...
        for i in 0..2 {
            ...
            match call_result {
                ...
                Err(e) => {
                    info!(
                        "  ⚠️  Recovery call {} failed (expected in test): {}",
                        i + 1,
                        e
                    );
        ...
        info!(
            "📊 Initial stats: {} active dialogs",
            initial_stats.active_dialogs
        );
        ...
        info!(
            "📊 Post-operation stats: {} active dialogs",
            persistent_stats.active_dialogs
        );
    ...
    async fn run_service_recovery(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        info!(
            ...
            self.backup_api.supports_outgoing_calls()
        );
        info!(
            "   • Supports incoming calls: {}",
            self.backup_api.supports_incoming_calls()
        );
        ...
        let recovery_dialog = self
            .backup_api
            .create_dialog(backup_uri, "sip:recovery@example.com")
            .await?;

        info!(
            "✅ Created recovery dialog on backup API: {}",
            recovery_dialog.id()
        );

        // Demonstrate real call establishment on backup API
        let backup_call_result = self
            .backup_api
            .make_call(backup_uri, "sip:backup-target@example.com", None)
            .await;
        match backup_call_result {
            ...
            Err(e) => {
                ...
                info!(
                    "💡 In production, backup API establishes real dialogs for service continuity"
                );
            }
        }
        ...
        info!(
            "📊 Backup API stats: {} active dialogs",
            backup_stats.active_dialogs
        );
    ...
    async fn run_recovery_coordination(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let coord_dialog = self
            .primary_api
            .create_dialog(&local_uri, "sip:coordination@example.com")
            .await?;
