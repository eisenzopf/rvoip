...
impl RegisterHandler for DialogManager {
    ...
    async fn handle_register_method(&self, request: Request, source: SocketAddr) -> DialogResult<()> {
        ...
        if self.should_auto_respond_to_register() {
            ...
            use rvoip_infra_common::events::cross_crate::{RvoipCrossCrateEvent, DialogToSessionEvent};
            let event = RvoipCrossCrateEvent::DialogToSession(
                DialogToSessionEvent::IncomingRegister {
                    ...
                    expires,
                    authorization,
                    call_id: request.call_id().map(|cid| cid.value().to_string())
...
impl DialogManager {
    ...
    pub async fn send_register_response(
        ...
        use rvoip_sip_core::{StatusCode, ResponseBuilder, TypedHeader};
        ...
        let request = self.transaction_manager
        ...
        if let Some(cseq) = request.typed_header::<rvoip_sip_core::types::cseq::CSeq>() {
            debug!("Original request has CSeq: {} {}", cseq.sequence(), cseq.method());
        } else {
        ...
        let status = StatusCode::from_u16(status_code)
            .map_err(|e| DialogError::protocol_error(&format!("Invalid status code {}: {}", status_code, e)))?;
        
        // Build response using response builder
        let response = if status_code == 401 {
            // Build 401 Unauthorized with WWW-Authenticate header
            let mut resp = crate::transaction::utils::response_builders::create_response(&request, status);
