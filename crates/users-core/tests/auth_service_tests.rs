...
async fn test_complete_authentication_flow() {
    ...
    let auth_result = auth_service
        .authenticate_password("testuser", "SecurePass123")
        .await
        .unwrap();
    ...
    let fail_result = auth_service
        .authenticate_password("testuser", "WrongPassword123")
        .await;
...
async fn test_password_validation() {
    ...
    let test_cases = vec![
        (
            "short1",
            "Your password needs to be at least 12 characters long. Try using a passphrase!",
        ),
        (
            "alllowercase123",
...
async fn test_change_password() {
    ...
    let initial_auth = auth_service
        .authenticate_password("changepass", "OldPassword123")
        .await
        .unwrap();

    // Change password
    auth_service
        .change_password(&user.id, "OldPassword123", "NewPassword456")
        .await
        .unwrap();

    // Old password should fail
    let old_pass_result = auth_service
        .authenticate_password("changepass", "OldPassword123")
        .await;
    ...
    let new_auth = auth_service
        .authenticate_password("changepass", "NewPassword456")
        .await
        .unwrap();
...
async fn test_inactive_user_cannot_authenticate() {
    ...
    let auth_result = auth_service
        .authenticate_password("inactiveuser", "SecurePass2024")
        .await;
...
async fn test_jwt_claims_content() {
    ...
    let auth_result = auth_service
        .authenticate_password("claimstest", "SecurePass2024")
        .await
        .unwrap();
