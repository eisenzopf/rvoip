...
async fn test_password_validation() {
    ...
    for (i, password) in invalid_passwords.iter().enumerate() {
        let username = if *password == "TestUser0" {
            "TestUser0"
        } else {
            &format!("invaliduser{}", i)
        };
    ...
    for (i, (input, expected_sanitized)) in xss_attempts.iter().enumerate() {
        ...
        if let Ok(user) = result {
            ...
            assert!(
                ...
                "Display name should not contain '<': {}",
                display_name
            );
            assert!(
                !display_name.contains('>'),
    ...
    for search in dangerous_searches {
        ...
        match result {
            Ok(users) => {
                ...
                assert!(
                    users.len() <= 1,
                    "Dangerous search '{}' should not return multiple users",
                    search
                );
            }
            Err(_) => {
...
async fn test_api_key_name_validation() {
    ...
    // Valid API key names
    let valid_names = vec!["my-api-key", "Production Key", "test_key_123"];
    ...
    for name in valid_names {
        ...
        assert!(
            result.is_ok(),
            "Valid API key name '{}' should be accepted",
            name
        );
    }
    ...
    for name in invalid_names {
        ...
        assert!(
            result.is_err(),
            "Invalid API key name '{}' should be rejected",
            name
        );
    }
}
