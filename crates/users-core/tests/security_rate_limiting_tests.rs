...
async fn test_ip_rate_limiting() {
    ...
    for i in 0..10 {
        let result = limiter
            .check_rate_limit(RateLimitIdentifier::Ip("192.168.1.1".to_string()))
            .await;
        assert!(result.is_ok(), "Request {} should be allowed", i + 1);
    }
    ...
    let result = limiter
        .check_rate_limit(RateLimitIdentifier::Ip("192.168.1.2".to_string()))
        .await;
    assert!(
        result.is_ok(),
...
async fn test_successful_login_clears_failed_attempts() {
    ...
    for i in 0..3 {
        ...
        if i < 2 {
            assert!(
                result.is_ok(),
                "Failed attempt {} should be allowed after reset",
                i + 1
            );
        } else {
            assert!(matches!(result, Err(RateLimitError::AccountLocked(_))));
...
async fn test_rate_limit_window_reset() {
    ...
    for _ in 0..5 {
        let result = limiter
            .check_rate_limit(RateLimitIdentifier::User("test_user".to_string()))
            .await;
        assert!(result.is_ok());
    }
    ...
    for i in 0..15 {
        ...
        let handle = tokio::spawn(async move {
            let result = limiter_clone
                .check_rate_limit(RateLimitIdentifier::User("concurrent_user".to_string()))
                .await;
            (i, result)
        });
        handles.push(handle);
...
async fn test_different_identifiers_separate_limits() {
    ...
    let result = limiter
        .check_rate_limit(RateLimitIdentifier::Ip("user1".to_string()))
        .await;
    assert!(
        result.is_ok(),
        "IP identifier should have separate limit from User identifier"
    );
}
...
async fn test_lockout_duration_in_error() {
    ...
    match result {
        Err(RateLimitError::AccountLocked(duration)) => {
            ...
            assert!(
                duration.as_secs() >= 14 * 60 && duration.as_secs() <= 15 * 60,
                "Lockout duration should be approximately 15 minutes, got {} seconds",
                duration.as_secs()
            );
        }
