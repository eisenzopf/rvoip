#!/usr/bin/env bash
# DTMF example: server auto-answers, client sends DTMF digits.
set -euo pipefail
...
cargo run -p rvoip-session-core-v3 --example streampeer_dtmf_server --quiet \
  2>&1 | sed "s/^/$(printf '\033[0;32m')[SERVER]$(printf '\033[0m') /" &
sleep 2

echo -e "${CYAN}[CLIENT]${NC} Starting DTMF sender"
cargo run -p rvoip-session-core-v3 --example streampeer_dtmf_client --quiet \
