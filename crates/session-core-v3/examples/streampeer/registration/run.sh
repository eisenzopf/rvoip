...
cargo run -p rvoip-session-core-v3 --example streampeer_registration_server --quiet \
  2>&1 | sed "s/^/$(printf '\033[0;32m')[REGISTRAR]$(printf '\033[0m') /" &
sleep 2

echo -e "${CYAN}[CLIENT]${NC} Starting"
cargo run -p rvoip-session-core-v3 --example streampeer_registration_client --quiet \
