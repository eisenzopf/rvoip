...
cargo run -p rvoip-session-core-v3 --example streampeer_bridge_carol --quiet \
  2>&1 | sed "s/^/$(printf '\033[0;35m')[CAROL]$(printf '\033[0m') /" &
sleep 1

echo -e "${GREEN}[BRIDGE]${NC} Starting (b2bua)"
cargo run -p rvoip-session-core-v3 --example streampeer_bridge_peer --quiet \
  2>&1 | sed "s/^/$(printf '\033[0;32m')[BRIDGE]$(printf '\033[0m') /" &
sleep 1

echo -e "${CYAN}[ALICE]${NC} Starting (caller)"
cargo run -p rvoip-session-core-v3 --example streampeer_bridge_alice --quiet \
