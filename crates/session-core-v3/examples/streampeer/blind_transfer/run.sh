...
cargo run -p rvoip-session-core-v3 --example streampeer_blind_transfer_charlie --quiet \
  2>&1 | sed "s/^/$(printf '\033[1;33m')[CHARLIE]$(printf '\033[0m') /" &
sleep 1

echo -e "${CYAN}[BOB]${NC} Starting (transferor) on port 5061"
cargo run -p rvoip-session-core-v3 --example streampeer_blind_transfer_bob --quiet \
  2>&1 | sed "s/^/$(printf '\033[0;36m')[BOB]$(printf '\033[0m') /" &
sleep 1

echo -e "${GREEN}[ALICE]${NC} Starting (caller) on port 5060"
cargo run -p rvoip-session-core-v3 --example streampeer_blind_transfer_alice --quiet \
