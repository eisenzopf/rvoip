...
cargo run -p rvoip-session-core-v3 --example callbackpeer_auto_answer_server --quiet \
  2>&1 | sed "s/^/$(printf '\033[0;32m')[SERVER]$(printf '\033[0m') /" &
sleep 2

echo -e "${CYAN}[CLIENT]${NC} Starting caller"
cargo run -p rvoip-session-core-v3 --example callbackpeer_auto_answer_client --quiet \
