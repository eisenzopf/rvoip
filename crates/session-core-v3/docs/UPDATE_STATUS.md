...
`session-core-v3` does **not** currently use UPDATE anywhere in its public API.
...
| Direction | Status |
|-----------|--------|
| Send UPDATE (UAC / UAS mid-dialog) | **Implemented** in `dialog-core/src/transaction/method/update.rs` — not currently invoked from session-core-v3 |
...
layer) but do not route any session-core-v3 public API through it until session
