...
`session-core-v3` supports two distinct audio topologies. They operate at
...
```rust
use rvoip_session_core_v3::{StreamPeer, AudioSource};
...
- State-machine auto-switchback: `crates/session-core-v3/state_machine/actions.rs`
  (`Action::SwitchToPassThroughOnActive`), wired in `state_tables/default.yaml`.
- Tests: `crates/session-core-v3/tests/early_media_tests.rs` (state-table
  wiring), `crates/session-core-v3/examples/streampeer/prack/` (end-to-end).
...
```rust
use rvoip_session_core_v3::{UnifiedCoordinator, AudioSource};
...
- Tests: `crates/session-core-v3/tests/bridge_roundtrip_integration.rs`
