...
| Session timer refresh-failure BYE | ⚠️ (wire-level implemented; end-to-end test still blocked — dialog-core's session_timer.rs fires `SessionRefreshed` optimistically on send, not on response, so a dead peer isn't yet observable as `SessionRefreshFailed`. Needs session_timer.rs to subscribe to UPDATE transaction events before an e2e test is meaningful.) |
