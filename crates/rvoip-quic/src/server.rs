...
async fn spawn_peer_session(
    ...
    let _coord = match subscription_handler {
        ...
        None => UctpCoordinator::start("quic", in_rx, out_tx, coord_events_tx, bearer),
    };
        ...
        tokio::spawn(async move {
            while let Some(event) = coord_events_rx.recv().await {
                let adapter_event = match event {
                    ...
                    | UctpSessionEvent::MediaFrame { connid, .. } => AdapterEvent::Native {
                        kind: "uctp.internal",
                        detail: connid.to_string(),
                    },
                    UctpSessionEvent::NegotiationFailed { sid, reason } => AdapterEvent::Native {
