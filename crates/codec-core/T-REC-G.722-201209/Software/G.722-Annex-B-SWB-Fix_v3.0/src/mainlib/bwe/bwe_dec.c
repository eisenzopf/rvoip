...
{	
  ...
  IF(L_sub(L_deposit_l(sAverageMag), L_shl(32, Shift)) < 0)
  {	
                       ...
                       )
{
  ...
  { 
    ...
    { 
      L_temp = L_add(L_mult0(temp, 12345), 20101L);
  ...
  { 
    ...
    array_oper(SWB_NORMAL_FENV, norm, pre_sEnv, pre_sEnv, &shl); 
  } 
  pit_fix = pre_sEnv; 
  ...
  *scoef_SWBQ = s_min(*scoef_SWBQ, pre_scoef_SWBQ); 
  ...
  array_oper(SWB_NORMAL_FENV, abs_s(temp), pit_fix, pit_fix, &shr); 
  ...
  IF (sub(mode, TRANSIENT) == 0)
  {
    FOR (i = 0; i < SWB_TRANSI_FENV; i++)
    {
      ...
      {
        L_Extract( norm_fix, &iACC_hi, &iACC_lo );
        iACC =Mpy_32_16( iACC_hi, iACC_lo,iEnv_fix[i] );
        L_Extract( iACC, &iACC_hi, &iACC_lo );
    ...
    IF (s_and(bit_switch_flag, 1) == 0 )   /* test if bit_switch_flag == 0 or 2 */
    {
      FOR (i = 0; i < SWB_NORMAL_FENV; i++)
      {
        ...
        IF(L_sub(L_temp, L_temp1) <= 0 && sub(pre_mode, TRANSIENT) != 0)
        {
          ...
          iACC = L_mac(iACC,temp, pre_sEnv[i]);
          if_Env_fix = round_fx(iACC);
        }
        pit_fix -= FENV_WIDTH;
        L_Extract(norm_fix,&iACC_hi,&iACC_lo);

        L_temp = Mpy_32_16( iACC_hi,iACC_lo,if_Env_fix);   /* 27 + scoef_SWBQ - 15 */
        L_Extract(L_temp,&iACC_hi,&iACC_lo);
                        ...
                        )
{
  ...
  zero16( NUM_FENV_VECT, index_fEnv_codeword);
  zero16( L_FRAME_WB, sY);
  zero16( SWB_NORMAL_FENV, sfEnv);
...
#endif
  /*****************************/ 
  mov16(SWB_NORMAL_FENV, dec_st->pre_fEnv, pre_sfEnv);
  ...
  IF (sub(ploss_status, 1) == 0)
  {
    ...
    {
      dec_st->spGain_sm[i] = extract_l_L_shr(L_add(L_deposit_l(dec_st->spGain_sm[i]), L_deposit_l(spGain[i]) ), 1); /* Q(14) */ 
      move16();
    ...
    *sig_Mode = NORMAL;		move16();	
    ...
    array_oper(SWB_NORMAL_FENV, 27853, dec_st->pre_fEnv, dec_st->pre_fEnv, &mult_r);
  ...
  ELSE
  {
    ...
    IF (s_and(bit_switch_flag, 1) == 0)                 /* test if bit_switch_flag == 0 or 2 */
    {
      ...
      IF(sub(mode, TRANSIENT) == 0)  /* normal frame */
      {
        ...
        *scoef_SWBQ = sub(add(Shift1, Shift), 17);
        ...
        mov16(SWB_NORMAL_FENV, sfEnv, sFenv_SVQ);

        sFenv_SVQQ = *scoef_SWBQ;		move16();
      }
      ELSE
      {
        index_fEnv_codebook[0] = GetBit(pBit, 1);	move16();
        index_fEnv_codebook[1] = GetBit(pBit, 1);	move16();
        ...
        array_oper(SWB_NORMAL_FENV, Shift, sfEnv, sfEnv, &shl);
        ...
        *scoef_SWBQ = sub(*scoef_SWBQ, index_fGain);
        ...
        {								
          ...
          L_temp = 0; move32();
          i_max = 0; move16();
          i_min = shl(10, *scoef_SWBQ); move16();
          FOR(i=0; i<SWB_NORMAL_FENV; i++)
          {
          ...
          IF(sub(shl(sub(i_max, i_min), 1), shl(5, *scoef_SWBQ)) > 0 && sub(i_min, shl(12, *scoef_SWBQ)))
          {
            ...
            FOR(i=0; i<SWB_NORMAL_FENV; i++)
            {
              IF(L_sub(L_mult0(5, sfEnv[i]), L_temp) < 0)
              {
    ...
    ELSE              
    {
      ...
      IF((coef_SWBQ1 > 0) || (coef_SWBQ1 == 0 && senerWB == 0))
      {
        IF(norm_MDCT_fix < 0)
        {
          array_oper(8, norm_MDCT_fix, sfEnv, sfEnv, &shr);
    ...
    IF (sub(mode, TRANSIENT) == 0)
    {
      ...
      T_modify_flag = GetBit(pBit, 1);
    }

    /* copy the BWE parameters and decoded coefficients */
    *sig_Mode = mode;  move16();
    mov16(SWB_NORMAL_FENV, sfEnv, dec_st->pre_fEnv);
    ...
    *index_g = index_fGain;			move16();
    ...
    array_oper(SWB_NORMAL_FENV, temp, sFenv_SVQ, sFenv_SVQ, &shr_r);
...
void T_Env_Postprocess(Word16 *sOut, Word16 *tPre)
{
  ...
  L_temp = L_mult0(sOut[0], 27852);
  temp = HALF_SUB_SWB_T_WIDTH_1;
  L_temp = L_mac0(L_temp, tPre[temp], 2621);
  temp = HALF_SUB_SWB_T_WIDTH_2;
  L_temp = L_mac0(L_temp, tPre[temp], 1638);
  temp = HALF_SUB_SWB_T_WIDTH_3;
  L_temp = L_mac0(L_temp, tPre[temp], 655);  /* Q(15) */
  sOut[0] = round_fx_L_shl(L_temp, 1);
  move16();

  L_temp = L_mult0(sOut[1], 27852);
  L_temp = L_mac0(L_temp, sOut[0], 2621);
  temp = HALF_SUB_SWB_T_WIDTH_1;
  L_temp = L_mac0(L_temp, tPre[temp], 1638);
  temp = HALF_SUB_SWB_T_WIDTH_2;

  L_temp = L_mac0(L_temp, tPre[temp], 655);  /* Q(15) */
  sOut[1] = round_fx_L_shl(L_temp, 1);
  move16();

  L_temp = L_mult0(sOut[2], 27852);
  L_temp = L_mac0(L_temp, sOut[1], 2621);
  L_temp = L_mac0(L_temp, sOut[0], 1638);
  temp = HALF_SUB_SWB_T_WIDTH_1;

  L_temp = L_mac0(L_temp, tPre[temp], 655);  /* Q(15) */
  sOut[2] = round_fx_L_shl(L_temp, 1);
  move16();

  FOR(i = 3; i < SWB_T_WIDTH; i++)
  {
    L_temp = L_mult0(sOut[i], 27852);
    L_temp = L_mac0(L_temp, sOut[i-1], 2621);
    L_temp = L_mac0(L_temp, sOut[i-2], 1638);
    L_temp = L_mac0(L_temp, sOut[i-3], 655);  /* Q(15) */
    sOut[i] = round_fx_L_shl(L_temp, 1);
    move16();
  }
                       ...
                       )
{
  ...
  IF ((temp == 0) || (sub(dec_st->pre_mode, TRANSIENT) == 0))
  {
    ...
    FOR (i = 0; i < SWB_TENV; i++)
    {
      ...
      L_Extract( enn_fix, &enn_fix_hi, &enn_fix_lo );
  ...
  IF (temp == 0 && sub(T_modify_flag, 1) == 0)
  {
    ...
    IF (pos != 0)
    {
      pit_fix += i_mult(SUB_SWB_T_WIDTH, pos);
