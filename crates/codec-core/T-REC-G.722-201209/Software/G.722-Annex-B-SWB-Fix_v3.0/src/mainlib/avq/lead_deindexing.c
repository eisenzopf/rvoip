                           ...
                           )
{
  ...
  /*****************************/
#ifdef DYN_RAM_CNT
  DYN_RAM_PUSH((UWord32) ((30 * SIZE_Word16) + (3 * SIZE_Ptr)), "dummy");
#endif
  /*****************************/
    ...
    *-------------------------------------------------------*/
    ...
    IF(sub(code_level, 1) > 0)
    {
      ...
      SWITCH (code_level)
      {
        ...
        case 3:
          l = Select_table22[m1][m];  move16();
          j = extract_l_L_shr(L_mult0(code_index, DIV_mult[l]), DIV_shift[l]);
      ...
      IF(sub(code_level, 2) > 0)
      {
        FOR (i=0;i<m1;i++)
        {
          ii = setor_8p_temp_1[i]; move16();
          iii = setor_8p_temp[ii]; move16();
          x[iii] = a1[2];     move16(); 
        }
        IF(sub(code_level, 3) > 0)
        {
          ii = setor_8p_temp_1[setor_8p_temp_2]; move16();
          iii = setor_8p_temp[ii]; move16();
          x[iii] = 6; move16();
        }
      }
