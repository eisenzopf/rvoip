...
/*****************************/
  ...
  lAcc = L_mac( lAcc, index, -32768 );

  sLow = extract_h( L_shl( lAcc, 15 ) );

  lAcc = L_deposit_h_table_sqrt_w[index-16];
  sDiff = sub_table_sqrt_w_i_i1[index-16];
...
/*****************************/
  ...
  *exp = mac_r(32768, *exp, -16384);     move16();
  ...
  L_tmp = L_mult_table_isqrt_m32768[i];   /* table[i] << 16         */
  tmp = table_isqrt_i_i1[i];      /* table[i] - table[i+1]) */
