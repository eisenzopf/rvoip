...
{   
  ...
  zero16_8(dec_st->prev_zero_vector);
                         ...
                         )
{
  ...
  /*****************************/
#ifdef DYN_RAM_CNT
  DYN_RAM_PUSH((UWord32) (2 * SIZE_Ptr + ( 12 + N_SV + WIDTH_BAND*(N_SV+N_SV_L1+N_SV_L2) + SWB_F_WIDTH ) * SIZE_Word16 + 2 * SIZE_Word32), "dummy");
#endif

  /*****************************/
  zero16_8(zero_vector_tmp );
  ...
  *unbits_L1 = AVQ_Demuxdec_Bstr( pBst_L1, smdct_coef_L1, N_BITS_AVQ_L1, N_SV_L1 ); move16();
  ...
  FOR( i = 0; i < N_SV_L1; i++ )
  {
    L_en = Sum_vect_E8( smdct_coef_L1 + shl(i,3));
  ...
  FOR( i = 0; i<N_SV; i++ )
  {
    ...
    IF( (zero_vector_tmp[i] == 0) && tmp16 == 0 )
    {
      ...
      {
        L_en = Sum_vect_E8( smdct_coef_L2 + k8);

        IF (L_en > 0)
        {
          zero_vector_tmp[i] = 2; move16();

          array_oper8(QCOEF, &smdct_coef_L2[k8], &scoef_dec[shl(i,3)], &shl);
        }
      }
      k8 = add(k8, 8);
    }
    ...
    {
      array_oper8(QCOEF, &smdct_coef_L1[shl(i,3)], &scoef_dec[shl(i,3)], &shl);
    }
  }
  i8 = sub(flg_bit, 1);

  flg_bit_2 = sub(flg_bit, 2);
  ...
  IF( tmp16 == 0 )
  {
    ...
    IF( i8 != 0 && (*unbits_L1 > 0) )
    {
      ...
      w_AVQ_state_dec->detzer_flg = GetBit1( &bptpt);
  ...
  IF( i8 != 0 && tmp16 == 0 && (w_AVQ_state_dec->detzer_flg==0) )
  {
    ...
    FOR( i=0; i<N_SV; i++ )
    {
      ...
      if( zero_vector_tmp[i] != 0 )
      {
        mov16_8(&scoef_dec[shl(i,3)],&svec_base[shl(cnt_used,3)]);
      ...
      *unbits_L1 = sub(*unbits_L1, N_BITS_FILL_L1); move16();
      ...
      IF( sub(ind_corr_max, CORR_RANGE_L1) < 0 )
      {
        FOR( i=0; i<N_SV; i++ )
        {
          IF( zero_vector_tmp[i] == 0 )
          {
            L_en = Sum_vect_E8( &svec_base[ind_corr_max]);
            ...
            {
              array_oper8(Gain16, &svec_base[ind_corr_max], &scoef_dec[shl(i,3)], &mult_r);
            }
            ELSE
            {
              mov16_8(&svec_base[ind_corr_max], &scoef_dec[shl(i,3)]);
            }
  ...
  FOR( i=0; i<N_SV; i++ )
  {
    mov16_8(&scoef_dec[shl(i,3)], &scoef_SWB_AVQ[shl(ord_bands[i],3)] );
  ...
  FOR( i=0; i<N_SV; i++ )
  {
    FOR( j=0; j<WIDTH_BAND; j++ )
    {
      ...
      L_tmp = L_mult(scoef_SWB_AVQ[shl(i,3)+j], senv_BWE[i]);
      scoef_SWB_AVQ[shl(i,3)+j]= round_fx_L_shl(L_tmp, 15-QCOEF);
      move16();
    }
  }
  /*****************************/
                       ...
                       ) 
{
  ...
  Word16 flg_bit_1;
  Word16 scoef_SWBQ1, scoef_SWBQ2, temp;

  AVQ_state_dec *w_AVQ_state_dec = (AVQ_state_dec *)p_AVQ_state_dec;

  zero16_8(zero_vector);
  zero16_8(ip);
  zero16_8(senv_BWE);
  ...
  {   
    ...
    *---------------------------------------------------------------------*/
    ...
    ELSE
    {
      ...
      /* ---------------------- */
      array_oper8(19661, senv_BWE, senv_BWE_mod, &mult);/* Q(12): Q(15) + Q(12) + 1 - 16 */
        ...
        IF (zero_vector[i] != 0) 
        {
          ...
          IF (ltmp > 0)
          {
            ...
            IF (sub(zero_vector[i],1) == 0)
            {
              ...
              {
                lbuff_Fenv_BWE = L_add(0, ltmp); /* Q25 */
              }
      ...
      IF( sub(layers,2) == 0)
      {
        ...
        FOR( i=0; i<N_SV; i++ )
        {
          IF( zero_vector[i] == 0 )
          {
            ...
            tmp16 = senv_BWE[i]; move16();
            if(detprob_flg > 0)
            {
                tmp16 = mult_r(tmp16, senv_BWE_table[detprob_flg]);
            }
            senv_BWE[i] = tmp16; move16();
          }
          ELSE
          {
          ...
          IF( sub(ind_corr_max,CORR_RANGE_L1) < 0)
          {
            FOR( i=0; i<N_SV; i++ )
            {
              IF( zero_vector[i] == 0 )
              {
                ...
                FOR( j=0; j<WIDTH_BAND; j++ )
                {
                  scoef_SWB_AVQ[shl(i,3)+j] = round_fx_L_shl_L_mult(svec_base[ind_corr_max+j],Gain16,3);  
                  move16();
                }
...
#endif
      L_tmp = L_mac_Array8(1, &scoef_SWB[i8], &scoef_SWB[i8]);
  ...
  FOR(i=0; i<N_SV; i++)
  {
    ...
    IF (j >= 0)
    {
      ...
      if (j > 0) /* sub(zero_vector[i], 1) > 0, so its value is 2 */
        k = sGainBWE; move16();
      FOR (j = shl(i,3); j < add(shl(i,3),8); j++)
      {
    ...
    { 
      ...
      IF ( w_AVQ_state_dec->prev_zero_vector[i] != 0 && zero_vector[i] == 0) 
      {
        ...
        { 
          ...
          lbuff_coef_pow = L_abs_L_deposit_l(scoef_SWB[k]);

          IF (scoef_SWB_AVQ[k] == 0)
          { 
            IF(L_sub(L_preAVQ0[k], L_add(L_preAVQ1[k], L_prefSp[k])) > 0)
            {
              L_temp = L_add(L_mls(L_shl(L_preAVQ1[k], temp2), 3), L_mls(L_shl(L_prefSp[k], temp2), 3));
      ...
      FOR (i = 0; i < N_SV; i++)
      {
        ...
        { 
          ...
          {  
            ...
            IF (k == 0)
            { 
              L_temp = L_mac0( L_mult0(sh_sprefSp[k], 22938), abs_scoef_SWB[k], 29491);
              L_temp = L_mac0(L_temp, sh_sprefSp[k+1], 6554);
              L_temp = L_mac0(L_temp, abs_scoef_SWB[k+1], 6554);
            }
            ELSE IF (sub(k, N_SV * WIDTH_BAND - 1) == 0)
            {
              L_temp = L_mac0( L_mult0(sh_sprefSp[k], 16384), abs_scoef_SWB[k], 22938);
              L_temp = L_mac0(L_temp, sh_sprefSp[k-1], 9830);
              L_temp = L_mac0(L_temp, shl(abs_s(scoef_SWB[k-1]), temp1), 16384);
            }
            ELSE
            { 
              L_temp = L_mac0( L_mult0(sh_sprefSp[k], 16384), abs_scoef_SWB[k], 22938);
              L_temp = L_mac0(L_temp, sh_sprefSp[k-1], 6554);
              L_temp = L_mac0(L_temp, shl(abs_s(scoef_SWB[k-1]), temp1), 6554);
              L_temp = L_mac0(L_temp, sh_sprefSp[k+1], 6554);
              L_temp = L_mac0(L_temp, abs_scoef_SWB[k+1], 6554);
            } 
            en = round_fx(L_temp); 
        ...
        ELSE
        {
          ...
          FOR (j = 0; j < WIDTH_BAND; j++)
          {
            ...
            IF (w_AVQ_state_dec->sbuffAVQ[k] == 0)
            {
              IF (k == 0)
              {
                L_temp = L_mac(L_mult(sh_sprefSp[k], 4915), abs_scoef_SWB[k], 21299);
                L_temp = L_mac(L_temp, sh_sprefSp[k+1], 3277);
                en = mac_r(L_temp, abs_scoef_SWB[k+1], 3277);
              }
              ELSE IF (sub(k, N_SV * WIDTH_BAND - 1) == 0)
              {
                L_temp = L_mac(L_mult(sh_sprefSp[k], 4915), abs_scoef_SWB[k], 21299);
                L_temp = L_mac(L_temp, sh_sprefSp[k-1], 1638);
                en = mac_r(L_temp, shl(abs_s(scoef_SWB[k-1]), temp1), 4915);
              }
              ELSE
              {
                L_temp = L_mac(L_mult(sh_sprefSp[k], 4915), abs_scoef_SWB[k], 21299);
                L_temp = L_mac(L_temp, sh_sprefSp[k-1], 1638);
                L_temp = L_mac(L_temp, shl(abs_s(scoef_SWB[k-1]), temp1), 1638);
                L_temp = L_mac(L_temp, sh_sprefSp[k+1], 1638);
                en = mac_r(L_temp, abs_scoef_SWB[k+1], 1638);
              }
  ...
  mov16_8(zero_vector, w_AVQ_state_dec->prev_zero_vector);
  ...
  IF (flg_bit_1 == 0)
  {
    ...
    FOR ( i = 0; i<N_SV; i++ )
    {
      ...
      FOR ( j = 0; j<WIDTH_BAND; j++ )
      {
        ...
        scoef_tmp = extract_h( L_mac( L_mult(27853, sbuff_abs), 4915, w_AVQ_state_dec->scoef_SWB_abs_old[i8+j] ) );
