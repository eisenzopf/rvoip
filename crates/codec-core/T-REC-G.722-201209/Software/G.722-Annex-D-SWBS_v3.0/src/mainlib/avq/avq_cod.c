                       ...
                       )
{
  ...
  FOR( i=0; i<Nsv; i++ )
  {
    ...
    RE8_Cod( ptr, &nq[pos], &I[pos], &kv[shl(pos,3)] );
    ...
    IF (nq[pos] > 0)
    {
      ...
      IF (sub(add(add(bits, Calc_bits[nq[pos]]), j), nb_bits) <= 0)
      {
        bits = add(Calc_bits[nq[pos]], bits);
