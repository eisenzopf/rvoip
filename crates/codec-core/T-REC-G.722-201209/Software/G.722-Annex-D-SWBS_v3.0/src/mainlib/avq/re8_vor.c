             ...
             )
{
  ...
  *---------------------------------------------------------------*/
  ...
  FOR (i=6; i>=1; i--)
  {
    Lsum = L_mac (Lsum, k[i], 1);
    ytp[i] = L_mac(ytp[7], k[i], 1);       move32();

    z[i] = L_shl(ytp[i], m_tmp);        move32();   /* (int)(floor(y[7]*QR+0.5))>>m */
  }

  Lsum = L_mac0( Lsum, k[0], 4);
...
{
  Word16 i,s,id,pos,ka, tmp16;
  Word32 Ltmp, Ls;
  Word32 C;
  Word16 *ptr;
  /*****************************/
#ifdef DYN_RAM_CNT
  DYN_RAM_PUSH((UWord32) ((6 * SIZE_Word16) + (3 * SIZE_Word32) + SIZE_Ptr), "dummy");
#endif
    ...
    *-------------------------------------------------------------------*/
    IF (sub(s, NB_SPHERE) <= 0)
    {
      ...
      *---------------------------------------------------------------*/
      ...
      move16();
      ptr = &Da_id_[pos];
      FOR (i=0; i<Da_nb_[s - 1]; i++)/* get the number of absolute leaders used on the shell of number s */
      {
        IF (sub(id, *ptr) == 0)
        {
