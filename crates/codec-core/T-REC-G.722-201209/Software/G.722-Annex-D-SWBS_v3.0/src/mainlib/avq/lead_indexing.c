                            ...
                            )
{
  ...
  /*****************************/
#ifdef DYN_RAM_CNT
  DYN_RAM_PUSH((UWord32) ((24 * SIZE_Word16) + (2 * SIZE_Ptr)), "dummy");
#endif
  ...
  IF (sub(a2[1], 2) == 0 && s_xor(a1[0], 1) && sub(ka, 5))
  {
    ...
    *I = extract_l(L_add(IS_new[ka], code_index));
    move16();
  }
  ELSE
  {
                             ...
                             )
{
  ...
  Word16 temp, temp1;
  Word16 Iters;
  const Word16 *ptr_pos0, *ptr_pos1; 
  ...
  ptr_pos0 =  pos_vector;
  ptr_pos1 =  pos_vector + 1;
  temp1 = sub(pos_num, 1);
  FOR (i = 0; i < Iters; i++)
  {
    select_table23 = Select_table22[temp1];  move16();
    code_index = add(code_index,sub( select_table23[sub(temp, *ptr_pos0++)],select_table23[sub(pulse_num,*ptr_pos1++)]) );
    temp1 = sub(temp1, 1);
  }
