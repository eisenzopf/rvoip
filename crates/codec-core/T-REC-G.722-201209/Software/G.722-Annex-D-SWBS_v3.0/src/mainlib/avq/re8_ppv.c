...
)
{
  ...
  *--------------------------------------------------------------*/
  tmp16 = mac_r(x[0], extract_h(x[0]), 1);

  y[0] = shl(tmp16, 1);                      move16();
  /* sum += y[i] */
  tmp16 = mac_r(x[1], extract_h(x[1]), 1);
  y[1] = shl(tmp16, 1);                      move16();
  sum = add(y[0], y[1]);
  FOR (i=2; i<8; i++)
  {
    /* round to ..., -2, 0, 2, ... ([-1..1[ --> 0) */
    tmp16 = mac_r(x[i], extract_h(x[i]), 1);
