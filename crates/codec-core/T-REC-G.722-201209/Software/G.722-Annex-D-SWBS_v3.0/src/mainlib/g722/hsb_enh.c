                        ...
                        )
{
  ...
  *code0 = add(*code0, shl(ih,6)); move16();
  ...
  IF(wbenh_flag > 0)
  {
    ...
    test();
    IF ((L_msu0(i_sum, i_abs, i) < 0) && (*enh_no > 0))
    {
      ...
      PushBit1( (UWord16) w16tmp, pBit_wbenh);
                  ...
                  Word16 i, UWord16 **pBit_wbenh, Word16 wbenh_flag, Word16 *enh_no, Word32 *i_sum)
{
  ...
  IF (wbenh_flag > 0)
  {
    ...
    IF ((i == 0) || ((L_msu0(*i_sum, i_abs, i) < 0) && (*enh_no > 0)) || (sub(add(i, *enh_no), L_FRAME_NB) == 0))
    {
      ih23 = shl(ih,1);

      if (GetBit1( pBit_wbenh)>0)
      {
        ih23 = s_or(ih23, 1);
      }
              ...
              )
{
  ...
  ELSE {
    /* rc[0] == -r[1]/r[0]   */
    IF (sub (32256, rc[0]) < 0)   /* r[1]/r[0] < -0.984375 */
    {
      sAlpha = sub (32767, rc[0]);
