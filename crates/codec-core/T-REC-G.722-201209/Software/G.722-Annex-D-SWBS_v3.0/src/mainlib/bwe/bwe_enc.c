               ...
               )
{
...
#ifdef LAYER_STEREO
    Word16 g_s[4] = {16384,16384,16384,16384};//Q14
    Word16 j;
    Word16 deltaGain, stepGain, gainCorrection;
...
#ifdef DYN_RAM_CNT
    {
        ...
        ssize =  (UWord32) (1*SIZE_Ptr);
#ifdef LAYER_STEREO
        ssize += (UWord32) ((3+SWB_TRANSI_FENV+NUM_FENV_CODEBOOK+NUM_FENV_VECT+2+L_FRAME_WB+1+4+4+4)*SIZE_Word16);
#else
    ...
    IF(channel == 2) /* stereo */
    {
        /*gain correction for SWB mono*/
        FOR(j = 0; j < SWB_BN; j++)
        {
            g_s[j+1] = gain[j]; move16();//Q14
        }

        FOR(j = 0; j < 3; j++)
        {
            deltaGain = sub(g_s[j+1],g_s[j]);
            stepGain = mult(deltaGain ,inv_swb_int_bands[j]);//q14
            gainCorrection = g_s[j]; move16();
            FOR(i=swb_int_bands[j]; i<swb_int_bands[j+1]; i++)
            {
                sY[i] = extract_h(L_mult(sY[i],gainCorrection )), move16();; 
                gainCorrection = add(gainCorrection ,stepGain);
            }
        }
        norm = sub(norm,1);
    }/* stereo */
