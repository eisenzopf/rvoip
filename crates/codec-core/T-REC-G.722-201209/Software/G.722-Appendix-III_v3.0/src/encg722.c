...
char *argv[];
{
  ...
  short           mask=(short)0xFFFF; /* mode 2, mode 3 bit-mask */
	...
	while ((fread ((char *) incode, sizeof (short), N, F_inp)) == N) {
#ifdef WMOPS
                setCounter(spe1Id);
                fwc();
                Reset_WMOPS_counter();
		...
		if(!g192_outp){
         ...
         if (fwrite ((char *) code, sizeof (short), N/2, F_cod) != N/2){
					KILL(FileOut,-4);
			}
			if (debug) {
			...
			if (fwrite ((char *) outp_frame, sizeof (short), (2+outp_frame[1]), F_cod) != (2+outp_frame[1])){
				KILL(FileOut,-4);
			}
			if (debug) {
