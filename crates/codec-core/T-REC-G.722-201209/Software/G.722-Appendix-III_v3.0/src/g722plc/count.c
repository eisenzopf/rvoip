...
/* $Id: count.c 1.1 2002/09/10 21:16:24 jthyssen Exp $ */
/*
 ***************************************************************************
 *
...
void DMEM_output(){
  ...
  fprintf (stdout, "   Worst Case DMEM Usage: %d bytes\n",dmemCounter.WorstCase);
...
const BASIC_OP op_weight =
{
  ...
  3, 3, 3, 4, 3, 18, 3, 1, 2, 1, 2, 2, 
  /* G.723.1 exclusives */ 
  6, 1, 1, 
  /* shiftless 32-bit operators */ 
  1, 1, 1,
  /* Broadcom higher level operators */ 
  3, 3, 6, 4, 44 /* TBD */
};
...
void Reset_WMOPS_counter (void)
{
    ...
    total_wmops[currCounter] += ((float) tot) * (float)0.0002;
...
void WMOPS_output (Word16 dtx_mode)
{
     ...
     */
    tot = WMOPS_frameStat();
    tot_wm = total_wmops[currCounter] + ((float) tot) * (float)0.0002;
    ...
    fprintf (stdout, "  WorstCase=%.3f",
             ((float) glob_wc[currCounter]) * 0.0002);
    ...
    if (dtx_mode == 0)
    {
        ...
        fprintf (stdout, "  WorstWC=%.3f", ((float) tot_wc) * 0.0002);
    }
