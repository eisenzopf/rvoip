...
use crate::codecs::g729a_new::common::basic_operators::*;

/// Gamma1 coefficient for perceptual weighting (0.94 in Q15)
pub const GAMMA1: Word16 = 30802;

/// Gamma2 coefficient for perceptual weighting (0.6 in Q15)
pub const GAMMA2: Word16 = 19661;
...
mod tests {
    ...
    fn test_perceptual_weighting_from_csv() {
        ...
        let input_path = test_dir.join("test_inputs.csv");
        let output_path = test_dir.join("rust_output.csv");
        
        let file = File::open(&input_path).expect("Failed to open test_inputs.csv");
        let mut output = File::create(&output_path).expect("Failed to create rust_output.csv");
        
        writeln!(output, "\nrunning 1 test").unwrap();
        writeln!(output, "test_id,p0,p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,f0,f1,f2,f3,f4,f5,f6,f7,f8,f9,f10").unwrap();
        ...
        for (index, line) in reader.lines().enumerate() {
            ...
            write!(output, "{}", test_id).unwrap();
            for val in p.iter() { write!(output, ",{}", val).unwrap(); }
            for val in f.iter() { write!(output, ",{}", val).unwrap(); }
