...
fn test_alaw_encoding_compliance() {
    ...
    println!(
        "  Accuracy vs G.191 format: {:.2}% ({}/{} samples)",
...
fn test_mulaw_encoding_compliance() {
    ...
    for i in 0..total_tested {
        ...
        if our == reference {
            ...
            if differences <= 10 {
                println!(
                    "  Format diff #{}: input={}, our=0x{:02x}, G.191=0x{:02x}",
...
fn test_alaw_decoding_compliance() {
    ...
    let our_decoded: Vec<i16> = reference_encoded
        .iter()
...
fn test_mulaw_decoding_compliance() {
    ...
    for i in 0..total_tested {
        if our_decoded[i] == reference_decoded[i] {
            ...
            if differences <= 10 {
                println!(
                    "  Format diff #{}: encoded=0x{:02x}, our={}, G.191={}",
...
fn test_g711_self_consistency() {
    ...
    let test_values = vec![
        -32768i16, -16384, -8192, -4096, -2048, -1024, -512, -256, -128, -64, -32, -16, -8, -4, -2,
        -1, 0, 1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32767,
    ];

    println!(
        "📊 Testing {} values for self-consistency",
        test_values.len()
    );
    ...
    for &sample in &test_values {
        ...
        if alaw_error > 2000 {
            ...
            if alaw_errors <= 5 {
                println!(
                    "  A-law large error: {} -> 0x{:02x} -> {} (error: {})",
        ...
        if mulaw_error > 2000 {
            ...
            if mulaw_errors <= 5 {
                println!(
                    "  μ-law large error: {} -> 0x{:02x} -> {} (error: {})",
    ...
    println!(
        "  A-law large errors: {}/{} values",
        alaw_errors,
        test_values.len()
    );
    println!(
        "  μ-law large errors: {}/{} values",
        mulaw_errors,
        test_values.len()
    );
    ...
    let alaw_success_rate =
        (test_values.len() - alaw_errors) as f64 / test_values.len() as f64 * 100.0;
    let mulaw_success_rate =
        (test_values.len() - mulaw_errors) as f64 / test_values.len() as f64 * 100.0;

    assert!(
        alaw_success_rate >= 90.0,
        "A-law self-consistency should be >= 90%"
    );
    assert!(
        mulaw_success_rate >= 90.0,
        "μ-law self-consistency should be >= 90%"
    );
}
...
fn test_algorithm_correctness() {
    ...
    for (input, expected_alaw, expected_mulaw) in test_cases {
        ...
        assert_eq!(
            our_alaw, expected_alaw,
            "A-law mismatch for input {}",
            input
        );
        assert_eq!(
            our_mulaw, expected_mulaw,
            "μ-law mismatch for input {}",
            input
        );
        ...
        println!(
            "  Input {}: A-law=0x{:02x}→{} (err={}), μ-law=0x{:02x}→{} (err={})",
        ...
        assert!(
            alaw_error <= 2000,
            "A-law quantization error too large for input {}",
            input
        );
        assert!(
            mulaw_error <= 2000,
            "μ-law quantization error too large for input {}",
            input
        );
    }
...
fn test_itu_compliance_summary() {
    ...
    println!(
        "  A-law test data: {} samples (G.191 format)",
        alaw_encoded.len()
    );
    println!(
        "  μ-law test data: {} samples (G.191 format)",
        mulaw_encoded.len()
    );
    println!(
        "  Sample range: {} to {}",
    ...
    println!(
        "  📝 Our implementation targets standard ITU-T G.711, not G.191 codec testing format"
    );
