...
impl ChannelMixer {
    ...
    pub fn with_settings(mono_to_stereo_gain: f32, stereo_to_mono_coeffs: (f32, f32)) -> Self {
        Self {
            mono_to_stereo_gain,
    ...
    pub fn mix_channels(
        ...
        match (source_layout, target_layout) {
            (ChannelLayout::Mono, ChannelLayout::Stereo) => self.mono_to_stereo(input_samples),
            (ChannelLayout::Stereo, ChannelLayout::Mono) => self.stereo_to_mono(input_samples),
            _ => {
...
mod tests {
    ...
    fn test_mono_to_stereo() {
        ...
        let output = mixer
            .mix_channels(&input, ChannelLayout::Mono, ChannelLayout::Stereo)
            .unwrap();
    ...
    fn test_stereo_to_mono() {
        ...
        let output = mixer
            .mix_channels(&input, ChannelLayout::Stereo, ChannelLayout::Mono)
            .unwrap();
    ...
    fn test_no_conversion_needed() {
        ...
        let input = vec![100, 200, 300];
        let output = mixer
            .mix_channels(&input, ChannelLayout::Mono, ChannelLayout::Mono)
            .unwrap();
