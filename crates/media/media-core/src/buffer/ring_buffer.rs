    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            RingBufferError::InvalidCapacity => {
                write!(f, "Ring buffer capacity must be a power of 2")
            }
...
impl<T> RingBuffer<T> {
        ...
        T: Clone,
    {
        ...
        let item = buffer[read_pos]
            .as_ref()
            .ok_or(RingBufferError::BufferEmpty)?;
    ...
    pub async fn force_push(&self, item: T) -> Option<T> {
        ...
        trace!(
            "Ring buffer force_push: write_pos={}, overwritten={}",
            next_write_pos,
            overwritten_item.is_some()
        );
