...
//!
//! This module implements the pure encode/decode, independent of
//! `RtpSession`, sockets, or the async runtime. `rvoip-rtp-core`'s own UDP
//! transport (`transport::udp`) uses [`TelephoneEvent::decode`] to build the
//! `RtpEvent::DtmfEvent` it emits for inbound PT 101 packets. This type used
//! to live in `rvoip-media-core`; it moved down here so a caller that only
//! wants the codec, without the rest of the media pipeline, can depend on
//! `rvoip-rtp-core` alone. `rvoip-media-core` re-exports both types from
//! their original path for existing consumers.
