...
use rvoip_rtp_core::{
    ...
    buffer::{BufferLimits, PacketPriority, TransmitBufferConfig},
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    std::thread::spawn(|| {
        ...
        eprintln!(
            "Emergency timeout triggered after {} seconds - terminating process",
    ...
    client
        .set_priority_threshold(0.8, PacketPriority::High)
        .await?;

    // Launch the timer task that will stop the example after MAX_RUNTIME_SECONDS
    let timeout_task = tokio::spawn(async move {
        info!(
            "Timer started: example will run for max {} seconds",
            MAX_RUNTIME_SECONDS
        );
        ...
        info!(
            "Timer expired after {} seconds, signaling graceful shutdown",
            MAX_RUNTIME_SECONDS
        );
    ...
    while i < PACKETS_TO_SEND && running.load(Ordering::SeqCst) {
        ...
        let frame = MediaFrame {
            ...
            data: format!("Frame {}", i).into_bytes().into(),
