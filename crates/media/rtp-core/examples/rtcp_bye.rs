...
use bytes::Bytes;
use rvoip_rtp_core::{
    packet::rtcp::RtcpPacket, RtpEvent, RtpSession, RtpSessionConfig, RtpSessionEvent,
};
        ...
        while let Ok(event) = receiver_events.recv().await {
            match event {
                RtpSessionEvent::PacketReceived(packet) => {
                    info!(
                        "Receiver got packet: PT={}, SEQ={}, SSRC={:08x}",
                ...
                RtpSessionEvent::Bye { ssrc, reason } => {
                    info!(
                        "Receiver got BYE event from SSRC={:08x}, reason: {:?}",
                        ssrc, reason
                    );
    ...
    let _sender_task = tokio::spawn(async move {
        while let Ok(event) = sender_events.recv().await {
            match event {
                RtpSessionEvent::PacketReceived(packet) => {
                    info!(
                        "Sender got packet: PT={}, SEQ={}, SSRC={:08x}",
                        packet.header.payload_type,
                        packet.header.sequence_number,
                        packet.header.ssrc
                    );
                ...
                RtpSessionEvent::Bye { ssrc, reason } => {
                    info!(
                        "Sender got BYE from SSRC={:08x}, reason: {:?}",
                        ssrc, reason
                    );
                }
                RtpSessionEvent::NewStreamDetected { ssrc } => {
