...
impl fmt::Display for SubscriptionState {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            SubscriptionState::Terminated { reason } => match reason {
...
impl fmt::Display for SubscriptionTerminationReason {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            SubscriptionTerminationReason::ServerTerminated(msg) => {
                write!(f, "server terminated: {}", msg)
            }
    ...
    pub fn from_header_value(value: &str) -> Self {
        ...
        match state.as_str() {
            ...
            "terminated" => {
                ...
                let reason = parts.iter().find_map(|part| {
                    let kv: Vec<&str> = part.split('=').collect();
    ...
    pub fn to_header_value(&self) -> String {
        match self {
            ...
            SubscriptionState::Terminated { reason } => match reason {
                ...
                Some(SubscriptionTerminationReason::ServerTerminated(msg)) => {
                    format!("terminated;reason={}", msg)
                }
