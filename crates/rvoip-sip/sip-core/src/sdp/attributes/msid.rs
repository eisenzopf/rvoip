...
pub fn parse_msid(value: &str) -> Result<(String, Option<String>)> {
    ...
    if trimmed.chars().any(|c| {
        ...
        return Err(Error::SdpParsingError(format!(
            "MSID contains invalid characters: {}",
            value
        )));
    }
    ...
    if tokens.len() > 2 {
        return Err(Error::SdpParsingError(format!(
            "MSID contains invalid spaces within identifiers: {}",
            value
        )));
    }
        ...
        Ok((_, (stream_id, track_id))) => {
            ...
            if stream_id.is_empty() {
                return Err(Error::SdpParsingError(
                    "Empty stream identifier in msid".to_string(),
                ));
            }
            ...
            if let Some(ref track) = track_id {
                if track.is_empty() {
                    return Err(Error::SdpParsingError(
                        "Empty track identifier in msid".to_string(),
                    ));
                }
            }
...
mod tests {
    ...
    fn test_real_world_examples() {
        ...
        assert_eq!(
            result.1,
            Some("RTCvZxSnCiUxIJQgeMqiYOOQBgIVKOxQyMnG".to_string())
        );
        ...
        assert_eq!(
            result.1,
            Some("{97f7183c-0398-504a-g95e-4fbe40e2b28a}".to_string())
        );
    }
