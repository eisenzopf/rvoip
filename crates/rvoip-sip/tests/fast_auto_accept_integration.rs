...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn fast_auto_accept_direct_resolution_methods_are_observational_noops() {
    let cases = [
        DirectResolutionMode::Accept,
        DirectResolutionMode::Reject,
        DirectResolutionMode::Redirect,
    ...
    for (idx, mode) in cases.into_iter().enumerate() {
        ...
        assert!(
            wait_for_answer_without_failure(&mut alice_events, &call_id, Duration::from_secs(8))
                .await,
