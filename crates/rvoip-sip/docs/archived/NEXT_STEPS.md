...
- `crates/rvoip-sip/docs/BENCHMARKING.md` (new sub-section with
  measured numbers)
...
| 3 | UDP transport single-task recv loop | `crates/rvoip-sip-transport/src/transport/udp/mod.rs:38` | One spawned task parses + dispatches every inbound packet. Has worked fine until now because the downstream layers were the bottleneck. |
...
- `crates/rvoip-sip/src/api/callback_peer.rs` (if suspect #1)
- `crates/rvoip-sip/src/state_machine/executor.rs` +
  `crates/rvoip-sip/src/adapters/media_adapter.rs` (if suspect #2)
- `crates/rvoip-sip-transport/src/transport/udp/mod.rs` (if suspect #3)
- `crates/rvoip-sip/docs/PROFILING.md` (new Scenario 9)
...
| --- | --- | --- | --- |
| 1 | **Port-zero rejection** (RFC 3264 §6) — declined m-lines must answer `m=audio 0 …` | `crates/rvoip-sip/src/adapters/media_adapter.rs:646-652` (acknowledged TODO) | Strict middleboxes reject the answer; will hit on interop |
