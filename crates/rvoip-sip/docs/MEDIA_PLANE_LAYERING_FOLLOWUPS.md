...
Command:
```bash
RUST_LOG=debug cargo test -p rvoip-sip --lib 2>&1 \
...
```bash
cargo build -p rvoip-sip
cargo test -p rvoip-sip --lib
cargo test -p rvoip-sip --test '*'
...
cargo test -p rvoip-sip --features dev-insecure-tls --test tls_call_integration
...
cargo test -p rvoip-sip --lib
cargo test -p rvoip-sip --test '*' --features dev-insecure-tls
./crates/session-core/examples/run_all.sh
```
...
```bash
cargo test -p rvoip-sip --lib sdp_offer_matches_legacy_format
```
...
cargo build -p rvoip-sip
cargo test -p rvoip-sip --lib
cargo test -p rvoip-sip --test srtp_call_integration --features dev-insecure-tls
cargo test -p rvoip-sip --test audio_roundtrip_integration
...
cargo clean -p rvoip-sip -p rvoip-media-core -p rvoip-rtp-core -p rvoip-dialog-core
