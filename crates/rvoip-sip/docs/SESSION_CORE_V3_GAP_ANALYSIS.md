...
This document analyzes **only session-core** to identify gaps for its intended purpose: **single session management** with support for:
...
**Scope:** session-core is for **single session use cases** (softphones, IVR, simple agents).
...
Per the **Layered Architecture Proposal**, session-core is ONE layer:
...
rvoip-b2bua              ← Two-leg bridging (separate crate)
    ↓
session-core          ← Single session management (THIS CRATE)
    ↓
dialog-core + media-core
```

**session-core responsibility:** Manage ONE session at a time.
...
## Critical Gaps for session-core
...
**Needed (session-core provides primitive only):**
```rust
...
- ⚠️ session-core needs **SIP digest auth** (protocol-level)
- ✅ Implement SIP digest auth in session-core (or dialog-core)
...
- ✅ session-core needs to **connect TO registrar servers**
- ❌ No code reuse between them (different roles)

**Action:** Implement REGISTER client in dialog-core or session-core.
...
## What session-core SHOULD Have

Per the layered architecture, session-core should focus on **single session management**:
...
### ❌ NOT for session-core (Separate Crates):
...
## Minimal session-core for Production
...
**Deliverable:** Production-ready session-core for softphones and simple agents
...
**Deliverable:** session-core ready for b2bua crate to build on
...
**Not needed by session-core:**
- ❌ registrar-core is for building registrar SERVERS
- ❌ session-core needs registrar CLIENT
...
### session-core Current Status: 80% Ready
...
- ❌ No code sharing
- ✅ session-core connects TO registrars built with registrar-core

---

## Final Recommendation

### For session-core to be production-ready:
...
**The infrastructure is solid. session-core just needs registration, authentication, and the new APIs. Everything else belongs in separate crates (b2bua, call-center).**
