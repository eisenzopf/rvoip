#!/usr/bin/env bash
# Custom handler example: full CallHandler implementation.
set -euo pipefail
...
cargo run -p rvoip-sip --example callback_peer_trait_handler_server --quiet \
  2>&1 | sed "s/^/$(printf '\033[0;32m')[SERVER]$(printf '\033[0m') /" &
sleep 2

echo -e "${CYAN}[CLIENT]${NC} Starting"
cargo run -p rvoip-sip --example callback_peer_trait_handler_client --quiet \
