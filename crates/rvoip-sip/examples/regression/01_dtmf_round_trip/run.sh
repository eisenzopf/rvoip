...
cargo run -p rvoip-sip --example regression_dtmf_round_trip_server --quiet > "$SERVER_LOG" 2>&1 &
SERVER_PID=$!
sleep 2

echo -e "${CYAN}[CLIENT]${NC} Starting DTMF sender"
cargo run -p rvoip-sip --example regression_dtmf_round_trip_client --quiet \
