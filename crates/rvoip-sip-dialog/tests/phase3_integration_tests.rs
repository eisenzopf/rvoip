...
impl Phase3TestEnvironment {
    ...
    async fn new() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let (mut server_transport, server_transport_rx) =
            TransportManager::new(server_config).await?;
        server_transport.initialize().await?;
        let server_addr = server_transport
            .default_transport()
            .await
            .ok_or("No default transport")?
            .local_addr()?;

        let (server_transaction_manager, server_global_rx) =
            TransactionManager::with_transport_manager(
        ...
        let (mut client_transport, client_transport_rx) =
            TransportManager::new(client_config).await?;
        ...
        let (client_transaction_manager, client_global_rx) =
            TransactionManager::with_transport_manager(
        ...
        let server_config = rvoip_sip_dialog::api::config::ServerConfig::default();
        let client_config = rvoip_sip_dialog::api::config::ClientConfig::default();
    ...
    async fn create_established_dialog_client(
        ...
        let dialog = self
            .client
            .create_dialog(&local_uri.to_string(), &remote_uri.to_string())
            .await?;
        ...
        let dialog_manager = self.client.dialog_manager();
        {
            ...
            dialog.state = rvoip_sip_dialog::DialogState::Confirmed;
        }
...
async fn test_phase3_bye_integration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "✅ BYE sent using Phase 3 functions - transaction: {}",
        transaction_id
    );

    // Verify the BYE was properly constructed by checking transaction exists
    assert!(
        !transaction_id.to_string().is_empty(),
...
async fn test_phase3_refer_integration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let dialog_id = env.create_established_dialog_client().await?;
    println!(
        "✅ Created established dialog for REFER test: {}",
        dialog_id
    );
    ...
    println!(
        "✅ REFER sent using Phase 3 functions - transaction: {}",
        transaction_id
    );
...
async fn test_phase3_update_integration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let dialog_id = env.create_established_dialog_client().await?;
    println!(
        "✅ Created established dialog for UPDATE test: {}",
        dialog_id
    );
    ...
    let transaction_id = env
        .client
        .send_update(&dialog_id, Some(new_sdp.to_string()))
        .await?;
    println!(
        "✅ UPDATE sent using Phase 3 functions - transaction: {}",
        transaction_id
    );
...
async fn test_phase3_info_integration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let transaction_id = env
        .client
        .send_info(&dialog_id, info_content.to_string())
        .await?;
    println!(
        "✅ INFO sent using Phase 3 functions - transaction: {}",
        transaction_id
    );
...
async fn test_phase3_notify_integration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let dialog_id = env.create_established_dialog_client().await?;
    println!(
        "✅ Created established dialog for NOTIFY test: {}",
        dialog_id
    );
    ...
    let transaction_id = env
        ...
        .await?;
    println!(
        "✅ NOTIFY sent using Phase 3 functions - transaction: {}",
        transaction_id
    );
...
async fn test_phase3_response_building_integration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let call_handle = env
        .server
        .handle_invite(invite_request, env.client_addr)
        .await?;
...
async fn test_phase3_complete_dialog_workflow() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let info_tx = env
        .client
        .send_info(&dialog_id, "Session info".to_string())
        .await?;
    ...
    let update_tx = env
        .client
        .send_update(&dialog_id, Some(update_sdp.to_string()))
        .await?;
    println!(
        "✅ Step 3: Sent UPDATE using Phase 3 function - {}",
        update_tx
    );
    ...
    let notify_tx = env
        ...
        .send_notify(
            &dialog_id,
            "dialog".to_string(),
            Some("State change".to_string()),
        )
        .await?;
    println!(
        "✅ Step 4: Sent NOTIFY using Phase 3 function - {}",
        notify_tx
    );
    ...
    let refer_tx = env
        .client
        .send_refer(&dialog_id, refer_target, None)
        .await?;
    println!(
        "✅ Step 5: Sent REFER using Phase 3 function - {}",
        refer_tx
    );
    ...
    for tx in &all_transactions {
        assert!(
            !tx.to_string().is_empty(),
            "All transaction IDs should be valid"
        );
    }

    println!(
        "✅ Complete workflow: All {} SIP methods sent using Phase 3 dialog functions",
...
async fn test_phase3_error_handling_integration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let refer_result = env
        .client
        .send_refer(&invalid_dialog_id, "sip:test@example.com".to_string(), None)
        .await;
    assert!(
        refer_result.is_err(),
        "REFER with invalid dialog should fail"
    );
    println!("✅ REFER error handling works with Phase 3 integration");

    let update_result = env
        .client
        .send_update(&invalid_dialog_id, Some("test sdp".to_string()))
        .await;
    assert!(
        update_result.is_err(),
        "UPDATE with invalid dialog should fail"
    );
...
async fn test_phase3_server_side_integration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let info_tx = env
        .server
        .send_info(&dialog_id, "Server info".to_string())
        .await?;
    ...
    let notify_tx = env
        ...
        .await?;
    println!(
        "✅ Server sent NOTIFY using Phase 3 functions - {}",
        notify_tx
    );
