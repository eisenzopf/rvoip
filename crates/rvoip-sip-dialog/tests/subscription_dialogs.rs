...
async fn test_subscribe_creates_dialog() {
    ...
    tokio::time::timeout(Duration::from_millis(500), async {
        if let Some(event) = event_rx.recv().await {
            match event {
                DialogEvent::SubscriptionCreated {
                    event_package,
                    expires,
                    ..
                } => {
...
async fn test_subscribe_with_unsupported_event_package() {
    ...
    assert!(
        dialog_id.is_none(),
        "Unsupported event should not create dialog"
    );
}
...
async fn test_event_package_validation() {
    use rvoip_sip_dialog::subscription::EventPackage;
    ...
    impl EventPackage for TestPackage {
        ...
        fn accept_types(&self) -> Vec<rvoip_sip_core::types::content_type::ContentType> {
            vec![]
        }
