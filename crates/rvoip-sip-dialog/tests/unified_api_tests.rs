...
async fn test_configuration_backward_compatibility() {
    ...
    let legacy_client = rvoip_sip_dialog::api::config::ClientConfig::new(local_addr)
    ...
    let legacy_server = rvoip_sip_dialog::api::config::ServerConfig::new(local_addr)
...
async fn test_legacy_compatibility_maintained() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let legacy_client = rvoip_sip_dialog::api::config::ClientConfig::new(env.local_address);
    let unified_config: DialogManagerConfig = legacy_client.into();
