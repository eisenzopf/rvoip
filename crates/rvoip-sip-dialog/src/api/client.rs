...
//!
//! ```rust,no_run
//! use rvoip_sip_dialog::api::{DialogClient, DialogApi, ClientConfig};
...
//!
//! ```rust,no_run
//! use rvoip_sip_dialog::api::{DialogClient, ClientConfig};
//! use rvoip_sip_dialog::transaction::TransactionManager;
//! use std::sync::Arc;
//!
...
//!
//! ```rust,no_run
//! use rvoip_sip_dialog::api::{DialogClient, DialogApi};
...
//! # Ok(())
//! # }
//! # async fn setup_dependencies() -> Result<(std::sync::Arc<rvoip_sip_dialog::transaction::TransactionManager>, rvoip_sip_dialog::api::ClientConfig), Box<dyn std::error::Error>> { unimplemented!() }
//! ```
//!
//! ## Usage Patterns
//!
...
//!
//! ```rust,no_run
//! use rvoip_sip_dialog::api::DialogClient;
//!
...
//!
//! ```rust,no_run
//! use rvoip_sip_dialog::api::{DialogClient, ApiError};
//!
...
//!
//! ```rust
//! use rvoip_sip_dialog::api::ClientConfig;
...
/// ```rust,no_run
/// use rvoip_sip_dialog::api::{DialogClient, ClientConfig};
/// use rvoip_sip_dialog::transaction::{TransactionManager, TransactionEvent};
/// use tokio::sync::mpsc;
/// use std::sync::Arc;
///
...
impl DialogClient {
    ...
    ///
    /// ```rust,no_run
    /// use rvoip_sip_dialog::api::{DialogClient, ClientConfig, DialogApi};
    /// use rvoip_sip_dialog::transaction::{TransactionManager, TransactionEvent};
    /// use tokio::sync::mpsc;
    /// use std::sync::Arc;
    ///
    ...
    pub async fn make_call(
        ...
        let dialog_id = self
            ...
            .create_outgoing_dialog(
                local_uri, remote_uri, None, // Let dialog manager generate call-id
            )
            .await
    ...
    pub async fn send_status_response(
        ...
        let response = self
            .build_response(transaction_id, status_code, reason)
            .await?;
...
impl DialogApi for DialogClient {
    ...
    async fn start(&self) -> ApiResult<()> {
        ...
        self.dialog_manager.start().await.map_err(ApiError::from)?;
    ...
    async fn stop(&self) -> ApiResult<()> {
        ...
        self.dialog_manager.stop().await.map_err(ApiError::from)?;
