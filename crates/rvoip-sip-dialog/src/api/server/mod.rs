...
//!
//! ```rust,no_run
//! use rvoip_sip_dialog::api::{DialogServer, DialogApi, ServerConfig};
...
//! # Ok(())
//! # }
//! # async fn setup_dependencies() -> Result<(std::sync::Arc<rvoip_sip_dialog::transaction::TransactionManager>, rvoip_sip_dialog::api::ServerConfig), std::io::Error> { unimplemented!() }
//! ```
//!
...
//!
//! ```rust,no_run
//! use rvoip_sip_dialog::api::{DialogServer, DialogApi};
//! use rvoip_sip_dialog::events::SessionCoordinationEvent;
...
//!
//! ```rust,no_run
//! use rvoip_sip_dialog::api::DialogServer;
...
//!
//! ```rust
//! use rvoip_sip_dialog::api::ServerConfig;
...
//!
//! ```rust,no_run
//! use rvoip_sip_dialog::api::{DialogServer, ApiError};
//! use rvoip_sip_core::StatusCode;
//! use rvoip_sip_dialog::dialog::DialogId;
//!
...
//! # async fn cleanup_media_session(dialog_id: &rvoip_sip_dialog::dialog::DialogId) -> Result<(), Box<dyn std::error::Error + Send + Sync>> { Ok(()) }
//! # async fn setup_dependencies() -> Result<(std::sync::Arc<rvoip_sip_dialog::transaction::TransactionManager>, rvoip_sip_dialog::api::ServerConfig), std::io::Error> { unimplemented!() }
//! ```
//!
...
//!     tokio::sync::mpsc::channel::<rvoip_sip_dialog::events::SessionCoordinationEvent>(100);
//! server.start().await?;
//!
...
//!     while let Some(event) = session_rx.recv().await {
//!         match event {
//!             rvoip_sip_dialog::events::SessionCoordinationEvent::IncomingCall { dialog_id, .. } => {
...
//!                 }
//!             },
//!             rvoip_sip_dialog::events::SessionCoordinationEvent::CallTerminated { dialog_id, .. } => {
