...
use super::super::{ApiError, ApiResult, DialogApi, DialogStats};
use crate::manager::DialogManager;
use crate::transaction::{TransactionEvent, TransactionManager};
...
impl DialogApi for DialogServer {
    ...
    async fn start(&self) -> ApiResult<()> {
        info!(
            "Starting DialogServer on {}",
            self.config.dialog.local_address
        );
