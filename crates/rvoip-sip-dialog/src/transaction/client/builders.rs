...
/// # Example
/// ```
/// use rvoip_sip_dialog::transaction::client::builders::InviteBuilder;
/// use std::net::SocketAddr;
///
...
impl InviteBuilder {
    ...
    pub fn from_detailed(
        ...
        self.from_tag = tag
            .map(|s| s.to_string())
            .or_else(|| Some(format!("tag-{}", Uuid::new_v4().simple())));
        self
    }
    ...
    pub fn build(self) -> Result<Request> {
        ...
        let from_uri = self
            .from_uri
            .ok_or_else(|| Error::Other("From URI is required".to_string()))?;
        let to_uri = self
            .to_uri
            .ok_or_else(|| Error::Other("To URI is required".to_string()))?;
        let request_uri = self.request_uri.unwrap_or_else(|| to_uri.clone());
        let local_addr = self
            .local_address
            .ok_or_else(|| Error::Other("Local address is required for Via header".to_string()))?;
        ...
        if let Some(sdp_content) = &self.sdp_content {
            builder = builder
                .header(TypedHeader::ContentType(ContentType::from_type_subtype(
                    "application",
                    "sdp",
                )))
                .header(TypedHeader::ContentLength(ContentLength::new(
                    sdp_content.len() as u32,
                )))
...
/// # Example
/// ```
/// use rvoip_sip_dialog::transaction::client::builders::ByeBuilder;
/// use std::net::SocketAddr;
///
/// let local_addr: SocketAddr = "127.0.0.1:5060".parse().unwrap();
...
impl ByeBuilder {
    ...
    pub fn build(self) -> Result<Request> {
        ...
        let call_id = self
            .call_id
            .ok_or_else(|| Error::Other("Call-ID is required".to_string()))?;
        let from_uri = self
            .from_uri
            .ok_or_else(|| Error::Other("From URI is required".to_string()))?;
        let from_tag = self.from_tag.ok_or_else(|| {
            Error::Other("From tag is required for in-dialog request".to_string())
        })?;
        let to_uri = self
            .to_uri
            .ok_or_else(|| Error::Other("To URI is required".to_string()))?;
        let to_tag = self
            .to_tag
            .ok_or_else(|| Error::Other("To tag is required for in-dialog request".to_string()))?;
        let request_uri = self.request_uri.unwrap_or_else(|| to_uri.clone());
        let local_addr = self
            .local_address
            .ok_or_else(|| Error::Other("Local address is required".to_string()))?;
...
/// # Example
/// ```
/// use rvoip_sip_dialog::transaction::client::builders::RegisterBuilder;
/// use std::net::SocketAddr;
///
/// let local_addr: SocketAddr = "127.0.0.1:5060".parse().unwrap();
...
impl RegisterBuilder {
    ...
    /// Set the address-of-record being registered (To header)
    pub fn aor(mut self, uri: impl Into<String>) -> Self {
        self.aor_uri = Some(uri.into());
        self
    }
    ...
    /// Set a fully-built Contact header
    pub fn contact_header(mut self, contact: Contact) -> Self {
        self.contact_header = Some(contact);
        self
    }
    ...
    pub fn build(self) -> Result<Request> {
        ...
        let registrar_uri = self
            .registrar_uri
            .ok_or_else(|| Error::Other("Registrar URI is required".to_string()))?;
        let from_uri = self
            .from_uri
            .ok_or_else(|| Error::Other("From URI is required".to_string()))?;
        let aor_uri = self.aor_uri.unwrap_or_else(|| from_uri.clone());
        let local_addr = self
            .local_address
            .ok_or_else(|| Error::Other("Local address is required".to_string()))?;
        ...
        builder = match self.contact_header {
            Some(contact) => builder.header(TypedHeader::Contact(contact)),
...
#[cfg(test)]
mod register_builder_tests {
    use super::*;
    use rvoip_sip_core::types::headers::HeaderAccess;

    #[test]
    fn register_builder_creates_wire_ready_register() {
        let request = RegisterBuilder::new()
            .registrar("sip:pbx.example.com:5060")
            .aor("sip:1001@pbx.example.com")
            .user_info("sip:1001@pbx.example.com", "")
            .contact("sip:1001@192.0.2.10:5070")
            .local_address("192.0.2.10:5070".parse().unwrap())
            .expires(3600)
            .call_id("reg-test-call")
            .cseq(7)
            .build()
            .unwrap();

        assert_eq!(request.method(), Method::Register);
        assert_eq!(request.uri().to_string(), "sip:pbx.example.com:5060");
        assert_eq!(request.call_id().unwrap().value(), "reg-test-call");
        assert_eq!(request.cseq().unwrap().sequence(), 7);
        assert_eq!(
            request.to().unwrap().uri().to_string(),
            "sip:1001@pbx.example.com"
        );
        assert_eq!(
            request.from().unwrap().uri().to_string(),
            "sip:1001@pbx.example.com"
        );
        assert_eq!(
            request.raw_header_value(&HeaderName::Contact).unwrap(),
        ...
        assert!(request.header(&HeaderName::Via).is_some());
        assert!(request.header(&HeaderName::MaxForwards).is_some());
        assert!(request.header(&HeaderName::Expires).is_some());
        assert!(request.header(&HeaderName::ContentLength).is_some());
        rvoip_sip_core::validation::validate_wire_request(&request).unwrap();
    }
    ...
    fn register_builder_preserves_custom_authorization() {
        let request = RegisterBuilder::new()
            ...
            .header(TypedHeader::Other(
                ...
                rvoip_sip_core::types::header::HeaderValue::Raw(
                    b"Digest username=\"1001\"".to_vec(),
                ),
            ))
            .build()
            .unwrap();
...
/// # Example
/// ```
/// use rvoip_sip_dialog::transaction::client::builders::InDialogRequestBuilder;
...
pub mod quick {
    ...
    pub fn options(target_uri: &str, from_uri: &str, local_addr: SocketAddr) -> Result<Request> {
        use rvoip_sip_core::builder::SimpleRequestBuilder;
