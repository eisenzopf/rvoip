...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    session.on_event(|event| {
        if let SessionEvent::StateChanged { from: _, to } = event {
            if to == CallState::Ringing {
                println!("[BOB] Incoming call!");
            }
        }
    }).await?;
    
    // Wait for ringing state
    while session.state().await? != CallState::Ringing {
    ...
    while session.state().await? != CallState::Active {
        sleep(Duration::from_millis(100)).await;
    }
    ...
    println!("[BOB] Sent 250 frames, received {} frames", received);
    println!("[BOB] Done!");
    Ok(())
}