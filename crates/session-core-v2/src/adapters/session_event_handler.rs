...
impl CrossCrateEventHandler for SessionCrossCrateEventHandler {
    async fn handle(&self, event: Arc<dyn CrossCrateEvent>) -> Result<()> {
        ...
        // Note: We need to clone the Arc first before attempting downcast
        let event_clone = event.clone();
        
        // For now, we'll still use string parsing as Arc<dyn T> downcast is tricky
        // TODO: Implement proper event type registration to avoid this
...
impl SessionCrossCrateEventHandler {
    ...
    async fn handle_incoming_call(&self, event_str: &str) -> Result<()> {
        ...
        let dialog_id = self.extract_field(event_str, "session_id: \"").unwrap_or_else(|| "unknown".to_string());
        ...
        self.state_machine.store.create_session(
            session_id.clone(),
            Role::UAS, // User Agent Server role for incoming calls
            true, // with history
        ).await.map_err(|e| SessionError::InternalError(format!("Failed to create session: {}", e)))?;
        
        // Map the dialog to the session
        // Parse dialog_id string to UUID (dialog-core uses UUIDs as IDs)
        let dialog_uuid = uuid::Uuid::parse_str(&dialog_id).unwrap_or_else(|_| uuid::Uuid::new_v4());
        self.registry.map_dialog(session_id.clone(), DialogId(dialog_uuid));
        
        // Map the dialog in dialog adapter
        self.dialog_adapter.dialog_to_session.insert(
            rvoip_dialog_core::DialogId(dialog_uuid),
            session_id.clone()
        );
        self.dialog_adapter.session_to_dialog.insert(
            session_id.clone(),
            rvoip_dialog_core::DialogId(dialog_uuid)
        );
        
        // Store the transaction info for sending responses
        // We need to create a dummy Request and TransactionKey for now
        // In a real implementation, these would be passed through the event
        let transaction_key = rvoip_dialog_core::transaction::TransactionKey::from_str(&transaction_id)
            .map_err(|e| SessionError::InternalError(format!("Invalid transaction ID: {}", e)))?;
        let source_socket_addr = source_addr.parse::<std::net::SocketAddr>()
            .unwrap_or_else(|_| std::net::SocketAddr::from(([127, 0, 0, 1], 5061)));
        
        // Create a dummy request - this is a workaround since we don't have the full request
        ...
        // Create incoming call info and send through channel
        ...
        info!("Created session {} for incoming call from {} (dialog: {}, tx: {})", session_id.0, from, dialog_id, transaction_id);
        
        // Now process the IncomingCall event through the state machine
        let event_type = EventType::IncomingCall { from, sdp };
    ...
    async fn handle_dialog_state_changed(&self, event_str: &str) -> Result<()> {
        if let Some(session_id) = self.extract_session_id(event_str) {
            ...
            debug!("Dialog state changed for session {}", session_id);
            // TODO: Map to appropriate EventType when state machine supports it
        }
        Ok(())
    }
    ...
    async fn handle_reinvite_received(&self, event_str: &str) -> Result<()> {
        if let Some(session_id) = self.extract_session_id(event_str) {
            ...
            // TODO: Add EventType::ReinviteReceived to state machine
            debug!("Re-INVITE received for session {} with SDP: {:?}", session_id, sdp);
        }
        Ok(())
    }
    ...
    async fn handle_transfer_requested(&self, event_str: &str) -> Result<()> {
        if let Some(session_id) = self.extract_session_id(event_str) {
            ...
            // TODO: Add EventType::TransferRequested to state machine
            debug!("Transfer requested for session {} to {}", session_id, refer_to);
        }
        Ok(())
    }
    ...
    async fn handle_media_quality_degraded(&self, event_str: &str) -> Result<()> {
        if let Some(session_id) = self.extract_session_id(event_str) {
            // TODO: Add EventType::MediaQualityDegraded to state machine
            warn!("Media quality degraded for session {}", session_id);
        }
        Ok(())
    }
    ...
    async fn handle_dtmf_detected(&self, event_str: &str) -> Result<()> {
        if let Some(session_id) = self.extract_session_id(event_str) {
            ...
            // TODO: Add EventType::DtmfDetected to state machine
            info!("DTMF '{}' detected for session {}", digit, session_id);
        }
        Ok(())
    }
    ...
    async fn handle_rtp_timeout(&self, event_str: &str) -> Result<()> {
        if let Some(session_id) = self.extract_session_id(event_str) {
            // TODO: Add EventType::RtpTimeout to state machine
            error!("RTP timeout for session {}", session_id);
            
            // For now, treat as media error
            if let Err(e) = self.state_machine.process_event(
                &SessionId(session_id),
                EventType::MediaError("RTP timeout".to_string())
            ).await {
                error!("Failed to process RTP timeout: {}", e);
            }
        }
        Ok(())
    }
    ...
    async fn handle_packet_loss_threshold_exceeded(&self, event_str: &str) -> Result<()> {
        if let Some(session_id) = self.extract_session_id(event_str) {
            let loss = self.extract_field(event_str, "loss_percentage: ")
                .and_then(|s| s.parse::<f32>().ok())
                .unwrap_or(0.0);
            
            // TODO: Add EventType::PacketLossThresholdExceeded to state machine
            warn!("Packet loss {}% exceeded threshold for session {}", loss, session_id);
        }
        Ok(())
    }
}