...
| # | Task | Touches |
|---|---|---|
| V2.A.1 | Create `crates/rvoip-core-traits/` skeleton with Cargo.toml + empty modules. | new dir |
...
**Files.**
- New crate: `crates/rvoip-harness/` containing trait surfaces
...
**Tasks (9).**
1. Create `crates/rvoip-harness` skeleton (Cargo.toml, lib.rs, 4 traits,
   no-op default impls).
...
- Modify: `Cargo.toml` (add `[features]`), workspace `Cargo.toml`
  (register new crates).
- New: `crates/rvoip-identity/` skeleton, `crates/rvoip-client/`
  skeleton (deferred internals but present for layering).

**Tasks (4).**
1. Add `[features]` to `crates/rvoip-core/Cargo.toml`:
   ```toml
   [features]
