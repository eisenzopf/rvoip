...
fn build_request_with_macro(
    ...
    let uri_to_use = match SimpleRequestBuilder::new(method.clone(), uri) {
        ...
        Err(_) => {
            println!("Found problematic URI scheme, using default URI instead");
            // Return a default request for URIs that fail to parse
            return Ok(RequestBuilder::new(method, "sip:example.com")
                .expect("Failed to create RequestBuilder with default URI")
                .from("Alice", "sip:alice@example.com", Some("tag-value"))
                .to("Bob", "sip:bob@example.com", None)
                .call_id("dummy-call-id-for-test")
                .cseq(1)
                .via("example.com", "UDP", Some("z9hG4bK123"))
                .max_forwards(70)
                .build());
        }
    };
