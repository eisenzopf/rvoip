...
fn test_invalid_uris() {
    ...
    assert!(
        result.is_err(),
        "URI with empty user part should be rejected"
    );
    ...
    assert!(
        result.is_err(),
        "URI with non-numeric port should be rejected"
    );
    ...
    assert!(
        result.is_err(),
        "URI with port value out of range should be rejected"
    );
}
...
fn test_valid_uris() {
    ...
    assert!(
        result.is_ok(),
        "Valid SIP URI with parameters should be accepted"
    );
    ...
    assert!(
        result.is_ok(),
        "Valid SIP URI with headers should be accepted"
    );
    ...
    assert!(
        result.is_ok(),
        "Valid SIP URI with IPv4 host should be accepted"
    );
    ...
    assert!(
        result.is_ok(),
        "Valid SIP URI with IPv6 host should be accepted"
    );
}
