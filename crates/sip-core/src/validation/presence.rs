...
pub fn validate_publish_request(request: &Request) -> Result<()> {
    ...
    let has_event = request
        .headers
        .iter()
        .any(|h| matches!(h, TypedHeader::Event(_)) || h.name() == HeaderName::Event);
    ...
    let has_sip_if_match = request
        .headers
        .iter()
        .any(|h| matches!(h, TypedHeader::SipIfMatch(_)) || h.name() == HeaderName::SipIfMatch);
...
pub fn validate_subscribe_request(request: &Request) -> Result<()> {
    ...
    let has_expires = request
        .headers
        .iter()
        .any(|h| matches!(h, TypedHeader::Expires(_)) || h.name() == HeaderName::Expires);
...
pub fn validate_notify_request(request: &Request) -> Result<()> {
    ...
    if !has_subscription_state {
        return Err(Error::ValidationError(
            "NOTIFY request MUST contain Subscription-State header (RFC 6665 Section 7.2.1)"
                .to_string(),
        ));
    }
...
mod tests {
    ...
    fn test_validate_subscribe_request_valid() {
        ...
        request
            .headers
            .push(TypedHeader::Expires(Expires::new(3600)));
    ...
    fn test_validate_notify_request_valid() {
        ...
        request
            .headers
            .push(TypedHeader::SubscriptionState(SubscriptionState::active(
                3599,
            )));

        // NOTIFY must be in a dialog, so To must have a tag
        let to = To::new("sip:alice@example.com".parse().unwrap()).with_tag("xyz123");
