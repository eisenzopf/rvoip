...
use crate::types::{
    auth::{
        AuthenticationInfo, AuthenticationInfoParam, Authorization, Credentials, DigestParam,
        ProxyAuthenticate, ProxyAuthorization, Qop, WwwAuthenticate,
    },
...
fn test_auth_headers() {
    ...
    assert!(
        auth_info_header.is_some(),
        "Authentication-Info header not found"
    );

    if let Some(TypedHeader::AuthenticationInfo(info)) = auth_info_header {
        assert!(info.0.contains(&AuthenticationInfoParam::NextNonce(
            "47364c23432d2e131a5fb210812c".to_string()
        )));
        assert!(info.0.contains(&AuthenticationInfoParam::Qop(Qop::Auth)));
        assert!(info.0.contains(&AuthenticationInfoParam::ResponseAuth(
            "988df663c6161de88914873c9975fd4c".to_string()
        )));
        assert!(info
            .0
            .contains(&AuthenticationInfoParam::Cnonce("0a4f113b".to_string())));
...
fn test_header_roundtrip_conversion() {
    ...
    if let HeaderValue::Authorization(value) = &header.value {
        if let Credentials::Digest { params } = &value.0 {
            assert!(params
                .iter()
                .any(|p| matches!(p, DigestParam::Username(name) if name == "alice")));
        } else {
    ...
    if let HeaderValue::AuthenticationInfo(value) = &header.value {
        assert!(value.0.contains(&AuthenticationInfoParam::NextNonce(
            "47364c23432d2e131a5fb210812c".to_string()
        )));
        assert!(value.0.contains(&AuthenticationInfoParam::ResponseAuth(
            "988df663c6161de88914873c9975fd4c".to_string()
        )));
    } else {
