    ...
    T: Serialize + DeserializeOwned,
{
    fn to_sip_value(&self) -> SipJsonResult<SipValue> {
        ...
        let json_value = serde_json::to_value(self).map_err(SipJsonError::SerializeError)?;
    ...
    fn from_sip_value(value: &SipValue) -> SipJsonResult<Self> {
        ...
        serde_json::from_value::<T>(json_value).map_err(SipJsonError::DeserializeError)
    }
}
    ...
    T: Serialize + DeserializeOwned + SipJson,
{
                ...
                if path_str.ends_with(".uri") || path_str.ends_with("Uri") {
                    if let Some(scheme) = v.get_path("scheme").and_then(|s| s.as_str()) {
                        ...
                        if let Some(host_obj) = v.get_path("host") {
                            if let Some(domain) =
                                host_obj.get_path("Domain").and_then(|d| d.as_str())
                            {
                                uri.push_str(domain);
                            } else {
                    ...
                    if let Some(sent_protocol) = v.get_path("sent_protocol") {
                        ...
                        if let Some(host_obj) = v.get_path("sent_by_host") {
                                ...
                                host_obj.get_path("Domain").and_then(|d| d.as_str())
                            {
                                ...
                                // Port (if present)
                                if let Some(port) =
                                    v.get_path("sent_by_port").and_then(|p| p.as_f64())
                                {
                                    if port != 5060.0 {
                        ...
                        if let Some(params) = v.get_path("params") {
                            ...
                            if let Some(received) =
                                params.get_path("Received").and_then(|r| r.as_str())
                            {
                ...
                if path_str.contains(".Contact") {
                    if let Some(arr) = v.as_array() {
                        if !arr.is_empty() {
                            ...
                            if let Some(params) =
                                first.get_path("Params").and_then(|p| p.as_array())
                            {
                                if !params.is_empty() {
                                    if let Some(address) = params[0].get_path("address") {
                                        if let Some(uri) = address.get_path("uri") {
                                            ...
                                            if let Some(scheme) =
                                                uri.get_path("scheme").and_then(|s| s.as_str())
                                            {
                                                uri_str.push_str(scheme);
                                                uri_str.push(':');

                                                if let Some(user) =
                                                    uri.get_path("user").and_then(|u| u.as_str())
                                                {
    ...
    fn to_json_string(&self) -> SipJsonResult<String> {
        serde_json::to_string(self).map_err(|e| SipJsonError::SerializeError(e))
    }

    fn to_json_string_pretty(&self) -> SipJsonResult<String> {
        serde_json::to_string_pretty(self).map_err(|e| SipJsonError::SerializeError(e))
    }

    fn from_json_str(json_str: &str) -> SipJsonResult<Self> {
        serde_json::from_str::<Self>(json_str).map_err(|e| SipJsonError::DeserializeError(e))
    }
}
...
mod tests {
    ...
    fn test_request_to_json() {
        ...
        assert!(
            json.contains("\"method\":\"Invite\""),
            "JSON doesn't contain method"
        );
        assert!(
            json.contains("\"display_name\":\"Alice\""),
            "JSON doesn't contain display name"
        );
        assert!(
            json.contains("\"Tag\":\"tag12345\""),
            "JSON doesn't contain tag"
        );
    }
    ...
    fn test_path_accessor_chaining() {
        ...
        assert!(
            ...
            "From display_name should exist via path access"
        );
        ...
        assert!(
            method.is_some(),
            "method field should exist via path access"
        );
    ...
    fn test_message_json_cseq() {
        ...
        assert!(
            json_str.contains("From"),
            "From header should exist in JSON"
        );
        assert!(
            json_str.contains("Alice"),
            "From display name should exist in JSON"
        );
    ...
    fn test_complex_query_patterns() {
        ...
        assert!(
            request.path("headers.From").is_some(),
            "From header should exist"
        );
        assert_eq!(
            request.path_str_or("headers.From.display_name", ""),
            "Alice"
        );

        // Verify To header fields
        assert!(
            request.path("headers.To").is_some(),
            "To header should exist"
        );
        assert_eq!(request.path_str_or("headers.To.display_name", ""), "Bob");

        // Verify Via headers exist
        assert!(
            request.path("headers.Via").is_some(),
            "Via header should exist"
        );

        // Verify the Contact header exists
        assert!(
            request.path("headers.Contact").is_some(),
            "Contact header should exist"
        );
    ...
    fn test_from_sip_value() {
        ...
        assert!(
            parsed_value.is_ok(),
            "Should be able to parse request from JSON string"
        );
    ...
    fn test_edge_cases_and_error_handling() {
        ...
        assert!(
            request.path("headers.From.display_name").is_some(),
            "From display_name should exist"
        );
        assert!(
            request.path("headers.From.params[0].Tag").is_some(),
            "From tag should exist"
        );
    ...
    fn test_deep_paths_with_special_characters() {
        ...
        special_headers.insert(
            ...
            SipValue::String("application/sdp".to_string()),
        );
        special_headers.insert(
            "User-Agent".to_string(),
            SipValue::String("rvoip-test/1.0".to_string()),
        );
    ...
    fn test_path_methods() {
        ...
        assert_eq!(
            request.path("headers.From.display_name").unwrap().as_str(),
            Some("Alice")
        );
        assert!(request.path("non.existent.path").is_none());
        ...
        assert_eq!(
            request.path_str("headers.From.display_name").unwrap(),
            "Alice"
        );

        // Test default value fallback
        assert_eq!(
            request.path_str_or("non.existent.path", "default"),
            "default"
        );
        assert_eq!(
            request.path_str_or("headers.From.display_name", "default"),
            "Alice"
        );
    }
    ...
    fn test_message_json_via() {
        ...
        assert!(
            json_str.contains("pc33.atlanta.com"),
            "Via host should exist in JSON"
        );
        assert!(
            json_str.contains("z9hG4bK776asdhds"),
            "Via branch should exist in JSON"
        );
    }
    ...
    fn test_response_json() {
        ...
        assert!(
            ...
            "From display name should be in JSON"
        );
        assert!(
            json_str.contains("Alice"),
            "To display name should be in JSON"
        );
