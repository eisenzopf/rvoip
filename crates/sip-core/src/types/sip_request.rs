...
impl Request {
    ...
    /// Retrieves the CSeq sequence number, if present
    ///
    /// # Returns
    /// An optional CSeq sequence number
    pub fn cseq_number(&self) -> Option<u32> {
        self.cseq().map(|cseq| cseq.seq)
    }
    ...
    /// Retrieves the From URI as a string, if present
    ///
    /// # Returns
    /// An optional From URI string
    pub fn from_uri(&self) -> Option<String> {
        self.from().map(|from| from.address().uri.to_string())
    }
    ...
    /// Retrieves the To URI as a string, if present
    ///
    /// # Returns
    /// An optional To URI string
    pub fn to_uri(&self) -> Option<String> {
        self.to().map(|to| to.address().uri.to_string())
    }

    /// Retrieves the Via branch parameter, if present
    ///
    /// # Returns
    /// An optional Via branch string
    ...
    /// Retrieves the Via transport protocol from the first (topmost) Via header
    ///
    /// # Returns
    /// An optional Via transport string from the first Via header
    pub fn via_transport(&self) -> Option<&str> {
        self.first_via_transport()
    }

    /// Retrieves the Via host from the first (topmost) Via header
    ///
    /// # Returns
    /// An optional Via host string from the first Via header
    pub fn via_host(&self) -> Option<String> {
        self.first_via_host()
    }
    ...
    /// # Returns
    /// An optional Via transport string from the first Via header
    pub fn first_via_transport(&self) -> Option<&str> {
        for header in &self.headers {
            if let TypedHeader::Via(via) = header {
                if let Some(via_header) = via.0.first() {
                    return Some(&via_header.sent_protocol.transport);
                }
            }
        }
        None
    ...
    /// # Returns
    /// An optional Via host string from the first Via header
    pub fn first_via_host(&self) -> Option<String> {
        for header in &self.headers {
            if let TypedHeader::Via(via) = header {
                if let Some(via_header) = via.0.first() {
                    return Some(via_header.sent_by_host.to_string());
                }
            }
        }
        None
    ...
    pub fn all_via_transports(&self) -> Vec<String> {
        ...
        for header in &self.headers {
            if let TypedHeader::Via(via) = header {
                for via_header in &via.0 {
                    transports.push(via_header.sent_protocol.transport.clone());
                }
            }
        }
        transports
    ...
    pub fn all_via_hosts(&self) -> Vec<String> {
        ...
        for header in &self.headers {
            if let TypedHeader::Via(via) = header {
                for via_header in &via.0 {
                    hosts.push(via_header.sent_by_host.to_string());
                }
            }
        }
        hosts
            ...
            if let TypedHeader::Via(via) = header {
                for via_header in &via.0 {
                    for param in &via_header.params {
                        if let crate::types::param::Param::Branch(branch) = param {
                            branches.push(branch.clone());
                            break; // Only one branch per Via header
                        }
                    }
                }
            }
...
mod tests {
    ...
    fn test_new_with_essentials() {
        ...
        let request =
            Request::new_with_essentials(Method::Invite, to_uri, from_uri, "abc123@example.com", 1);
