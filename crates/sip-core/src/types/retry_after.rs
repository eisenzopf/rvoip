...
mod tests {
    ...
    fn test_retry_after_with_params() {
        let retry = RetryAfter::new(60).with_param(Param::new("reason", Some("maintenance")));
        assert_eq!(retry.to_string(), "60;reason=maintenance");
    }
    ...
    fn test_retry_after_from_str() {
        ...
        // Test complex case
        let retry =
            RetryAfter::from_str("3600 (System upgrade);duration=7200;reason=maintenance").unwrap();
        assert_eq!(retry.delay, 3600);
