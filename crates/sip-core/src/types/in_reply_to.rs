...
impl TypedHeaderTrait for InReplyTo {
    ...
    fn to_header(&self) -> Header {
        Header::new(Self::header_name(), HeaderValue::InReplyTo(self.clone()))
    }
    ...
    fn from_header(header: &Header) -> Result<Self> {
        if header.name != HeaderName::InReplyTo {
            return Err(Error::ParseError(format!(
                "Expected In-Reply-To header, got {}",
                header.name
            )));
        }
        ...
        let value_str = match &header.value {
            ...
            _ => {
                return Err(Error::ParseError(format!(
                    "Expected In-Reply-To header value, got {:?}",
                    header.value
                )))
            }
        };
...
mod tests {
    ...
    fn test_in_reply_to_with_multiple_strings() {
        let strings = vec!["id1@domain.com".to_string(), "id2@domain.com".to_string()];
