    ...
    fn from_str(s: &str) -> Result<Self> {
        ...
        if parts.is_empty() {
            return Err(Error::ParseError(
                "Empty Subscription-State header".to_string(),
            ));
        }
...
mod tests {
    ...
    fn test_subscription_state_terminated_with_retry() {
        let state = SubscriptionState::terminated_with_retry(TerminationReason::Giveup, 3600);
        assert_eq!(
            state.to_string(),
            "terminated;reason=giveup;retry-after=3600"
        );

        let parsed =
            SubscriptionState::from_str("terminated;reason=giveup;retry-after=3600").unwrap();
        assert_eq!(parsed, state);
    }
    ...
    fn test_substate_from_str() {
        ...
        assert_eq!(
            SubState::from_str("Terminated").unwrap(),
            SubState::Terminated
        );
    ...
    fn test_termination_reason_from_str() {
        assert_eq!(
            TerminationReason::from_str("timeout").unwrap(),
            TerminationReason::Timeout
        );
        assert_eq!(
            TerminationReason::from_str("rejected").unwrap(),
            TerminationReason::Rejected
        );
        assert_eq!(
            TerminationReason::from_str("custom").unwrap(),
            TerminationReason::Other("custom".to_string())
        );
    }
}
