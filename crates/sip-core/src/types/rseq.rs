...
impl FromStr for RSeq {
    ...
    fn from_str(s: &str) -> Result<Self> {
        ...
        match trimmed.parse::<u32>() {
            ...
            Err(_) => Err(crate::error::Error::ParseError(format!(
                "Invalid RSeq value: {}",
                s
            ))),
        }
...
mod tests {
    ...
    fn test_rseq_from_header() {
        // Create a header with raw value
        let header = Header::new(HeaderName::RSeq, HeaderValue::Raw(b"42".to_vec()));
        ...
        // Wrong header name
        let header = Header::new(HeaderName::From, HeaderValue::Raw(b"42".to_vec()));
    ...
    fn test_rseq_in_reliable_provisional_response() {
        ...
        let rseq_header = response
            .headers
            .iter()
            .find(|h| h.name() == HeaderName::RSeq);
    ...
    fn test_rseq_sequence_in_dialog() {
        ...
        let header1 = response1
            .headers
            .iter()
            .find(|h| h.name() == HeaderName::RSeq)
            .unwrap();
        let header2 = response2
            .headers
            .iter()
            .find(|h| h.name() == HeaderName::RSeq)
            .unwrap();
