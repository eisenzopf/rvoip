...
use crate::sdp::media::types::{is_valid_media_type, parse_media_type};
use crate::types::sdp::MediaDescription;
use nom::{
    bytes::complete::tag, character::complete::space1, combinator::opt, sequence::tuple, IResult,
};
...
mod tests {
    ...
    fn test_parse_basic_media_description() {
        ...
        assert!(
            result.is_ok(),
            "Failed to parse basic audio media description"
        );
    ...
    fn test_parse_with_m_prefix() {
        ...
        assert!(
            result.is_ok(),
            "Failed to parse media description with m= prefix"
        );
    ...
    fn test_parse_multiple_formats() {
        ...
        assert!(
            result.is_ok(),
            "Failed to parse media description with multiple formats"
        );
    ...
    fn test_parse_application_media() {
        ...
        assert!(
            result.is_ok(),
            "Failed to parse application media description"
        );
    ...
    fn test_parse_with_port_count() {
        ...
        assert!(
            result.is_ok(),
            "Failed to parse media description with port count"
        );
    ...
    fn test_parse_secure_rtp() {
        ...
        assert!(
            result.is_ok(),
            "Failed to parse media description with SRTP"
        );
    ...
    fn test_parse_whitespace_handling() {
        ...
        assert!(
            result.is_ok(),
            "Failed to parse media line with extra whitespace"
        );
