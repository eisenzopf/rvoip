...
pub fn update_media_with_attribute(
...
mod tests {
    ...
    fn test_update_media_with_direction_attribute() {
        ...
        assert!(
            result.is_ok(),
            "Failed to update media with direction attribute"
        );
        assert_eq!(
            media.direction,
            Some(MediaDirection::SendRecv),
            "Media direction not updated correctly"
        );
        assert_eq!(
            media.generic_attributes.len(),
            0,
            "Direction should not be added to generic attributes"
        );
    }
    ...
    fn test_update_media_with_ptime_attribute() {
        ...
        assert!(
            result.is_ok(),
            "Failed to update media with ptime attribute"
        );
        assert_eq!(media.ptime, Some(20), "Media ptime not updated correctly");
        assert_eq!(
            media.generic_attributes.len(),
            0,
            "Ptime should not be added to generic attributes"
        );
    }
    ...
    fn test_update_media_with_generic_attribute() {
        ...
        assert!(
            result.is_ok(),
            "Failed to update media with generic attribute"
        );
        assert_eq!(
            media.generic_attributes.len(),
            1,
            "Generic attribute not added"
        );

        if let ParsedAttribute::Ssrc(stored_attr) = &media.generic_attributes[0] {
            assert_eq!(
                stored_attr.ssrc_id, ssrc_attr.ssrc_id,
                "SSRC ID not stored correctly"
            );
            assert_eq!(
                stored_attr.attribute, ssrc_attr.attribute,
                "SSRC attribute not stored correctly"
            );
            assert_eq!(
                stored_attr.value, ssrc_attr.value,
                "SSRC value not stored correctly"
            );
        } else {
    ...
    fn test_parse_media_attributes() {
        ...
        assert_eq!(
            media.generic_attributes.len(),
            2,
            "Incorrect number of attributes added"
        );
        assert_eq!(
            media.generic_attributes, attrs,
            "Attributes not stored correctly"
        );
    }
    ...
    fn test_is_media_level_attribute() {
        ...
        let media_attrs = [
            "rtpmap", "fmtp", "ptime", "maxptime", "sendrecv", "sendonly", "recvonly", "inactive",
            "ssrc", "rtcp", "rtcp-mux", "mid", "extmap",
        ];

        for attr in &media_attrs {
            assert!(
                is_media_level_attribute(attr),
                "Failed to recognize media attribute: {}",
                attr
            );
        }

        // Test non-media level attributes
        let non_media_attrs = ["group", "tool", "timing", "charset", "invalid-attribute"];

        for attr in &non_media_attrs {
            assert!(
                !is_media_level_attribute(attr),
                "Incorrectly recognized as media attribute: {}",
                attr
            );
        }
    }
