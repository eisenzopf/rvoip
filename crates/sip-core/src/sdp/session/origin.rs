...
pub fn parse_origin_line(value: &str) -> Result<Origin> {
    ...
    if let Ok((remainder, origin)) = parse_origin_nom(value) {
        ...
        if !remainder.trim().is_empty() {
            return Err(Error::SdpParsingError(format!(
                "Invalid o= line format (extra content): {}",
                value
            )));
        }
        return Ok(origin);
    ...
    let value_to_parse = if let Some(stripped) = value.strip_prefix("o=") {
        stripped
    } else {
        value
    };
...
mod tests {
    ...
    fn test_parse_basic_origin() {
        ...
        assert_eq!(
            origin.sess_version, "2890842807",
            "Incorrect session version"
        );
        assert_eq!(origin.net_type, "IN", "Incorrect network type");
        assert_eq!(origin.addr_type, "IP4", "Incorrect address type");
        assert_eq!(
            origin.unicast_address, "10.47.16.5",
            "Incorrect unicast address"
        );
    }
    ...
    fn test_parse_with_hyphen_username() {
        ...
        assert!(
            result.is_ok(),
            "Failed to parse origin with hyphen username"
        );
    ...
    fn test_parse_ipv6_address() {
        ...
        assert_eq!(
            origin.unicast_address, "2001:db8::1",
            "Incorrect unicast address"
        );
    }
    ...
    fn test_parse_hostname() {
        ...
        let origin = result.unwrap();
        assert_eq!(
            origin.unicast_address, "example.com",
            "Incorrect unicast address"
        );
    }
    ...
    fn test_parse_numeric_username() {
        ...
        assert!(
            result.is_ok(),
            "Failed to parse origin with numeric username"
        );
    ...
    fn test_parse_large_session_values() {
        ...
        assert!(
            result.is_ok(),
            "Failed to parse origin with large session values"
        );

        let origin = result.unwrap();
        assert_eq!(
            origin.sess_id, "9223372036854775807",
            "Incorrect session ID"
        );
        assert_eq!(
            origin.sess_version, "9223372036854775807",
            "Incorrect session version"
        );
    }
    ...
    fn test_whitespace_handling() {
        ...
        assert!(
            result.is_ok(),
            "Failed to parse origin with extra whitespace"
        );
