...
pub fn parse_mid(value: &str) -> Result<String> {
    ...
    if trimmed.contains(' ') {
        return Err(Error::SdpParsingError(format!(
            "MID contains spaces, which is not allowed for tokens: {}",
            value
        )));
    }
    ...
    if trimmed.chars().any(|c| c.is_control()) {
        return Err(Error::SdpParsingError(format!(
            "MID contains control characters, which is not allowed for tokens: {}",
            value
        )));
    }
    ...
    if !trimmed.is_ascii() {
        return Err(Error::SdpParsingError(format!(
            "MID contains non-ASCII characters, which is not allowed for tokens: {}",
            value
        )));
    }
    ...
    if trimmed.chars().any(|c| {
        ...
        return Err(Error::SdpParsingError(format!(
            "MID contains characters not allowed in tokens: {}",
            value
        )));
    }
