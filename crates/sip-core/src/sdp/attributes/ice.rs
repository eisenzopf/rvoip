...
pub fn parse_ice_ufrag(value: &str) -> Result<String> {
    ...
    if trimmed.chars().any(|c| c.is_control()) {
        return Err(Error::SdpParsingError(format!(
            "Invalid ice-ufrag value contains control characters: {}",
            value
        )));
    }
    ...
    if !trimmed.is_ascii() {
        return Err(Error::SdpParsingError(format!(
            "Invalid ice-ufrag value contains non-ASCII characters: {}",
            value
        )));
    }
...
pub fn parse_ice_pwd(value: &str) -> Result<String> {
    ...
    if trimmed.chars().any(|c| c.is_control()) {
        return Err(Error::SdpParsingError(format!(
            "Invalid ice-pwd value contains control characters: {}",
            value
        )));
    }
    ...
    if !trimmed.is_ascii() {
        return Err(Error::SdpParsingError(format!(
            "Invalid ice-pwd value contains non-ASCII characters: {}",
            value
        )));
    }
...
mod tests {
    ...
    fn test_real_world_examples() {
        ...
        assert_eq!(
            parse_ice_options(options2).unwrap(),
            vec!["trickle", "ice2"]
        );
    }
}
