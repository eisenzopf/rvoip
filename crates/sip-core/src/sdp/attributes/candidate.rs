...
pub fn parse_candidate(value: &str) -> Result<ParsedAttribute> {
    ...
    if parts.len() < 8 {
        return Err(Error::SdpParsingError(format!(
            "Invalid candidate format, insufficient parts: {}",
            value
        )));
    }
    ...
    let component_id = match parts[1].parse::<u32>() {
        Ok(id) if (1..=256).contains(&id) => id,
        _ => {
            return Err(Error::SdpParsingError(format!(
                "Invalid component ID in candidate: {}",
                parts[1]
            )))
        }
    };
    ...
    if transport.to_uppercase() != "UDP" && transport.to_uppercase() != "TCP" {
        return Err(Error::SdpParsingError(format!(
            "Invalid transport in candidate: {}",
            transport
        )));
    }
    ...
    {
        return Err(Error::SdpParsingError(format!(
            "Invalid connection address in candidate: {}",
            connection_address
        )));
    }
    ...
    if parts[6] != "typ" {
        return Err(Error::SdpParsingError(format!(
            "Expected 'typ' keyword in candidate, found: {}",
            parts[6]
        )));
    }
        ...
        match parts[idx] {
            "raddr" => {
                ...
                if !is_valid_ipv4(&addr) && !is_valid_ipv6(&addr) && !is_valid_hostname(&addr) {
                    return Err(Error::SdpParsingError(format!(
                        "Invalid related address in candidate: {}",
                        addr
                    )));
                }
            ...
            "rport" => {
                ...
                match parts[idx].parse::<u16>() {
                    ...
                    _ => {
                        return Err(Error::SdpParsingError(format!(
                            "Invalid related port in candidate: {}",
                            parts[idx]
                        )))
                    }
                }
...
mod tests {
    ...
    fn test_parse_relay_candidate() {
        ...
        let value =
            "3 1 UDP 16777215 2001:db8:1234::1 10000 typ relay raddr 2001:db8:1234::2 rport 9000";
    ...
    fn test_invalid_candidates() {
        ...
        assert!(
            parse_candidate("1 1 UDP 2130706431 10.0.1.1 8998 typ srflx raddr 192.168.1.1")
                .is_err()
        );
    }
