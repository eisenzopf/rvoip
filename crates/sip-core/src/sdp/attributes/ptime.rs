...
pub fn parse_ptime(value: &str) -> Result<u32> {
    ...
    if trimmed.contains('.') {
        return Err(Error::SdpParsingError(format!(
            "ptime must be an integer value without decimals: {}",
            value
        )));
    }
    ...
    if !trimmed.chars().all(|c| c.is_ascii_digit()) {
        return Err(Error::SdpParsingError(format!(
            "ptime must contain only digits: {}",
            value
        )));
    }
...
pub fn parse_maxptime(value: &str) -> Result<u32> {
    ...
    if trimmed.contains('.') {
        return Err(Error::SdpParsingError(format!(
            "maxptime must be an integer value without decimals: {}",
            value
        )));
    }
    ...
    if !trimmed.chars().all(|c| c.is_ascii_digit()) {
        return Err(Error::SdpParsingError(format!(
            "maxptime must contain only digits: {}",
            value
        )));
    }
