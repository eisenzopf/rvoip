...
fn parse_simulcast_version(input: &str) -> IResult<&str, SimulcastVersion> {
    ...
    map(
        separated_list1(char(','), parse_simulcast_alternative),
...
pub fn parse_simulcast_struct(value: &str) -> Result<Vec<SimulcastAttribute>> {
    ...
    if value.is_empty() {
        return Err(Error::SdpParsingError(
            "Empty simulcast attribute".to_string(),
        ));
    }
    ...
    while i < parts.len() {
        ...
        if i >= parts.len() {
            return Err(Error::SdpParsingError(format!(
                "Incomplete simulcast attribute: {}",
                value
            )));
        }
        ...
        if versions_str.trim().ends_with(',') || versions_str.trim().ends_with(';') {
            return Err(Error::SdpParsingError(format!(
                "Invalid simulcast stream format (trailing delimiter): {}",
                versions_str
            )));
        }

        // Check for invalid RID format
        if versions_str.contains('@')
            || versions_str.contains('!')
            || versions_str.contains('#')
            || versions_str.contains('$')
            || versions_str.contains(",,")
            || versions_str.contains(";;")
        {
            return Err(Error::SdpParsingError(format!(
                "Invalid RID format in simulcast: {}",
                versions_str
            )));
        }
        ...
        match parse_simulcast_stream_versions(&normalized_versions) {
            ...
            Err(_) => {
                return Err(Error::SdpParsingError(format!(
                    "Invalid simulcast stream versions: {}",
                    versions_str
                )));
            }
        }
    ...
    if result.is_empty() {
        Err(Error::SdpParsingError(format!(
            "Failed to parse simulcast attribute: {}",
            value
        )))
    } else {
        Ok(result)
...
pub fn parse_simulcast_compat(value: &str) -> Result<(Vec<String>, Vec<String>)> {
    ...
    for attr in simulcast_attrs {
        let str_versions: Vec<String> = attr
            .stream_versions
            .iter()
            .map(|version| {
                version
                    ...
                    .iter()
                    .map(|alt| {
                        let prefix = if matches!(alt.status, SimulcastStatus::Paused) {
                            "~"
                        } else {
                            ""
                        };
...
#[deprecated(
    since = "0.1.0",
    note = "Use parse_simulcast() returning SimulcastAttribute instead"
)]
...
mod tests {
    ...
    fn test_simulcast_parsing() {
        ...
        assert_eq!(
            attrs.len(),
            2,
            "Expected two simulcast attributes (send and recv)"
        );
        ...
        assert_eq!(
            send_attr.stream_versions.len(),
            2,
            "Expected 2 send stream versions"
        );
        ...
        assert_eq!(
            recv_attr.stream_versions.len(),
            2,
            "Expected 2 recv stream versions"
        );
        ...
        assert!(matches!(
            send_version2.alternatives[0].status,
            SimulcastStatus::Paused
        ));
    ...
    fn test_complex_simulcast() {
        ...
        assert_eq!(send_version1.alternatives[0].rid, "a1");
        assert!(matches!(
            send_version1.alternatives[0].status,
            SimulcastStatus::Active
        ));
        assert_eq!(send_version1.alternatives[1].rid, "a2");
        assert!(matches!(
            send_version1.alternatives[1].status,
            SimulcastStatus::Paused
        ));
        assert_eq!(send_version1.alternatives[2].rid, "a3");
        assert!(matches!(
            send_version1.alternatives[2].status,
            SimulcastStatus::Active
        ));
        ...
        assert!(matches!(
            send_version2.alternatives[1].status,
            SimulcastStatus::Paused
        ));
        ...
        assert_eq!(
            recv_attr.stream_versions.len(),
            3,
            "Expected 3 recv stream versions"
        );
    }
    ...
    fn test_simulcast_edge_cases() {
        ...
        assert!(
            parse_simulcast_struct(invalid_pattern).is_err(),
            "Should reject trailing comma in alternatives"
        );

        // Test with trailing semicolon
        let invalid_pattern2 = "send a1;";
        assert!(
            parse_simulcast_struct(invalid_pattern2).is_err(),
            "Should reject trailing semicolon in versions"
        );
    ...
    fn test_rfc8853_examples() {
        ...
        assert!(matches!(
            recv_attr.stream_versions[0].alternatives[0].status,
            SimulcastStatus::Paused
        ));
        assert_eq!(recv_attr.stream_versions[0].alternatives[1].rid, "rid-2");
        assert!(matches!(
            recv_attr.stream_versions[0].alternatives[1].status,
            SimulcastStatus::Active
        ));
    }
    ...
    fn test_invalid_simulcast_formats() {
        // Empty string
        assert!(
            parse_simulcast_struct("").is_err(),
            "Should reject empty string"
        );

        // Missing stream versions
        assert!(
            parse_simulcast_struct("send").is_err(),
            "Should reject missing stream versions"
        );

        // Empty stream versions
        assert!(
            parse_simulcast_struct("send ").is_err(),
            "Should reject empty stream versions"
        );

        // Invalid direction
        assert!(
            parse_simulcast_struct("invalid a1").is_err(),
            "Should reject invalid direction"
        );

        // Invalid RID format with @
        assert!(
            parse_simulcast_struct("send a1@invalid").is_err(),
            "Should reject invalid RID format with @"
        );

        // Malformed version list (empty part between commas)
        assert!(
            parse_simulcast_struct("send a1,,a2").is_err(),
            "Should reject empty parts between commas"
        );

        // Malformed version list (empty part between semicolons)
        assert!(
            parse_simulcast_struct("send a1;;a2").is_err(),
            "Should reject empty parts between semicolons"
        );

        // Missing direction value
        assert!(
            parse_simulcast_struct("a1").is_err(),
            "Should reject missing direction"
        );

        // Valid format but with direction in wrong case
        assert!(
            parse_simulcast_struct("SEND a1").is_err(),
            "Should reject direction in wrong case"
        );
    }
    ...
    fn test_whitespace_handling() {
        ...
        assert!(
            result.is_ok(),
            "Should handle spaces around commas and semicolons"
        );
        let attrs = result.unwrap();
