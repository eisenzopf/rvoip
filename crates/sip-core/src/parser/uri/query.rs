    ...
    for part in parts {
        ...
        if let Some(pos) = part.iter().position(|&c| c == b'=') {
            ...
            if !key_bytes.is_empty() {
                let key = unescape_uri_component(key_bytes).map_err(|_| {
                    nom::Err::Error(nom::error::Error::new(input, nom::error::ErrorKind::Char))
                })?;
                let value = unescape_uri_component(value_bytes).map_err(|_| {
                    nom::Err::Error(nom::error::Error::new(input, nom::error::ErrorKind::Char))
                })?;
...
mod tests {
    ...
    fn test_query_with_all_allowed_chars() {
        ...
        assert!(rem.is_empty());
        assert_eq!(
            params.get("unreserved"),
            Some(&"abc123-_.!~*'()".to_string())
        );
    ...
    fn test_individual_reserved_chars() {
        ...
        for (input, expected) in inputs.iter() {
            ...
            assert!(rem.is_empty());
            let param_name =
                std::str::from_utf8(&input[..input.iter().position(|&c| c == b'=').unwrap()])
                    .unwrap();
    ...
    fn test_rfc4475_query_torture() {
        ...
        let (rem, params) =
            parse_query_params(b"complex=%26%3D%3B&multi%20word=complex%20value").unwrap();
        assert!(rem.is_empty());
