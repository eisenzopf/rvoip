...
pub use host::{host, hostport, port};
...
pub fn parse_sip_uri(bytes: &[u8]) -> ParseResult<Uri> {
    ...
    match res {
        ...
        Err(nom::Err::Error(e)) | Err(nom::Err::Failure(e)) => Err(nom::Err::Error(e)),
        Err(e) => Err(e),
    }
}
        ...
        if !ipv6_reference_open && colon_pos + 1 < input.len() {
            ...
            if !port_start.is_empty() && port_start[0] != b'[' {
                ...
                if i > 0 {
                    ...
                    match port_digits.len().cmp(&5) {
                        std::cmp::Ordering::Greater => {
                            // More than 5 digits definitely exceeds 65535
                            let error =
                        ...
                        std::cmp::Ordering::Less => {
                            // Less than 5 digits, definitely valid
                        }
                    }
                }
            }
...
fn parse_tel_uri(input: &[u8]) -> ParseResult<Uri> {
    ...
    let (input, number) = recognize(take_while(|c| {
...
mod tests {
        ...
        match parse_sip_uri_fixed(uri_bytes) {
            Ok((rem, uri)) => {
                ...
                assert!(uri
                    .parameters
                    .iter()
                    .any(|p| matches!(p, Param::Transport(s) if s == "tcp")));
    ...
    fn test_parse_sip_uri_ipv4() {
        ...
        assert_eq!(uri.password, None);
        assert!(
            matches!(uri.host, Host::Address(addr) if addr == IpAddr::from(Ipv4Addr::new(192, 168, 0, 1)))
        );
    ...
    fn test_parse_sip_uri_complex() {
        let uri_bytes =
            b"sips:bob:password@[fe80::1]:5061;transport=tls;maddr=192.0.2.1?Subject=Hello";
        let (rem, uri) = parse_uri(uri_bytes).expect("Parsing failed");
        assert!(rem.is_empty());
        assert_eq!(uri.scheme, Scheme::Sips);
        ...
        assert!(uri
            .parameters
            .iter()
            .any(|p| matches!(p, Param::Transport(s) if s == "tls")));
        assert!(uri
            .parameters
            .iter()
            .any(|p| matches!(p, Param::Maddr(s) if s == "192.0.2.1")));
        assert!(!uri.headers.is_empty());
        let headers = uri.headers;
    ...
    fn test_rfc3261_example_uris() {
        ...
        for example in examples {
            ...
            assert!(
                result.is_ok(),
                "Failed to parse RFC 3261 example: {}",
                String::from_utf8_lossy(example)
            );
            let (rem, _) = result.unwrap();
            assert!(
                rem.is_empty(),
                "Remaining bytes should be empty for: {}",
                String::from_utf8_lossy(example)
            );
        }
    }
    ...
    fn test_uri_with_all_components() {
        ...
        assert!(uri
            .parameters
            .iter()
            .any(|p| matches!(p, Param::Method(s) if s == "INVITE")));
        assert!(uri
            .parameters
            .iter()
            .any(|p| matches!(p, Param::Ttl(val) if *val == 60)));
        assert!(uri.parameters.iter().any(|p| matches!(p, Param::Lr)));
        assert!(uri
            .parameters
            .iter()
            .any(|p| matches!(p, Param::Maddr(s) if s == "192.168.1.1")));
    ...
    fn test_uri_character_escaping() {
        ...
        let (_, uri) =
            parse_uri(uri_bytes).expect("Failed to parse URI with escaped user/password");
        ...
        let uri_bytes =
            b"sip:alice@example.com?subject=urgent%20meeting&location=conference%20room";
        let (_, uri) = parse_uri(uri_bytes).expect("Failed to parse URI with escaped headers");
        assert_eq!(
            uri.headers.get("subject"),
            Some(&"urgent meeting".to_string())
        );
        assert_eq!(
    ...
    fn test_uri_parameter_combinations() {
        ...
        assert!(uri
            .parameters
            .iter()
            .any(|p| matches!(p, Param::User(s) if s == "phone")));
        assert!(uri
            .parameters
            .iter()
            .any(|p| matches!(p, Param::Method(s) if s == "INVITE")));
        assert!(uri
            .parameters
            .iter()
            .any(|p| matches!(p, Param::Ttl(val) if *val == 120)));
        assert!(uri.parameters.iter().any(|p| matches!(p, Param::Lr)));
        ...
        assert!(uri
            .parameters
            .iter()
            .any(|p| matches!(p, Param::Other(n, None) if n == "custom")));
    }
    ...
    fn test_sip_tel_subscriber_syntax() {
        ...
        let uri_bytes = b"sip:+1(212)555-1212@gateway.com;user=phone";
        let (_, uri) =
            parse_uri(uri_bytes).expect("Failed to parse URI with phone visual separators");
        assert_eq!(uri.user, Some("+1(212)555-1212".to_string()));
    }
    ...
    fn test_sip_uri_with_complex_parameters() {
        ...
        assert!(parsed
            .parameters
            .iter()
            .any(|p| matches!(p, Param::Ttl(val) if *val == 5)));
        assert!(parsed
            .parameters
            .iter()
            .any(|p| matches!(p, Param::Maddr(s) if s == "239.255.255.1")));
        assert!(parsed
            .parameters
            .iter()
            .any(|p| matches!(p, Param::User(s) if s == "phone")));
    ...
    fn test_sip_uri_with_complex_headers() {
        ...
        // Check headers
        assert_eq!(
            parsed.headers.get("Accept-Contact"),
    ...
    fn test_full_featured_sip_uri() {
        ...
        assert_eq!(parsed.headers.get("priority"), Some(&"urgent".to_string()));
        assert_eq!(
            parsed.headers.get("Call-Info"),
            Some(&"<http://www.example.com/alice/photo.jpg>;purpose=icon".to_string())
        );
    }
    ...
    fn test_strict_rfc_compliance_edge_cases_port_non_numeric() {
        ...
        assert!(
            result.is_err(),
            "Parser should reject non-numeric port values"
        );
    }
    ...
    fn test_strict_rfc_compliance_edge_cases_param_no_name() {
        ...
        assert!(
            result.is_err(),
            "Parser should reject parameters with no name"
        );
    }
    ...
    fn test_strict_rfc_compliance_edge_cases_userinfo_multiple_colons() {
        ...
        assert!(
            result.is_err(),
            "Parser should reject multiple colons in userinfo"
        );
    }
    ...
    fn test_strict_rfc_compliance_edge_cases_host_leading_period() {
        ...
        assert!(
            result.is_err(),
            "Parser should reject leading periods in domain"
        );
    }
