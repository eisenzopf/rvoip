...
pub fn ttl_value(input: &[u8]) -> ParseResult<u8> {
    ...
    map_res(
        ...
        Ok::<u8, nom::Err<nom::error::Error<&[u8]>>>, // Ensure Ok type matches expected Result
    )(input)
}
...
mod tests {
    ...
    fn test_qvalue() {
        ...
        assert_eq!(
            qvalue(b"0.5;param"),
            Ok((&b";param"[..], NotNan::new(0.5).unwrap()))
        );
        assert_eq!(
            qvalue(b"1.0,next"),
            Ok((&b",next"[..], NotNan::new(1.0).unwrap()))
        );
    ...
    fn test_text_utf8_trim() {
        ...
        // Basic ASCII
        assert_eq!(
            text_utf8_trim(b"Subject Text"),
            Ok((&[][..], &b"Subject Text"[..]))
        );
        assert_eq!(text_utf8_trim(b"OneChar"), Ok((&[][..], &b"OneChar"[..])));
        assert_eq!(
            text_utf8_trim(b"One\t Two"),
            Ok((&[][..], &b"One\t Two"[..]))
        ); // Internal LWS
        assert_eq!(
            text_utf8_trim(b"Leading LWS"),
            Ok((&[][..], &b"Leading LWS"[..]))
        ); // LWS before second char

        // With UTF-8
        assert_eq!(
            text_utf8_trim(&[b'H', b'e', b'l', b'l', b'o', b' ', 0xC3, 0xA7]),
            Ok((
                &[][..],
                &[b'H', b'e', b'l', b'l', b'o', b' ', 0xC3, 0xA7][..]
            ))
        ); // "Hello ç"
        assert_eq!(
            text_utf8_trim(&[0xC3, 0xA7, b' ', b'W', b'o', b'r', b'l', b'd']),
            Ok((
                &[][..],
                &[0xC3, 0xA7, b' ', b'W', b'o', b'r', b'l', b'd'][..]
            ))
        ); // "ç World"
        assert_eq!(
            text_utf8_trim(&[0xC3, 0xA7, b'\t', 0xE2, 0x82, 0xAC]),
            Ok((&[][..], &[0xC3, 0xA7, b'\t', 0xE2, 0x82, 0xAC][..]))
        ); // "ç\t€"

        // RFC 3261 examples (from Subject, Call-ID, Reason-Phrase etc.)
        assert_eq!(
            text_utf8_trim(b"I know you're there"),
            Ok((&[][..], &b"I know you're there"[..]))
        );
        assert_eq!(
            text_utf8_trim(b"Unsupported Media Type"),
            Ok((&[][..], &b"Unsupported Media Type"[..]))
        );
        ...
        assert_eq!(
            text_utf8_trim(b"First\r\n Second"),
            Ok((&[][..], &b"First\r\n Second"[..]))
        );
        assert_eq!(
            text_utf8_trim(b"Multi\r\n Line\r\n Text"),
            Ok((&[][..], &b"Multi\r\n Line\r\n Text"[..]))
        );
        ...
        assert_eq!(
            text_utf8_trim(folded_input),
            Ok((&[][..], &folded_input[..]))
        ); // Raw bytes kept for now

        // Multiple line folding
        let complex_folding = b"Line1\r\n Line2\r\n Line3";
        assert_eq!(
            text_utf8_trim(complex_folding),
            Ok((&[][..], &complex_folding[..]))
        ); // Raw bytes kept for now
    ...
    fn test_rfc3261_examples() {
        ...
        assert_eq!(
            text_utf8_trim(b"Bad Request"),
            Ok((&[][..], &b"Bad Request"[..]))
        );
        assert_eq!(
            text_utf8_trim(b"Not Found"),
            Ok((&[][..], &b"Not Found"[..]))
        );
    }
    ...
    fn test_rfc4475_examples() {
        ...
        assert_eq!(
            text_utf8_trim(b"This is a text with escaped %22quotes%22"),
        ...
        assert_eq!(
            text_utf8_trim(b"text with  lots   of    whitespace"),
