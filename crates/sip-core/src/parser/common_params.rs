...
pub fn unquote_string(input: &[u8]) -> std::result::Result<String, Error> {
    ...
    String::from_utf8(unescaped)
        .map_err(|e| Error::ParseError(format!("UTF-8 error in quoted string: {}", e)))
}
...
fn gen_value(input: &[u8]) -> ParseResult<GenericValue> {
    ...
    if input.len() >= 7 && // Min IP length (1.2.3.4)
       (input.iter().filter(|&&c| c == b'.').count() == 3 || // IPv4 has 3 dots
        input.iter().filter(|&&c| c == b':').count() >= 2)
    {
...
mod tests {
    ...
    fn test_gen_value() {
        ...
        assert!(
            matches!(val_ip, GenericValue::Host(Host::Address(a)) if a == IpAddr::from(Ipv4Addr::new(192,0,2,1)))
        );
    ...
    fn test_generic_param_value() {
        ...
        assert!(
            matches!(param, Param::Other(n, Some(GenericValue::Token(v))) if n == "name" && v == "value")
        );
    }
    ...
    fn test_generic_param_host() {
        ...
        assert!(
            matches!(param, Param::Other(n, Some(GenericValue::Host(Host::Address(a)))) if n == "maddr" && a == IpAddr::from(Ipv4Addr::new(192,0,2,1)))
        );
    }
    ...
    fn test_generic_param_quoted() {
        ...
        assert!(rem.is_empty());
        assert!(
            matches!(param, Param::Other(n, Some(GenericValue::Quoted(v))) if n == "display" && v == "Bob Smith")
        );
    }
    ...
    fn test_generic_param_empty_quoted() {
        ...
        assert!(rem.is_empty());
        assert!(
            matches!(param, Param::Other(n, Some(GenericValue::Quoted(v))) if n == "empty" && v == "")
        );
    }
    ...
    fn test_accept_param_generic() {
        ...
        assert!(
            matches!(param, Param::Other(n, Some(GenericValue::Token(v))) if n == "level" && v == "1")
        );
    }
    ...
    fn test_contact_param_item() {
        ...
        assert!(
            matches!(param_gen, Param::Other(n, Some(GenericValue::Quoted(v))) if n == "+sip.instance" && v == "urn:uuid:123")
        );
    }
    ...
    fn test_semicolon_params_rfc_cases() {
        ...
        let (rem, params) =
            semicolon_params0(b"; param1 = value1 ;  param2;  param3=\"quoted\"").unwrap();
        assert!(rem.is_empty());
        assert_eq!(params.len(), 3);
        ...
        if let Param::Other(name, Some(value)) = &params[0] {
            assert_eq!(name, "param1");
            assert!(
                value.as_str().unwrap() == "value1 " || value.as_str().unwrap() == "value1",
