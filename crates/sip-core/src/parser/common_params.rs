...
pub fn tag_param(input: &[u8]) -> ParseResult<Param> {
    // Add debug logging
    // tracing::error!("Attempting to parse tag from: {:?}", input);
    ...
    match preceded(tag_no_case(b"tag"), preceded(equal, token))(input) {
        Ok((rem, tag_bytes)) => {
            ...
            match str::from_utf8(tag_bytes) {
                Ok(s) => {
                    // tracing::error!("Successfully parsed tag: {}", s);
                    Ok((rem, Param::Tag(s.to_string())))
                },
                Err(e) => {
                    // tracing::error!("UTF-8 error in tag: {:?}", e);
        ...
        Err(e) => {
            // tracing::error!("Failed to parse tag parameter: {:?}", e);
            Err(e)
        }
    }
}
