...
fn display_name(input: &[u8]) -> ParseResult<String> {
    alt((
        ...
        map_res(recognize(many1(terminated(token, opt(lws)))), |bytes| {
            str::from_utf8(bytes).map(|s| s.trim().to_string())
        }),
    ))(input)
}
...
pub fn name_addr(input: &[u8]) -> ParseResult<Address> {
    ...
    let closing_bracket_pos = input.iter().position(|&c| c == b'>').ok_or_else(|| {
...
mod tests {
    ...
    fn test_delimited_manually() {
        ...
        assert_eq!(
            std::str::from_utf8(uri_part).unwrap(),
    ...
    fn test_name_addr_angle_bracketed() {
        ...
        assert!(
            rem5.is_empty(),
            "Expected empty remainder, got: {:?}",
            std::str::from_utf8(rem5).unwrap_or("invalid UTF-8")
        );
