...
fn warn_agent(input: &[u8]) -> ParseResult<WarnAgent> {
    alt((
        ...
        map_res(token, |t| {
            str::from_utf8(t)
                .map(|s| WarnAgent::Pseudonym(s.to_string()))
...
mod tests {
    ...
    fn test_warning_value() {
        ...
        assert_eq!(val.text, b"Session parameter 'foo' not understood".to_vec());
        // Compare Vec<u8>
    }
    ...
    fn test_parse_warning_multiple() {
        ...
        assert!(
            matches!(warnings[1].agent, WarnAgent::HostPort(Host::Address(a), None) if a == IpAddr::from(Ipv4Addr::new(192,168,1,1)))
        );
    }
}
