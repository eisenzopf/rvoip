...
fn enclosed_uri(input: &[u8]) -> ParseResult<String> {
    ...
    map_res(
        ...
        |uri_bytes: &[u8]| -> Result<String, CrateError> {
            ...
            if trimmed.is_empty() {
                return Err(CrateError::ParseError(
                    "Empty URI after trimming".to_string(),
                ));
            }
            ...
            if scheme.is_empty() || !scheme.chars().next().unwrap().is_ascii_alphabetic() {
                return Err(CrateError::ParseError(format!(
                    "Invalid scheme: {}",
                    scheme
                )));
            }
            ...
            if scheme_end.unwrap() + 1 >= trimmed.len() {
                return Err(CrateError::ParseError(
                    "Missing URI content after scheme".to_string(),
                ));
            }
...
fn create_safe_uri(uri_str: &str) -> Result<Uri, CrateError> {
    ...
    match crate::parser::uri::parse_uri(uri_str.as_bytes()) {
        ...
        Err(e) => {
            ...
            eprintln!(
                "Error-Info: Failed to parse URI '{}' with nom parser: {:?}. Storing as raw.",
                uri_str, e
            );
...
mod tests {
    ...
    fn test_parse_error_info() {
        ...
        assert!(
            matches!(&infos[0].params[0], Param::Other(n, Some(v)) if n == "reason" && v.to_string() == "Foo")
        );
    }
    ...
    fn test_parse_error_info_multiple() {
        ...
        assert!(
            matches!(&infos[1].params[0], Param::Other(n, Some(v)) if n == "param" && v.to_string() == "1")
        );
    }
    ...
    fn test_uri_schemes() {
        ...
        let test_cases = [
            ...
            (
                "<http://example.com/errors/busy>",
                "http://example.com/errors/busy",
            ),
            (
                "<https://example.com/errors/busy>",
                "https://example.com/errors/busy",
            ),
            ("<tel:+1-212-555-1234>", "tel:+1-212-555-1234"),
            ("<mailto:user@example.com>", "mailto:user@example.com"), // Additional scheme test
            (
                "<ftp://ftp.example.com/error.txt>",
                "ftp://ftp.example.com/error.txt",
            ), // Additional scheme test
        ];
    ...
    fn test_whitespace_handling() {
        ...
        assert!(
            result_ws_uri.is_ok(),
            "Should handle whitespace inside URI angle brackets"
        );
        let (_, infos_ws) = result_ws_uri.unwrap();
        assert_eq!(
            infos_ws[0].uri_str, "sip:busy@example.com",
            "URI should be trimmed"
        );
    }
    ...
    fn test_parameter_variations() {
        ...
        assert!(
            result_uri_param.is_err(),
            "URI as parameter value needs special handling"
        );
    }
    ...
    fn test_rfc_examples() {
        ...
        assert!(
            result2.is_ok(),
            "RFC example with parameter should parse successfully"
        );
        ...
        let input3 =
            "Error-Info: <sip:not-in-service@example.com>, <http://errors.example.com/busy.html>";
    ...
    fn test_abnf_compliance() {
        ...
        for case in &invalid_cases {
            ...
            assert!(
                result.is_err(),
                "Invalid ABNF case should be rejected: {:?}",
                case
            );
        }
    }
