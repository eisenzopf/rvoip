...
mod tests {
    ...
    fn test_parse_www_authenticate_other() {
        ...
        if let Challenge::Other { scheme, params } = &challenges[0] {
            ...
            assert!(params.contains(&AuthParam {
                name: "realm".to_string(),
                value: "apps.example.com".to_string()
            }));
            assert!(params.contains(&AuthParam {
                name: "type".to_string(),
                value: "1".to_string()
            ...
            assert!(params.contains(&AuthParam {
                ...
                value: "Login Required".to_string()
            }));
        } else {
    ...
    fn test_parse_www_authenticate_with_domain() {
        let input =
            br#"Digest realm="atlanta.com", nonce="8452cd5a", domain="sip:ss1.example.com""#;
        let result = parse_www_authenticate(input);
        assert!(result.is_ok());
        let (rem, challenges) = result.unwrap();
        assert!(rem.is_empty());
    ...
    fn test_malformed_parameter() {
        ...
        assert!(
            result.is_ok(),
            "Parser should be lenient with malformed parameters"
        );
