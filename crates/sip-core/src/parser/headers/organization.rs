...
pub fn parse_organization(input: &[u8]) -> ParseResult<Organization> {
    ...
    if let Some(pos) = input.iter().position(|&b| b == b';') {
        ...
        let (_, org) = delimited(
            sws,
            map(
                opt(map_res(text_utf8_trim, |bytes| {
                    // Apply line unfolding (returns Vec<u8>)
                    let unfolded = unfold_lws(bytes);
                    // Convert to UTF-8 string
                    let s = String::from_utf8(unfolded).map_err(|_| {
                        nom::Err::Error(nom::error::Error::new(bytes, nom::error::ErrorKind::Char))
                    })?;
...
mod tests {
    ...
    fn test_parse_organization_empty() {
        ...
        for test_case in &complex_whitespace_tests {
            ...
            assert!(
                rem.is_empty(),
                "Failed to consume all input for {:?}",
                test_case
            );
            assert_eq!(
                val,
                Organization::new(""),
                "Whitespace input not treated as empty Organization for {:?}",
                test_case
            );
        }
    }
