...
pub fn parse_contact(input: &[u8]) -> ParseResult<ContactValue> {
    alt((
        ...
        map(comma_separated_list1(contact_param), |params| {
...
mod tests {
    ...
    fn test_parse_contact_single_name_addr_params() {
        ...
        if let ContactValue::Params(params) = val {
            assert_eq!(params.len(), 1);
            assert_eq!(
                params[0].address.display_name,
                Some("Mr. Watson".to_string())
            );
            assert_eq!(params[0].address.uri.scheme, Scheme::Sip);
            ...
            assert!(params[0]
                .address
                .params
                .contains(&Param::Q(NotNan::new(0.7).unwrap())));
    ...
    fn test_parse_contact_multiple() {
        ...
        if let ContactValue::Params(params) = val {
            ...
            assert!(params[1]
                .address
                .params
                .contains(&Param::Tag("123".to_string())));
        } else {
            panic!("Expected Params variant");
        }
    }
    ...
    fn test_parse_contact_with_generic_params() {
        ...
        if let ContactValue::Params(params) = val {
            ...
            let has_methods = params[0]
                .address
                .params
                .iter()
                .any(|p| matches!(p, Param::Other(ref name, Some(_)) if name == "methods"));
            assert!(has_methods, "Should have methods parameter");

            // Check for the unknown-param
            let has_unknown =
                params[0].address.params.iter().any(
                    |p| matches!(p, Param::Other(ref name, Some(_)) if name == "unknown-param"),
                );
    ...
    fn test_parse_contact_case_insensitive_params() {
        ...
        if let ContactValue::Params(params) = val {
            ...
            assert!(params[0]
                .address
                .params
                .contains(&Param::Q(NotNan::new(0.5).unwrap())));
    ...
    fn test_parse_contact_with_multiple_params() {
        ...
        if let ContactValue::Params(params) = val {
            ...
            assert!(
                params[0].address.params.len() >= 3,
                "Should have at least 3 parameters, found {}",
                params[0].address.params.len()
            );

            // Check q parameter
            let has_q = params[0].address.params.iter().any(|p| match p {
            ...
            let has_expires = params[0].address.params.iter().any(|p| match p {
            ...
            // Check tag parameter
            let has_tag = params[0].address.params.iter().any(|p| match p {
            ...
            let has_sip_instance =
                params[0].address.params.iter().any(
                    |p| matches!(p, Param::Other(ref name, Some(_)) if name == "+sip.instance"),
                );
    ...
    fn test_parse_contact_with_sips_uri() {
        ...
        if let ContactValue::Params(params) = val {
            ...
            let has_transport = params[0]
                .address
                .uri
                .parameters
                .iter()
                .any(|p| matches!(p, Param::Transport(ref t) if t == "tls"));
            assert!(
                has_transport,
                "SIPS URI should have transport=tls parameter"
            );
        } else {
            panic!("Expected Params variant");
        }
    ...
    fn test_parse_contact_with_uri_params() {
        ...
        if let ContactValue::Params(params) = val {
            ...
            let has_transport = params[0]
                ...
                .parameters
                .iter()
                .any(|p| matches!(p, Param::Transport(ref t) if t == "tcp"));
    ...
    fn test_parse_contact_invalid_q_value() {
        ...
        if let Ok((_, ContactValue::Params(params))) = parse_contact(input) {
            if let Some(Param::Q(q)) = params[0]
                .address
                .params
                .iter()
                .find(|p| matches!(p, Param::Q(_)))
            {
    ...
    fn test_parse_contact_malformed_params() {
        ...
        if result.is_ok() {
            ...
            if let ContactValue::Params(params) = val {
                ...
                let no_q_param = !params[0]
                    .address
                    .params
                    .iter()
                    .any(|p| matches!(p, Param::Q(_)));
                assert!(no_q_param, "Should not parse invalid q value as Param::Q");

                // It should be parsed as a generic parameter
                let generic_q = params[0]
                    .address
                    .params
                    .iter()
                    .any(|p| matches!(p, Param::Other(name, _) if name == "q"));
