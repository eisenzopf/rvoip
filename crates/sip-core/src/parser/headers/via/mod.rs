...
use crate::types::via::{SentProtocol, ViaHeader}; // Use the main Param enum
...
fn sent_protocol(input: &[u8]) -> ParseResult<SentProtocol> {
    map_res(
        ...
        |(name_b, ver_b_opt, tran_b)| {
            ...
            Ok::<_, nom::Err<NomError<&[u8]>>>(SentProtocol {
                name,
                version,
                transport,
            })
...
mod tests {
    ...
    fn test_sent_by() {
        ...
        assert!(rem.is_empty());
        assert!(
            matches!(host, Host::Address(addr) if addr == IpAddr::from(Ipv4Addr::new(192, 0, 2, 1)))
        );
        assert_eq!(port, None);
    ...
    fn test_via_params() {
        ...
        assert!(
            matches!(p_ext, Param::Other(n, Some(GenericValue::Token(v))) if n == "custom" && v == "value")
        );
        ...
        assert!(
            matches!(p_quote, Param::Other(n, Some(GenericValue::Quoted(v))) if n == "comment" && v == "test value")
        );
    ...
    fn test_via_parm_complex() {
        let input =
            b"SIP/2.0/TCP client.biloxi.com:5060;branch=z9hG4bK74bf9;received=192.0.2.4;ttl=64";
        let result = via_param_parser(input);
        assert!(result.is_ok());
        ...
        assert!(via
            .params
            .contains(&Param::Branch("z9hG4bK74bf9".to_string())));
        assert!(via
            .params
            .contains(&Param::Received(Ipv4Addr::new(192, 0, 2, 4).into())));
    ...
    fn test_via_parm_with_rport() {
        ...
        let has_rport_value = via
            .params
            .iter()
            .any(|p| matches!(p, Param::Rport(Some(v)) if *v == 5060));
        assert!(
            has_rport_value,
            "Should have rport parameter with value 5060"
        );
    }
    ...
    fn test_via_parameter_handling() {
        ...
        assert!(via
            .params
            .iter()
            .any(|p| matches!(p, Param::Branch(s) if s == "z9hG4bK-524287-1---a6b23e33cdfc7905")));
        assert!(via.params.iter().any(|p| matches!(p, Param::Rport(None))));
        assert!(via
            .params
            .iter()
            .any(|p| matches!(p, Param::Received(ip) if *ip == Ipv4Addr::new(192, 168, 1, 100))));
        assert!(via
            .params
            .iter()
            .any(|p| matches!(p, Param::Other(s, None) if s == "test")));
    ...
    fn test_edge_cases() {
        ...
        assert!(vias[0]
            .params
            .iter()
            .any(|p| matches!(p, Param::Branch(s) if s == "z9hG4bK123")));
    ...
    fn test_via_rport_parameter() {
        ...
        let rport_param = via
            .params
            .iter()
            .find(|p| matches!(p, Param::Rport(Some(v)) if *v == 5060));
        assert!(rport_param.is_some(), "Should have an rport parameter");
        if let Some(Param::Rport(port)) = rport_param {
