...
fn m_parameter(input: &[u8]) -> ParseResult<(String, String)> {
    map_res(
        ...
        |(attr_bytes, value_string)| {
            str::from_utf8(attr_bytes).map(|attr_str| (attr_str.to_lowercase(), value_string))
        },
    )(input)
}
...
pub fn media_params_to_hashmap(
    params_b: Vec<(&[u8], &[u8])>,
) -> Result<HashMap<String, String>, std::str::Utf8Error> {
    params_b
        .into_iter()
        .map(|(attr_b, val_b)| {
...
mod tests {
    ...
    fn test_rfc_defined_types() {
        ...
        let test_types = [
            ...
            (
                b"multipart/alternative".as_ref(),
                "multipart",
                "alternative",
            ),
    ...
    fn test_quoted_string_handling() {
        ...
        let (_, mt) = media_type(input).unwrap();
        assert_eq!(
            mt.parameters.get("key"),
            Some(&"quoted \"value\" with \\backslashes".to_string())
        );
