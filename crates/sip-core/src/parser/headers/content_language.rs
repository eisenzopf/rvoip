...
fn parse_language_tag(input: &[u8]) -> IResult<&[u8], LanguageTag> {
    ...
    let (input, parts) =
        separated_list1(tag(b"-"), take_while1(|c: u8| c.is_ascii_alphanumeric()))(input)?;
...
mod tests {
    ...
    fn test_language_tag() {
        ...
        assert_eq!(
            tag_max, "abcdefgh-abcdefgh",
            "Should handle maximum allowed length"
        );

        // Error cases
        assert!(
            language_tag(b"toolongprimarytag").is_err(),
            "Primary tag too long (>8 chars)"
        );
        assert!(
            language_tag(b"en-toolongsubtag12").is_err(),
            "Subtag too long (>8 chars)"
        );
        assert!(language_tag(b"en-").is_err(), "Dangling hyphen not allowed");
        assert!(language_tag(b"-en").is_err(), "Leading hyphen not allowed");
        assert!(
            language_tag(b"en_us").is_err(),
            "Underscores not allowed per RFC 5646"
        );
        assert!(
            language_tag(b"1en").is_err(),
    ...
    fn test_rfc_examples() {
        ...
        for example in rfc_examples {
            ...
            assert!(
                result.is_ok(),
                "RFC example '{}' should parse successfully",
                example
            );
    ...
    fn test_malformed_inputs() {
        ...
        assert!(
            parse_content_language(b"Content-Language:").is_err(),
        ...
        assert!(
            parse_content_language(b"Content-Language: en,,fr").is_err(),
            "Empty elements should be rejected"
        );
        assert!(
            parse_content_language(b"Content-Language: en, ,fr").is_err(),
        ...
        assert!(
            parse_content_language(b"Content-Language: en,").is_err(),
            "Trailing comma should be rejected"
        );

        // Incorrect header
        assert!(
            parse_content_language(b"Content-Languages: en").is_err(),
    ...
    fn test_line_folding() {
        ...
        assert!(
            tag_result.is_err(),
            "Line folding within a language tag should be rejected"
        );
    ...
    fn test_extended_language_tag_validation() {
        ...
        assert!(
            digit_result.is_ok(),
            "4-digit subtag in non-script position should be valid"
        );
        ...
        assert!(
            invalid_script.is_err(),
            "Script tag with digit should be rejected"
        );
    }
}
