    ...
    alt((
        map(
            preceded(
                ...
                alt((
                    map_res(tag_no_case("icon"), |_| {
                        Ok::<InfoPurpose, nom::error::Error<&[u8]>>(InfoPurpose::Icon)
                    }),
                    map_res(tag_no_case("info"), |_| {
                        Ok::<InfoPurpose, nom::error::Error<&[u8]>>(InfoPurpose::Info)
                    }),
                    map_res(tag_no_case("card"), |_| {
                        Ok::<InfoPurpose, nom::error::Error<&[u8]>>(InfoPurpose::Card)
                    }),
                    map_res(token, |bytes| match str::from_utf8(bytes) {
                        Ok(purpose_str) => Ok(InfoPurpose::Other(purpose_str.to_string())),
                        Err(_) => Err(nom::error::Error::from_error_kind(
                            bytes,
...
fn convert_info_param_to_param(info_param: InfoParam) -> Param {
    match info_param {
        ...
        InfoParam::Purpose(purpose) => {
            ...
            Param::Other(
                "purpose".to_string(),
                Some(crate::types::param::GenericValue::Token(value_str)),
            )
        }
    }
}
...
fn info(input: &[u8]) -> ParseResult<CallInfoValue> {
    map(
        ...
        terminated(
            pair(
                delimited(laquot, take_while1(|c| c != b'>'), raquot),
...
mod tests {
    ...
    fn test_info_param() {
        ...
        assert!(
            matches!(param_g, InfoParam::Generic(Param::Other(n, Some(GenericValue::Token(v)))) if n=="random" && v=="xyz")
        );
    ...
    fn test_info_param_conversion() {
        ...
        assert!(
            matches!(param, Param::Other(n, Some(GenericValue::Token(v))) if n=="purpose" && v=="icon")
        );
        ...
        let param = convert_info_param_to_param(info_param);
        assert!(
            matches!(param, Param::Other(n, Some(GenericValue::Token(v))) if n=="purpose" && v=="custom")
        );
    ...
    fn test_parse_call_info_different_schemes() {
        ...
        for input in http_inputs.iter() {
            ...
            match &infos[0].uri.raw_uri {
                Some(raw) => {
                    ...
                    assert_eq!(
                        raw, original_uri,
                        "Raw URI not preserved correctly for {}",
                        original_uri
                    );
                }
                None => {
        ...
        match &infos[0].uri.raw_uri {
            Some(raw_uri) => {
                assert_eq!(
                    raw_uri, "sip:alice@example.com",
                    "SIP URI not preserved correctly"
                );
            }
            None => {
    ...
    fn test_parse_call_info_complex() {
        ...
        let whitespace_inputs = [
            b"<http://example.com>;purpose=icon,<http://another.example.com>;purpose=card"
                .as_slice(),
            b"<http://example.com>;purpose=icon, <http://another.example.com>;purpose=card"
                .as_slice(),
            b"<http://example.com>;purpose=icon,  <http://another.example.com>;purpose=card"
            ...
            b"<http://example.com>;purpose=icon,\r\n <http://another.example.com>;purpose=card"
                .as_slice(),
            b"<http://example.com>;purpose=icon,\r\n\t<http://another.example.com>;purpose=card"
                .as_slice(),
        ...
        for input in whitespace_inputs.iter() {
            ...
            assert!(
                result.is_ok(),
                "Failed to parse with whitespace variation: {}",
                String::from_utf8_lossy(input)
            );
            let (rem, infos) = result.unwrap();
            assert!(
                rem.is_empty(),
                "Non-empty remainder with whitespace variation: '{}'",
                String::from_utf8_lossy(rem)
            );
            assert_eq!(
                infos.len(),
                2,
                "Expected 2 info values with whitespace variation"
            );
        }
    }
    ...
    fn test_parse_call_info_header() {
        ...
        assert_eq!(
            CallInfo::header_name(),
            crate::types::header::HeaderName::CallInfo
        );
    ...
    fn test_call_info_with_complex_uris() {
        ...
        for input in inputs {
            ...
            assert!(
                rem.is_empty(),
                "Remaining input for complex URI '{}': {:?}",
                input,
                rem
            );
            assert_eq!(info_values.len(), 1);
    ...
    fn test_uri_whitespace_handling() {
        ...
        for input in inputs {
            ...
            assert!(
                result.is_ok(),
                "Failed to parse URI with whitespace: {}",
                input
            );
            let (rem, info_values) = result.unwrap();
            assert!(
                rem.is_empty(),
                "Remaining input for URI with whitespace '{}': {:?}",
                input,
                rem
            );
