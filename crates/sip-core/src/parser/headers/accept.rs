    ...
    alt((
        ...
        map_res(
            ...
            |bytes| {
                ...
                let val = s.parse::<f32>().map_err(|_| {
                    nom::Err::Error(nom::error::Error::new(bytes, ErrorKind::Float))
                })?;
                NotNan::new(val)
...
fn accept_range(input: &[u8]) -> ParseResult<(String, String, Vec<Param>)> {
    map(
        ...
        |((m_type, m_subtype), params)| {
            ...
            let subtype_str = std::str::from_utf8(m_subtype)
                .unwrap_or_default()
                .to_string();
...
mod tests {
    ...
    fn test_parse_accept() {
        ...
        let (rem, parsed) =
            comma_separated_list0(accept_range)(b"application/json, text/html;q=0.8, */*;q=0.1")
                .unwrap();
        assert!(rem.is_empty());

        let values: Vec<AcceptValue> = parsed
            .into_iter()
        ...
        assert_eq!(
            accept.0[0].m_type, "application",
            "application/json should be first with implicit q=1.0"
        );
        assert_eq!(accept.0[0].m_subtype, "json");

        assert_eq!(
            accept.0[1].m_type, "text",
            "text/html should be second with q=0.8"
        );
    ...
    fn test_sort_by_qvalue() {
        ...
        assert_eq!(
            accept.0[0].m_type, "g",
            "First item should be g/h with q=1.0"
        );
        assert_eq!(
            accept.0[1].m_type, "c",
            "Second item should be c/d with q=0.8"
        );
        assert_eq!(
            accept.0[2].m_type, "e",
            "Third item should be e/f with q=0.5"
        );
        assert_eq!(
            accept.0[3].m_type, "a",
            "Fourth item should be a/b with q=0.2"
        );
