    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        for param in &self.params {
            ...
            match param {
                ...
                Param::Other(name, Some(GenericValue::Quoted(val))) => {
                    write!(f, "{}=\"{}\"", name, val)?
                }
    ...
    alt((
        ...
        map_res(
            ...
            |bytes: &[u8]| {
                ...
                if primary.is_empty()
                    || primary.len() > 8
                    || !primary.iter().all(|&c| c.is_ascii_alphabetic())
                {
                    return Err(nom::Err::Error(nom::error::Error::new(
                        bytes,
                        nom::error::ErrorKind::Alpha,
                ...
                for part in parts.iter().skip(1) {
                    if part.is_empty()
                        || part.len() > 8
                        || !part.iter().all(|&c| c.is_ascii_alphanumeric())
                    {
                        return Err(nom::Err::Error(nom::error::Error::new(
                            bytes,
                            nom::error::ErrorKind::Alpha,
...
mod tests {
    ...
    fn test_language_range() {
        ...
        assert_eq!(
            range_alphanum, "en-gb2",
            "Should accept alphanumeric subtags"
        );

        // Invalid cases
        assert!(
            language_range(b"1234").is_err(),
            "Should reject non-alphabetic primary tags"
        );

        // Underscore handling check (RFC only allows hyphens)
        let underscore_result = language_range(b"en_us");
        assert!(
            underscore_result.is_err(),
            "Should reject underscores as per RFC"
        );
        ...
        assert!(
            long_tag_result.is_ok(),
            "Parser should succeed with a valid prefix"
        );
        ...
        assert_eq!(
            remainder,
            &b"i"[..],
            "Should leave the 9th character as remainder"
        );
        ...
        assert!(
            long_subtag_result.is_ok(),
            "Parser should succeed with a valid prefix"
        );
        let (remainder, value) = long_subtag_result.unwrap();
        assert_eq!(
            value, "en-abcdefgh",
            "Should only take a valid subtag length"
        );
        assert_eq!(
            remainder,
            &b"i"[..],
            "Should leave the extra character as remainder"
        );
        ...
        assert_eq!(
            grand_tag, "i-navajo",
            "Should accept RFC 5646 grandfathered tags"
        );
        ...
        assert_eq!(
            complex_value, "zh-yue-hk",
            "Should handle extended language subtags"
        );
    ...
    fn test_qvalue_validation() {
        // Valid q-values
        assert_eq!(
            validate_qvalue(NotNan::new(0.0).unwrap()),
            Some(NotNan::new(0.0).unwrap())
        );
        assert_eq!(
            validate_qvalue(NotNan::new(1.0).unwrap()),
            Some(NotNan::new(1.0).unwrap())
        );
        assert_eq!(
            validate_qvalue(NotNan::new(0.5).unwrap()),
    ...
    fn test_rfc5646_extensions() {
        ...
        assert_eq!(
            region_alpha_value, "en-us",
            "Should handle region subtags (alpha)"
        );
        ...
        assert_eq!(
            variant_value, "sl-rozaj-biske",
            "Should handle variant subtags"
        );
    ...
    fn test_language() {
        ...
        assert_eq!(
            lang_invalid_q.q, None,
            "Invalid q-value should be treated as None"
        );
        ...
        assert_eq!(
            lang_upper.range, "en-gb",
            "Language range should be lowercase"
        );
        assert_eq!(
            lang_upper.q,
            Some(NotNan::new(0.8).unwrap()),
            "Q param should be case insensitive"
        );
    }
    ...
    fn test_language_sorting() {
        ...
        assert!(
            lang1.language_equals("EN"),
            "Should match case-insensitively"
        );
        assert!(
            !lang1.language_equals("fr"),
    ...
    fn test_parse_accept_language() {
        ...
        assert!(
            result_with_case.is_ok(),
            "Case insensitive header parsing should work"
        );
    ...
    fn test_rfc_examples() {
        ...
        assert_eq!(
            languages2[0].range, "en-us",
            "Default q=1.0 should be first"
        );
        ...
        assert_eq!(
            languages_ws[0].range, "da",
            "Should handle extra whitespace"
        );
        assert_eq!(
            languages_ws[1].range, "en-gb",
            "Should handle extra whitespace"
        );
        assert_eq!(
            languages_ws[2].range, "en",
            "Should handle extra whitespace"
        );
    }
    ...
    fn test_complex_language_tags() {
        ...
        for tag in complex_tags.iter() {
            ...
            assert!(result.is_ok(), "Complex tag {:?} should be parseable", tag);
        }
    }
    ...
    fn test_edge_cases() {
        ...
        assert_eq!(
            languages_empty.len(),
            0,
            "Empty Accept-Language should return empty vec"
        );
        ...
        for case in invalid_cases.iter() {
            ...
            match language(case.as_bytes()) {
                            ...
                            || case.starts_with('3')) =>
                {
                    ...
                    panic!(
                        "Invalid case {:?} should be rejected: numeric primary tag",
                        case
                    );
                }
                Ok(_) if case.contains('_') => {
                    ...
                    panic!(
                        "Invalid case {:?} should be rejected: contains underscore",
                        case
                    );
                }
                Err(_) => {
    ...
    fn test_multiple_subtags_rfc5646() {
        ...
        assert_eq!(
            ...
            "Should handle all valid subtags"
        );
        ...
        assert_eq!(
            tag_variant, "de-latn-de-1901-x-private",
            "Should handle variant subtags"
        );
    }
}
