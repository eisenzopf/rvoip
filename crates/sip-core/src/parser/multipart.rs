...
fn parse_header_line(input: &[u8]) -> IResult<&[u8], (HeaderName, Vec<u8>)> {
    ...
    if let Some(colon_pos) = line.iter().position(|&b| b == b':') {
        ...
        match str::from_utf8(name_bytes) {
            ...
            Err(_) => {
                let header_name =
                    HeaderName::Other(String::from_utf8_lossy(name_bytes).into_owned());
...
fn parse_part(input: &[u8]) -> IResult<&[u8], MimePart> {
...
fn parse_boundary_delimiter<'a>(input: &'a [u8], boundary: &str) -> IResult<&'a [u8], bool> {
    ...
    if input.len() >= end_boundary.len() && &input[..end_boundary.len()] == end_boundary.as_bytes()
    {
        ...
        let (input, _) = alt((parse_crlf, tag(b"\n"), eof))(input)?;

        return Ok((input, true));
    }

    // Try to match normal boundary
    if input.len() >= dash_boundary.len()
        && &input[..dash_boundary.len()] == dash_boundary.as_bytes()
    {
    ...
    while pos < input.len() {
        ...
        if let Some(idx) = find_subsequence(&input[pos..], dash_boundary_bytes) {
            ...
            if is_start_of_line {
                ...
                let is_end_boundary = remaining_after_boundary + 2 <= input.len()
        ...
        Some((pos, is_end)) => {
            if is_end {
                ...
                if let Ok((remaining, _)) =
                    parse_boundary_delimiter(&current_input[pos..], boundary)
                {
                ...
                match find_next_boundary(current_input, boundary) {
                            ...
                            Err(_) => {
                                ...
                                if current_input.len() > skip_len {
                                    ...
                                    if is_end_boundary {
                                        ...
                                        if !current_input.is_empty() {
                                            body.epilogue =
                                                Some(Bytes::copy_from_slice(current_input));
                                        }
                                        break;
                                    }
                                } else {
                    ...
                    None => {
                        ...
                        if !current_input.is_empty() {
                            ...
                            part.raw_content = Bytes::copy_from_slice(trimmed_content);
                            part.parsed_content =
                                parse_part_content(&part.headers, &part.raw_content);
                            body.add_part(part);
                        }
            ...
            Err(_) => {
                ...
                match find_next_boundary(current_input, boundary) {
                    Some((pos, is_end_boundary)) => {
                        ...
                        if let Ok((input, is_end)) =
                            parse_boundary_delimiter(current_input, boundary)
                        {
                            current_input = input;
...
pub fn parse_multipart(content: &[u8], boundary: &str) -> Result<MultipartBody> {
    ...
    if boundary.is_empty() || boundary.len() > 70 {
        return Err(Error::ParseError(format!(
            "Invalid boundary length: {} (must be 1-70 characters)",
            boundary.len()
        )));
    }
    ...
    for c in boundary.chars() {
        if c.is_control() && c != '\t' {
            return Err(Error::ParseError(format!(
                "Invalid boundary: contains control character: U+{:04X}",
                c as u32
            )));
        }
    }
    ...
    match multipart_parser(content, boundary) {
        ...
        Err(nom::Err::Error(e)) | Err(nom::Err::Failure(e)) => {
            ...
            Err(Error::ParseError(format!(
                "Failed to parse multipart body near offset {}: {:?}",
                offset, e.code
            )))
        }
        Err(nom::Err::Incomplete(needed)) => Err(Error::ParseError(format!(
            "Incomplete multipart body: Needed {:?}",
            needed
        ))),
    }
}
