...
- Add to `[workspace.dependencies]`:
  ```
  rvoip-uctp           = { path = "crates/uctp/rvoip-uctp",           version = "0.1.26" }
  rvoip-quic           = { path = "crates/uctp/rvoip-quic",           version = "0.1.26" }
  rvoip-webtransport   = { path = "crates/uctp/rvoip-webtransport",   version = "0.1.26" }
  rvoip-auth-core      = { path = "crates/identity/auth-core",            version = "0.1.0"  } # if not already present
  rvoip-media-core     = { path = "crates/media/media-core",            version = "0.1.0"  } # if not already present; used by Phase 4 bridge demo for transcoding
  quinn                = { version = "0.11", default-features = false, features = ["runtime-tokio", "rustls-ring"] }
  web-transport-quinn  = "0.5"
  tokio-util           = { version = "0.7", features = ["codec", "compat"] }
  ```
  Existing workspace entries we reuse: `rustls`, `rustls-pemfile`, `rcgen`, `tokio`, `bytes`, `serde`, `serde_json`, `thiserror`, `async-trait`, `dashmap`, `tracing`, `uuid`, `chrono`, `futures`, `parking_lot`. **Confirmed at plan time:** `rvoip-media-core` is already a workspace.dependencies entry; `rvoip-auth-core` is **not** and must be added in this PR. Both crates exist on disk under `crates/identity/auth-core/` and `crates/media/media-core/`.
...
Separate PR against `rvoip-core` (lands before Phase 1 starts). Edits `crates/foundation/rvoip-core/src/capability.rs`:
...
This is the only rvoip-core source change required to start Phase 1. `IdentityAssurance` already exists in rvoip-core (`crates/foundation/rvoip-core/src/identity.rs:51-70`) with variants `Anonymous` / `Pseudonymous { ephemeral_key: Jwk }` / `Identified { credential_kind }` / `TaskScoped { ... }` / `UserAuthorized { ... }` — no changes to that enum needed.
...
```rust
// crates/foundation/rvoip-core/src/recording.rs (or wherever RecordingComplete lives)
...
Also: `crates/identity/auth-core/src/lib.rs` re-exports `BearerValidator`, `AuthError`, and `bearer_stub`. The `AuthError` type is what `rvoip_uctp::UctpError::Auth(#[from] rvoip_auth_core::AuthError)` consumes (§3.2.1).

**No changes** to the existing `DigestAuthenticator` — bearer validation is purely additive.

### 2.6 Per-crate `Cargo.toml` skeleton

**State today:** `crates/uctp/rvoip-uctp/` exists but contains only this plan file — no `src/`, no `Cargo.toml`. `crates/uctp/rvoip-quic/` and `crates/uctp/rvoip-webtransport/` do not exist at all. Phase 0 creates all three directories, their `src/lib.rs` stubs, and the `Cargo.toml` skeletons below.
...
`auth-core`'s workspace dependency key is `rvoip-auth-core` to match the existing convention (`crates/identity/auth-core/Cargo.toml` ships as `package = "rvoip-auth-core"`). Verify at implementation time.

### 3.2 Module layout

```
crates/uctp/rvoip-uctp/src/
...
`Pseudonymous` carries `ephemeral_key: Jwk` in rvoip-core's actual enum (see `crates/foundation/rvoip-core/src/identity.rs:51-70`); the stub generates one per-token rather than fabricating a permanent key.

**`UctpCoordinator` design.** Per-peer driver; one instance per substrate connection. Mirrors the `SipAdapter` concurrency model (see `crates/sip/rvoip-sip/src/adapter.rs:33-43`): `&self` methods, internal `DashMap`s, per-machine `parking_lot::Mutex` around the actual state-machine instance.
...
`rvoip-sip` invests heavily in `tracing` spans plus a `StageMetrics` / `record_metrics` infrastructure (see `crates/sip/rvoip-sip/src/adapter.rs`). UCTP must reach parity so a regression in the WT path vs. the QUIC path is visible — this is load-bearing for the demo's "cross-transport bridging works" claim. Three layers:
...
Line-for-line counterpart of `rvoip-sip::SipAdapter` (see `crates/sip/rvoip-sip/src/adapter.rs:33-43`). Same `by_connection`/`by_uctp_connid` `DashMap` pattern, same `out_tx`/`out_rx` mpsc channels, same `tokio::spawn` translator task that maps `UctpSessionEvent` → `AdapterEvent`.

`AdapterEvent` variant names (per `crates/foundation/rvoip-core/src/adapter.rs:72-91`): `InboundConnection { connection }`, `Connected { connection_id }`, `Ended { connection_id, reason }`, `Failed { connection_id, detail }`, `Native { kind, detail }`. The orchestrator normalizes these into its outward-facing `Event::ConnectionInbound` / `Event::ConnectionConnected` / `Event::ConnectionEnded` / `Event::ConnectionFailed` per `INTERFACE_DESIGN.md` §5; **don't conflate the two enums** — the adapter emits `AdapterEvent::*`, the orchestrator publishes `Event::*`.
...
**`ConnectionAdapter` method split for v0** (full trait surface is in `crates/foundation/rvoip-core/src/adapter.rs:97-126`):
...
```
crates/uctp/rvoip-webtransport/src/
  lib.rs
...
`rvoip-media-core` is the existing crate at `crates/media/media-core/`. The bridge frame-pump (manual in v0, replaced by `Orchestrator::bridge_connections` in v0.x — see §7) calls into `rvoip_media_core::codec::transcoding::Transcoder` when the two Connections' negotiated codecs differ. The actual signature today is:

```rust
// crates/media/media-core/src/codec/transcoding.rs
...
### 6.2 The demo: `crates/uctp/rvoip-uctp/examples/uctp_to_sip_bridge/`

Mirrors `crates/foundation/rvoip-core/examples/sip_only_orchestrator.rs` and extends it.
...
**Cargo discovery.** A subdir of `examples/` with multiple bare `.rs` files is **not** a layout `cargo run --example` recognizes — Cargo looks for either `examples/<name>.rs` or `examples/<name>/main.rs`. The plan commits to **explicit `[[example]]` entries**, which keeps the subdir grouping while allowing `cargo run --example <name>` to work. Add to `crates/uctp/rvoip-uctp/Cargo.toml`:
...
5. `orch.register(Arc::new(quic_adapter))?; orch.register(Arc::new(wt_adapter))?; orch.register(Arc::new(sip_adapter))?;` — `Orchestrator::register` takes `Arc<dyn ConnectionAdapter>` (per `crates/foundation/rvoip-core/src/orchestrator.rs:90`); each adapter struct is wrapped at the call site.
...
`crates/uctp/rvoip-uctp/tests/bridge_smoke.rs`:
...
| Template example to mirror | `crates/foundation/rvoip-core/examples/sip_only_orchestrator.rs` |
...
| # | PR | Touches | Blocks |
|---|---|---|---|
| 0a | Workspace + crate scaffolding | Root `Cargo.toml` (§2.1), `crates/uctp/rvoip-uctp/` `Cargo.toml` + `src/lib.rs` stub, new `crates/uctp/rvoip-quic/` and `crates/uctp/rvoip-webtransport/` directories with `Cargo.toml` + `src/lib.rs` stubs (§2.6). Adds `rvoip-auth-core` and `metrics` to `[workspace.dependencies]`. | All later PRs |
| 0b | `rvoip-core::CapabilityDescriptor` extension | `crates/foundation/rvoip-core/src/capability.rs` — seven new fields + four enums (§2.3, §3.4); converts `supports_dtmf_rfc4733` from field to method (§3.4); spec-§8 JSON round-trip test. | Phase 1 (needed for negotiation) |
| 0c | `rvoip-core::VconRef` placeholder | New `crates/foundation/rvoip-core/src/recording.rs` (or extends existing recording types) with `VconRef` enum + `Option<VconRef>` field on `RecordingComplete` (§2.4). | Phase 1 (needed for `recording.complete` envelope payload) |
| 0d | `auth-core::BearerValidator` | New `crates/identity/auth-core/src/bearer.rs` (§2.5); `lib.rs` re-exports. | Phase 1 (needed for `UctpCoordinator` to compile) |
...
Integration test [`crates/uctp/rvoip-uctp/tests/jwt_integration.rs`](tests/jwt_integration.rs) drives the coordinator A1 gate with real JWTs end-to-end.
