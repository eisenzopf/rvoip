...
fn parse_connection_address_nom<'a>(
    input: &'a str,
    addr_type: &'a str,
) -> IResult<&'a str, (String, Option<u8>, Option<u32>)> {
    if addr_type == "IP4" {
        ...
        let (input, ttl) = if let Some(pos) = input.find('/') {
            ...
            if input_after_slash.is_empty()
                || !input_after_slash.chars().next().unwrap().is_ascii_digit()
            {
                return Err(nom::Err::Error(nom::error::Error::new(
                    input,
                    nom::error::ErrorKind::Digit,
...
fn parse_connection_nom(input: &str) -> IResult<&str, ConnectionData> {
    ...
    let (input, (net_type, _, addr_type, _, addr_with_ttl)) = tuple((
        parse_network_type,
        space1,
        parse_address_type,
        space1,
    ...
    let (_, (address, ttl, count)) = parse_connection_address_nom(addr_with_ttl, addr_type)?;
...
pub fn parse_connection_line(value: &str) -> Result<ConnectionData> {
    ...
    let value_to_parse = if let Some(stripped) = value.strip_prefix("c=") {
        stripped
    } else {
        value
    };
    ...
    if parts.len() != 3 {
        return Err(Error::SdpParsingError(format!(
            "Connection data must have exactly 3 parts: {}",
            value
        )));
    }
    ...
    let (address, ttl, multicast_count) = if addr_part.contains('/') {
        ...
        if addr_parts.len() > 3 {
            return Err(Error::SdpParsingError(format!(
                "Invalid connection address format: {}",
                addr_part
            )));
        }
        ...
        if addr_type == "IP4" {
            if !is_valid_ipv4(&base_addr) {
                ...
                {
                    return Err(Error::SdpParsingError(format!(
                        "Invalid IPv4 address (too few segments): {}",
                        base_addr
                    )));
                }
...
fn validate_connection_data(conn_data: &ConnectionData) -> Result<()> {
    ...
    if addr_type == "IP4" {
        ...
        if looks_like_partial_ipv4 {
            return Err(Error::SdpParsingError(format!(
                "Invalid IPv4 address (incomplete format): {}",
                addr
            )));
        }
        ...
        if is_valid_ipv4(addr) {
            ...
            if conn_data.ttl.is_some() {
                ...
                if !ip.is_multicast() {
                    return Err(Error::SdpParsingError(format!(
                        "TTL provided for non-multicast address: {}",
                        addr
                    )));
                }
            }
            ...
            if conn_data.multicast_count.is_some() {
                ...
                if !ip.is_multicast() {
                    return Err(Error::SdpParsingError(format!(
                        "Multicast count provided for non-multicast address: {}",
                        addr
                    )));
                }
            }
        ...
        if is_valid_ipv6(addr) {
            ...
            if conn_data.ttl.is_some() {
                return Err(Error::SdpParsingError(format!(
                    "TTL not allowed for IPv6 address: {}",
                    addr
                )));
            }
...
mod tests {
    ...
    fn test_parse_basic_ipv4_connection() {
        ...
        assert_eq!(
            conn.connection_address, "192.168.1.1",
            "Incorrect connection address"
        );
        assert_eq!(conn.ttl, None, "TTL should be None for unicast");
        assert_eq!(
            conn.multicast_count, None,
            "Multicast count should be None for unicast"
        );
    }
    ...
    fn test_parse_ipv4_multicast() {
        ...
        let conn = result.unwrap();
        assert_eq!(
            conn.connection_address, "224.2.36.42",
            "Incorrect multicast address"
        );
    ...
    fn test_parse_ipv4_multicast_with_count() {
        ...
        assert!(
            result.is_ok(),
            "Failed to parse IPv4 multicast with TTL and count"
        );
    ...
    fn test_parse_basic_ipv6_connection() {
        ...
        assert_eq!(
            conn.connection_address, "2001:db8::1",
            "Incorrect connection address"
        );
    ...
    fn test_parse_ipv6_multicast() {
        ...
        let conn = result.unwrap();
        assert_eq!(
            conn.connection_address, "FF15::101",
            "Incorrect IPv6 multicast address"
        );
    ...
    fn test_parse_fqdn_connection() {
        ...
        let conn = result.unwrap();
        assert_eq!(
            conn.connection_address, "server.example.com",
            "Incorrect FQDN"
        );
    }
    ...
    fn test_ttl_with_non_multicast() {
        ...
        assert!(
            result.is_err(),
            "Should reject TTL with non-multicast address"
        );
    }
    ...
    fn test_invalid_multicast_count_format() {
        ...
        assert!(
            result.is_err(),
            "Should reject invalid multicast count format"
        );
    }
    ...
    fn test_too_many_parts() {
        ...
        assert!(
            result.is_err(),
            "Should reject too many parts in connection address"
        );
    }
    ...
    fn test_too_few_parts() {
        ...
        assert!(
            result.is_err(),
            "Should reject connection data with too few parts"
        );
    }
    ...
    fn test_whitespace_handling() {
        ...
        assert!(
            result.is_ok(),
            "Failed to parse connection with extra whitespace"
        );
