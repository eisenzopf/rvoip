...
fn attribute_pair_parser(input: &str) -> IResult<&str, (String, Option<String>)> {
    ...
    let (input, attr_value) = opt(preceded(
        char(':'),
        map(attribute_value_parser, |s: &str| s.to_string()),
    ))(input)?;
        ...
        Ok((_, ssrc)) => {
            ...
            if ssrc.attribute.is_empty() {
                return Err(Error::SdpParsingError(format!(
                    "Missing attribute name in ssrc: {}",
                    value
                )));
            }
...
mod tests {
    ...
    fn test_ssrc_basic_parsing() {
        ...
        assert!(
            result.is_ok(),
            "Failed to parse SSRC attribute without value"
        );
    ...
    fn test_ssrc_invalid_formats() {
        ...
        assert!(
            parse_ssrc(missing_id).is_err(),
            "Should reject missing SSRC ID"
        );

        // Missing attribute name
        let missing_attr = "12345 :value";
        assert!(
            parse_ssrc(missing_attr).is_err(),
            "Should reject missing attribute name"
        );

        // Invalid SSRC ID (not a number)
        let invalid_id = "abc cname:user@example.com";
        assert!(
            parse_ssrc(invalid_id).is_err(),
            "Should reject non-numeric SSRC ID"
        );

        // Empty attribute name
        let empty_attr = "12345 ";
        assert!(
            parse_ssrc(empty_attr).is_err(),
            "Should reject empty attribute name"
        );
    }
    ...
    fn test_ssrc_whitespace_handling() {
        ...
        assert!(
            result.is_ok(),
            "Should handle multiple spaces between parts"
        );
    }
    ...
    fn test_ssrc_edge_cases() {
        ...
        for name in valid_names.iter() {
            assert!(
                parse_ssrc(name).is_ok(),
                "Should handle valid attribute name: {}",
                name
            );
        }
