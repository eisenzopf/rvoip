...
pub fn parse_group(input: &str) -> Result<(String, Vec<String>)> {
        ...
        .all(|c| c.is_ascii_alphanumeric() || c == '-' || c == '_')
    {
        return Err(Error::SdpParsingError(
            "Invalid semantics value".to_string(),
        ));
    }
        ...
        && !parts[0].starts_with("x-")
    {
        ...
        return Err(Error::SdpParsingError(
            "Invalid semantics value, possibly misplaced identification tag".to_string(),
        ));
    }
    ...
    if parts.len() < 2 || parts[1].trim().is_empty() {
        return Err(Error::SdpParsingError(format!(
            "Group attribute must have at least one identification tag: '{}'",
            semantics
        )));
    }

    let identification_tags = parts[1].split_whitespace().map(|s| s.to_string()).collect();
...
mod tests {
    ...
    fn test_empty_identification_tags_list() {
        ...
        let no_tags = "BUNDLE";
        assert!(
            parse_group(no_tags).is_err(),
            "Parser should reject group with no identification tags"
        );
    ...
    fn test_invalid_formats() {
        ...
        assert!(
            parse_group(invalid_format).is_err(),
