...
pub fn parse_scalability_mode(
    mode: &str,
) -> Result<(String, Option<u32>, Option<u32>, Option<String>)> {
    ...
    if !['L', 'S', 'K'].contains(&pattern_char) {
        return Err(Error::SdpParsingError(format!(
            "Invalid scalability mode pattern: {}",
            pattern_char
        )));
    }
    ...
    if t_count > 1 {
        return Err(Error::SdpParsingError(
            "Multiple T markers in scalability mode".to_string(),
        ));
    }
    ...
    if let Some(pos) = t_pos {
        ...
        if !spatial_part.is_empty() {
            if !spatial_part.chars().all(|c| c.is_ascii_digit()) {
                return Err(Error::SdpParsingError(format!(
                    "Invalid spatial layer in scalability mode: {}",
                    spatial_part
                )));
            }
        ...
        if temporal_and_extra.is_empty() {
            return Err(Error::SdpParsingError(
                "Missing temporal value after T marker".to_string(),
            ));
        }
        ...
        if numeric_part.is_empty() {
            return Err(Error::SdpParsingError(
                "Missing temporal layer value".to_string(),
            ));
        }
        ...
        if rest.chars().all(|c| c.is_ascii_digit()) {
            ...
            return Err(Error::SdpParsingError(format!(
                "Invalid characters in spatial layer: {}",
                rest
            )));
        }
    }
...
mod tests {
    ...
    fn test_invalid_inputs() {
        ...
        assert!(
            parse_scalability_mode("").is_err(),
            "Empty string should be rejected"
        );

        // Test invalid pattern character - should be rejected
        assert!(
            parse_scalability_mode("X1T2").is_err(),
            "Invalid pattern character X should be rejected"
        );
        assert!(
            parse_scalability_mode("Z3").is_err(),
            "Invalid pattern character Z should be rejected"
        );

        // These should be considered invalid in a strict parser
        assert!(
            parse_scalability_mode("LaT2").is_err(),
            "Non-numeric character in spatial layer should be rejected"
        );
        assert!(
            parse_scalability_mode("L2Ta").is_err(),
            "Non-numeric character in temporal layer should be rejected"
        );
    }
    ...
    fn test_malformed_patterns() {
        ...
        assert!(
            parse_scalability_mode("L2T").is_err(),
            "Missing temporal value should be rejected"
        );

        // Multiple T characters
        assert!(
            parse_scalability_mode("L2T3T4").is_err(),
            "Multiple T characters should be rejected"
        );

        // Non-numeric characters in spatial layer
        assert!(
            parse_scalability_mode("LxT3").is_err(),
            "Non-numeric spatial layer should be rejected"
        );
    }
}
