...
fn parse_uint(input: &str) -> IResult<&str, u64> {
    map_res(recognize(preceded(opt(space0), digit1)), |s: &str| {
        s.trim().parse::<u64>()
    })(input)
}
...
pub fn parse_max_message_size(value: &str) -> Result<u64> {
    ...
    match all_consuming(parse_uint)(value.trim()) {
        Ok((_, size)) => Ok(size),
        Err(_) => Err(Error::SdpParsingError(format!(
            "Invalid max-message-size value: {}",
            value
        ))),
    }
}
...
mod tests {
    ...
    fn test_sctp_port_invalid() {
        // Non-numeric values
        assert!(
            parse_sctp_port("").is_err(),
            "Empty string should be rejected"
        );
        assert!(
            parse_sctp_port("abc").is_err(),
            "Non-numeric value should be rejected"
        );
        assert!(
            parse_sctp_port("123abc").is_err(),
            "Alpha-numeric value should be rejected"
        );

        // Out of range values
        assert!(
            parse_sctp_port("65536").is_err(),
            "Value exceeding u16::MAX should be rejected"
        );
        assert!(
            parse_sctp_port("100000").is_err(),
            "Value exceeding u16::MAX should be rejected"
        );

        // Negative values (should be caught by parser as non-numeric)
        assert!(
            parse_sctp_port("-1").is_err(),
            "Negative value should be rejected"
        );

        // Special characters
        assert!(
            parse_sctp_port("1,234").is_err(),
            "Value with comma should be rejected"
        );
        assert!(
            parse_sctp_port("1.234").is_err(),
            "Value with decimal point should be rejected"
        );
    }
    ...
    fn test_max_message_size_valid() {
        ...
        assert_eq!(
            parse_max_message_size("18446744073709551615").unwrap(),
    ...
    fn test_max_message_size_invalid() {
        // Non-numeric values
        assert!(
            parse_max_message_size("").is_err(),
            "Empty string should be rejected"
        );
        assert!(
            parse_max_message_size("abc").is_err(),
            "Non-numeric value should be rejected"
        );
        assert!(
            parse_max_message_size("1024kb").is_err(),
            "Alpha-numeric value should be rejected"
        );

        // Negative values (should be caught by parser as non-numeric)
        assert!(
            parse_max_message_size("-1024").is_err(),
            "Negative value should be rejected"
        );

        // Special characters
        assert!(
            parse_max_message_size("1,048,576").is_err(),
            "Value with commas should be rejected"
        );
        assert!(
            parse_max_message_size("1.5e6").is_err(),
