        ...
        Ok((remaining, port)) => {
            ...
            if !remaining.is_empty() {
                return Err(Error::SdpParsingError(format!(
                    "Invalid sctp-port format, trailing characters: {}",
                    value
                )));
            }
            Ok(port)
...
pub fn parse_max_message_size(value: &str) -> Result<u64> {
    ...
    match max_message_size_parser(value) {
        Ok((remaining, size)) => {
            ...
            if !remaining.is_empty() {
                return Err(Error::SdpParsingError(format!(
                    "Invalid max-message-size format, trailing characters: {}",
                    value
                )));
            }
            ...
            if size < 1 {
                return Err(Error::SdpParsingError(format!(
                    "Invalid max-message-size, must be greater than 0: {}",
                    size
                )));
            }
            Ok(size)
        ...
        Err(_) => Err(Error::SdpParsingError(format!(
            "Invalid max-message-size format: {}",
            value
        ))),
    }
}
