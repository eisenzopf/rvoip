...
mod tests {
    ...
    fn test_via_headers() {
        ...
        assert!(
            result.is_ok(),
            "Via header with port should parse successfully"
        );
        if let Ok((_, vias)) = result {
            let via = &vias[0];
        ...
        assert!(
            result.is_ok(),
            "Via header with multiple parameters should parse successfully"
        );
        if let Ok((_, vias)) = result {
            let via = &vias[0];
        ...
        assert!(
            result.is_ok(),
            "Via header with hidden parameter should parse successfully"
        );
        if let Ok((_, vias)) = result {
            let via = &vias[0];
            assert_eq!(via.branch(), Some("z9hG4bK776asdhds"));
            ...
            assert_eq!(
                via.param_value("hidden"),
                Some(None),
                "Hidden parameter should have no value"
            );
        }
        ...
        assert!(
            result.is_ok(),
            "Multiple Via headers should parse successfully"
        );
        if let Ok((_, vias)) = result {
        ...
        assert!(
            result.is_ok(),
            "Via header with IPv6 address should parse successfully"
        );
        if let Ok((_, vias)) = result {
            let via = &vias[0];
            assert!(
                matches!(via.host(), crate::types::uri::Host::Address(_)),
                "Host should be an IPv6 address"
            );
            assert_eq!(via.port(), Some(5060));
        }
    }
