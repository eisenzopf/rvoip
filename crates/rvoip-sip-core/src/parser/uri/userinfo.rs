...
pub fn userinfo(input: &[u8]) -> ParseResult<(String, Option<String>)> {
    ...
    map_res(
        terminated(pair(user, opt(preceded(tag(b":"), password))), tag(b"@")),
        |(user_bytes, pass_opt_bytes)| -> Result<(String, Option<String>), Error> {
            let user_str = unescape_uri_component(user_bytes)?;
            let pass_str_opt = pass_opt_bytes.map(unescape_uri_component).transpose()?;
...
mod tests {
    ...
    fn test_strict_rfc3261_user_unreserved() {
        ...
        for &c in &test_chars {
            let input = vec![c, b'@'];
            let (rem, (user, pass)) =
                userinfo(&input).unwrap_or_else(|_| panic!("Failed to parse char: {}", c as char));
            assert!(rem.is_empty());
    ...
    fn test_strict_rfc3261_password_special_chars() {
        ...
        for &c in &allowed_chars {
            ...
            let (rem, (user, pass)) = userinfo(&input)
                .unwrap_or_else(|_| panic!("Failed to parse allowed password char: {}", c as char));
            assert!(rem.is_empty());
            assert_eq!(user, "u");
        ...
        for &c in &disallowed_chars {
            let input = vec![b'u', b':', c, b'@'];
            assert!(
                userinfo(&input).is_err(),
                "Parser incorrectly accepted disallowed password char: {}",
                c as char
            );
        }
    }
    ...
    fn test_rfc4475_torture_tests() {
        ...
        // Example with !
        let (rem, (user, pass)) =
            userinfo(b"a!b@").unwrap_or_else(|_| panic!("Failed to parse user with ! character"));
        assert!(rem.is_empty());
        assert_eq!(user, "a!b");
        assert_eq!(pass, None);
    ...
    fn test_rfc3261_escaping_rules() {
        ...
        let (rem, (user, pass)) = userinfo(b"user%3B%3F%2F@")
            .unwrap_or_else(|_| panic!("Failed to parse escaped user-unreserved chars"));
        assert!(rem.is_empty());
        assert_eq!(user, "user;?/");
        assert_eq!(pass, None);
        ...
        let (rem, (user, pass)) = userinfo(b"first%20last:pass%22word%22@")
            .unwrap_or_else(|_| panic!("Failed to parse escaped space and quotes"));
        assert!(rem.is_empty());
    ...
    fn test_rfc5118_ipv6_ref_in_userinfo() {
        ...
        let (rem, (user, pass)) = userinfo(ipv6_userinfo)
            .unwrap_or_else(|_| panic!("Failed to parse IPv6 reference in userinfo"));
        assert!(rem.is_empty());
    ...
    fn test_rfc3261_tel_subscriber() {
        ...
        let (rem, (user, pass)) = userinfo(b"+1-212-555-0123@")
            .unwrap_or_else(|_| panic!("Failed to parse telephone-subscriber in userinfo"));
        assert!(rem.is_empty());
    ...
    fn test_escaped_separator() {
        ...
        let (rem, (user, pass)) = userinfo(b"user%3Aname@")
            .unwrap_or_else(|_| panic!("Failed to parse escaped colon in userinfo"));
        assert!(rem.is_empty());
        assert_eq!(user, "user:name");
        assert_eq!(pass, None);

        // Test that an escaped @ in user or password is not treated as end of userinfo
        let (rem, (user, pass)) = userinfo(b"user%40example.com@")
            .unwrap_or_else(|_| panic!("Failed to parse escaped @ in user"));
        assert!(rem.is_empty());
        assert_eq!(user, "user@example.com");
        assert_eq!(pass, None);

        let (rem, (user, pass)) = userinfo(b"user:pass%40word@")
            .unwrap_or_else(|_| panic!("Failed to parse escaped @ in password"));
        assert!(rem.is_empty());
        assert_eq!(user, "user");
        assert_eq!(pass, Some("pass@word".to_string()));
    }
