...
use crate::types::param::{GenericValue, Param}; // Using GenericValue now
...
fn other_param(input: &[u8]) -> ParseResult<Param> {
    ...
    map_res(
        ...
        |(name_bytes, value_opt_bytes)| -> Result<Param, Error> {
            ...
            let value_opt = value_opt_bytes.map(unescape_uri_component).transpose()?;
            ...
            Ok(Param::Other(name, value_opt.map(GenericValue::Token)))
        },
    )(input)
}
...
mod tests {
    ...
    fn test_uri_parameter_precedence() {
        ...
        assert!(rem.is_empty());
        assert!(
            matches!(param, Param::Other(n, Some(GenericValue::Token(v))) if n == "custom" && v == "value")
        );
    }
    ...
    fn test_rfc4475_case_sensitivity() {
        ...
        assert!(params1
            .iter()
            .any(|p| matches!(p, Param::Transport(v) if v == "udp")));
        assert!(params2
            .iter()
            .any(|p| matches!(p, Param::Transport(v) if v == "udp")));
        ...
        let val1 = params1.iter().find_map(|p| match p {
            Param::Other(n, Some(GenericValue::Token(v))) if n == "custom" || n == "Custom" => {
                Some(v.clone())
            }
            _ => None,
        });
    ...
    fn test_rfc5118_ipv6_params() {
        ...
        assert!(params
            .iter()
            .any(|p| matches!(p, Param::Maddr(m) if m == "[2001:db8::1]")));
        assert!(params.contains(&Param::Transport("tcp".to_string())));
    }
