...
pub fn parse_authentication_info(input: &[u8]) -> ParseResult<Vec<AuthenticationInfoParam>> {
    ...
    if !remaining.is_empty() {
        return Err(auth_info_error(
            remaining,
            AuthInfoError::UnexpectedTrailingContent,
        ));
    }
...
mod tests {
    use super::*;
    use crate::types::auth::{AuthenticationInfoParam, Qop};

    #[test]
    fn test_parse_authentication_info() {
        // Test with all possible parameters
        let input =
            br#"nextnonce="fedcba98", qop=auth, rspauth="abcdef01", cnonce="abc", nc=00000001"#;
        ...
        assert!(params.contains(&AuthenticationInfoParam::ResponseAuth(
            "abcdef01".to_string()
        )));
    ...
    fn test_parse_authentication_info_rfc_examples() {
        ...
        assert!(params.contains(&AuthenticationInfoParam::NextNonce(
            "47364c23432d2e131a5fb210812c".to_string()
        )));
        assert!(params.contains(&AuthenticationInfoParam::Qop(Qop::Auth)));
        assert!(params.contains(&AuthenticationInfoParam::ResponseAuth(
            "9fe042d5a51597f51dcd4b41749cdd7f".to_string()
        )));
    }
