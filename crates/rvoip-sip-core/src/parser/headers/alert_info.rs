...
mod tests {
    ...
    fn debug_uri_parsing() {
        ...
        match super::uri_with_generic_params(input) {
            Ok((_, (uri_str, _))) => match AlertInfoUri::from_str(&uri_str) {
                Ok(alert_uri) => {
    ...
    fn test_parse_alert_info_basic() {
        ...
        match &infos[0].uri {
            AlertInfoUri::Other { uri, .. } => {
                assert_eq!(
                    uri, "http://www.example.com/sounds/moo.wav",
                    "URI does not match expected value"
                );
            }
    ...
    fn test_parse_alert_info_multiple() {
        ...
        match &infos[0].uri {
            AlertInfoUri::Other { uri, .. } => {
                assert_eq!(
                    uri, "http://a.com/sound",
                    "First URI does not match expected value"
                );
            }
            _ => panic!("Expected HTTP URI but got a different type for first URI"),
        }
        assert!(
            infos[0].params.is_empty(),
            "First URI should not have parameters"
        );
        ...
        match &infos[1].uri {
            AlertInfoUri::Other { uri, .. } => {
                assert_eq!(
                    uri, "http://b.com/sound",
                    "Second URI does not match expected value"
                );
            }
    ...
    fn test_rfc_examples() {
        ...
        assert!(
            result2.is_ok(),
            "Failed to parse RFC-style example with parameter"
        );
