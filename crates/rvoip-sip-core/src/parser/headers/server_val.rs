...
fn product(input: &[u8]) -> ParseResult<Product> {
    map_res(
        ...
        |(name_bytes, version_opt)| {
            ...
            Ok::<Product, std::str::Utf8Error>(Product {
                name,
                version: version_opt,
            })
        },
...
pub fn server_val(input: &[u8]) -> ParseResult<ServerVal> {
    alt((
        ...
        map_res(comment, |bytes| {
            str::from_utf8(bytes).map(|s| ServerVal::Comment(s.to_string()))
        }),
    ))(input)
}
...
mod tests {
    ...
    fn test_server_val() {
        ...
        assert!(
            matches!(sv_p, ServerVal::Product(p) if p.name == "Product" && p.version == Some("V2".to_string()))
        );
