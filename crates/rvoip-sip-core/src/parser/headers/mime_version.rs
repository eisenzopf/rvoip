    ...
    delimited(
        ...
        map_res(
            ...
            |(major_bytes, minor_bytes)| {
                // Parse major version
                let major_str = str::from_utf8(major_bytes).map_err(|_| {
                    nom::Err::Failure(NomError::from_error_kind(major_bytes, ErrorKind::Char))
                })?;
                let major = major_str.parse::<u32>().map_err(|_| {
                    nom::Err::Failure(NomError::from_error_kind(major_bytes, ErrorKind::Digit))
                })?;

                // Parse minor version
                let minor_str = str::from_utf8(minor_bytes).map_err(|_| {
                    nom::Err::Failure(NomError::from_error_kind(minor_bytes, ErrorKind::Char))
                })?;
                let minor = minor_str.parse::<u32>().map_err(|_| {
                    nom::Err::Failure(NomError::from_error_kind(minor_bytes, ErrorKind::Digit))
                })?;

                // Convert u32 to u8 with error handling
                let major_u8 = u8::try_from(major).map_err(|_| {
                    nom::Err::Failure(NomError::from_error_kind(major_bytes, ErrorKind::TooLarge))
                })?;
                let minor_u8 = u8::try_from(minor).map_err(|_| {
                    nom::Err::Failure(NomError::from_error_kind(minor_bytes, ErrorKind::TooLarge))
                })?;
