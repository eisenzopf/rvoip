    ...
    map_res(
        ...
        |bytes| {
            str::from_utf8(bytes)
                ...
                .and_then(|s| {
                    Ok::<DispositionType, nom::Err<NomError<&[u8]>>>(
    ...
    preceded(
        ...
        map_res(
            ...
            |bytes| {
                str::from_utf8(bytes)
                    .map_err(|_| {
                        nom::Err::Failure(NomError::from_error_kind(bytes, ErrorKind::Char))
                    })
                    .and_then(|s| {
                        Ok::<Handling, nom::Err<NomError<&[u8]>>>(
                            match s.to_ascii_lowercase().as_str() {
...
mod tests {
    ...
    fn test_disp_param() {
        ...
        assert!(matches!(
            param_h,
            DispositionParam::Handling(Handling::Required)
        ));

        let (rem_g, param_g) = disp_param(b"filename=myfile.txt").unwrap();
        assert!(rem_g.is_empty());
        assert!(
            matches!(param_g, DispositionParam::Generic(Param::Other(n, Some(GenericValue::Token(v)))) if n == "filename" && v == "myfile.txt")
        );
    }
    ...
    fn test_parse_content_disposition_with_params() {
        ...
        assert!(params
            .iter()
            .any(|p| matches!(p, DispositionParam::Handling(Handling::Optional))));
    }
    ...
    fn test_handling_param_values() {
        ...
        assert!(params
            .iter()
            .any(|p| matches!(p, DispositionParam::Handling(Handling::Other(v)) if v == "custom")));
    }
    ...
    fn test_multiple_params_of_different_types() {
        let input =
            b"session; handling=required; filename=\"report.txt\"; size=1024; custom-param=value";
        let result = parse_content_disposition(input);
        assert!(result.is_ok());
        let (rem, (dtype, params)) = result.unwrap();
        assert!(rem.is_empty());
        ...
        assert!(params.iter().any(|p| {
            if let DispositionParam::Generic(Param::Other(name, Some(GenericValue::Token(value)))) =
                p
            {
