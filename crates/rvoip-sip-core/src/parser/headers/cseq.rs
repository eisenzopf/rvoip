...
fn cseq_value_method(input: &[u8]) -> ParseResult<(u32, Method)> {
    map_res(
        ...
        |(seq_bytes, method_bytes)| -> Result<(u32, Method), NomError<&[u8]>> {
            ...
            let seq_num = seq_str.parse::<u32>().map_err(|e| {
...
mod tests {
    ...
    fn test_whitespace_handling() {
        ...
        assert!(
            result_extra.is_ok(),
            "Should handle extra whitespace between sequence and method"
        );
    ...
    fn test_sequence_number_limits() {
        ...
        assert!(
            result_large.is_ok(),
            "Should handle maximum u32 sequence number"
        );
        ...
        assert!(
            ...
            "Should reject sequence number overflow"
        );
    ...
    fn test_abnf_edge_cases() {
        ...
        assert!(
            result_no_ws.is_ok(),
            "No whitespace before colon should be valid"
        );
        ...
        assert!(
            result_no_seq.is_err(),
            "Missing sequence number should be invalid"
        );
    }
