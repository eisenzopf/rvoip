...
fn test_parse_sip_uri() {
    ...
    assert!(
        uri.port.is_none() || uri.port == Some(0),
        "Port should be None or 0"
    );
    ...
    assert_eq!(
        ...
        "Subject header should be 'Meeting'"
    );
    assert_eq!(
        uri.headers.get("priority"),
        Some(&"urgent".to_string()),
        "Priority header should be 'urgent'"
    );
    ...
    assert_eq!(
        uri.host.to_string(),
        "192.0.2.1",
        "Host should be the IPv4 address '192.0.2.1'"
    );
    ...
    assert!(
        host_str == "[2001:db8::1]" || host_str == "2001:db8::1",
        "IPv6 host '{}' doesn't match expected format",
        host_str
    );
    ...
    // Test complex URI
    let uri_str =
        "sips:alice:secretword@example.com:5061;transport=tls?subject=Project%20X&priority=urgent";
    let uri = Uri::from_str(uri_str).expect("Failed to parse complex URI");
    assert_eq!(uri.scheme, Scheme::Sips, "URI scheme should be SIPS");
    assert_eq!(
        uri.user.as_deref(),
        Some("alice"),
        "User part should be 'alice'"
    );
    assert_eq!(
        uri.password.as_deref(),
        Some("secretword"),
        "Password should be 'secretword'"
    );
    assert_eq!(
        uri.host.to_string(),
        "example.com",
...
fn test_uri_to_string() {
    ...
    assert_eq!(
        uri.to_string(),
        uri_str,
        "Round-trip conversion failed for SIP URI"
    );
    ...
    assert_eq!(
        uri.to_string(),
        uri_str,
        "Round-trip conversion failed for SIPS URI"
    );
    ...
    assert_eq!(
        uri.to_string(),
        uri_str,
        "Round-trip conversion failed for IPv6 URI"
    );
}
...
fn test_invalid_uris() {
    ...
    assert!(
        result.is_err(),
        "URI with invalid scheme should be rejected"
    );
    ...
    assert!(
        result.is_err(),
        "URI with unclosed IPv6 bracket should be rejected"
    );
}
...
fn test_uri_parameters() {
    ...
    assert!(
        uri.parameters
            .iter()
            .any(|p| p.key() == "transport" && p.value().as_deref() == Some("tcp")),
    ...
    assert!(
        uri.parameters
            .iter()
            .any(|p| p.key() == "transport" && p.value().as_deref() == Some("tls")),
        "URI should have transport=tls parameter"
    );
    assert!(
        uri.parameters
            .iter()
            .any(|p| p.key() == "ttl" && p.value().as_deref() == Some("5")),
        "URI should have ttl=5 parameter"
    );
    assert!(
        uri.parameters
            .iter()
            .any(|p| p.key() == "method" && p.value().as_deref() == Some("INVITE")),
