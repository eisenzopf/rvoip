...
pub fn uri(uri_str: &str) -> Uri {
    Uri::from_str(uri_str)
        .unwrap_or_else(|e| panic!("Failed to parse test URI '{}': {:?}", uri_str, e))
}
...
pub fn param_tag(val: &str) -> Param {
    Param::Tag(val.to_string())
}
pub fn param_branch(val: &str) -> Param {
    Param::Branch(val.to_string())
}
pub fn param_expires(val: u32) -> Param {
    Param::Expires(val)
}
pub fn param_received(val: &str) -> Param {
    Param::Received(IpAddr::from_str(val).expect("Invalid IP address for received param"))
}
pub fn param_maddr(val: &str) -> Param {
    Param::Maddr(val.to_string())
}
pub fn param_ttl(val: u8) -> Param {
    Param::Ttl(val)
}
pub fn param_lr() -> Param {
    Param::Lr
}
pub fn param_q(val: f32) -> Param {
    Param::Q(NotNan::new(val).expect("Q value cannot be NaN"))
}
pub fn param_transport(val: &str) -> Param {
    Param::Transport(val.to_string())
}
pub fn param_user(val: &str) -> Param {
    Param::User(val.to_string())
}
pub fn param_method(val: &str) -> Param {
    Param::Method(val.to_string())
}
    ...
    T: Display + FromStr<Err = SipError> + PartialEq + Debug + Clone,
{
    ...
    match T::from_str(&displayed) {
        ...
        Err(e) => panic!(
            "Failed to parse back displayed string '{}': {:?}",
            displayed, e
        ),
    }
}
...
pub fn validate_message(
    ...
    match parse_sip_message(data) {
        Ok(message) => {
            ...
            // Check headers using HeaderAccess trait
            for (name, value) in expected_headers {
                let header_name = HeaderName::from_str(name)
                    .unwrap_or_else(|_| HeaderName::Other(name.to_string()));
                let headers = message.headers(&header_name);
                let found = headers.iter().any(|h| h.to_string().contains(value));

                if !found {
                    println!(
                        "Header '{}' with value containing '{}' not found",
                        name, value
                    );
                    return false;
                }
