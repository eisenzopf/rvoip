...
mod tests {
    ...
    fn test_transport_type_from_scheme() {
        assert_eq!(
            TransportType::from_scheme("sip").unwrap(),
            TransportType::Udp
        );
        assert_eq!(
            TransportType::from_scheme("sips").unwrap(),
            TransportType::Tls
        );
        assert_eq!(
            TransportType::from_scheme("tcp").unwrap(),
            TransportType::Tcp
        );
        assert_eq!(
            TransportType::from_scheme("udp").unwrap(),
            TransportType::Udp
        );
        assert_eq!(
            TransportType::from_scheme("tls").unwrap(),
            TransportType::Tls
        );
        assert_eq!(
            TransportType::from_scheme("ws").unwrap(),
            TransportType::WebSocket
        );
        assert_eq!(
            TransportType::from_scheme("wss").unwrap(),
            TransportType::SecureWebSocket
        );
    ...
    async fn test_factory_create_tcp_transport() {
        ...
        let bind_addr: SocketAddr = "127.0.0.1:0".parse().unwrap();
        let (transport, _rx) = factory
            .create_transport(TransportType::Tcp, bind_addr)
            .await
            .unwrap();
