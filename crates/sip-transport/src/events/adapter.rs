...
impl TransportEventAdapter {
    ...
    pub async fn publish_transport_event(&self, event: TransportEvent) -> Result<()> {
        ...
        if let Some(cross_crate_event) = self.convert_transport_to_cross_crate_event(&event) {
            ...
            {
                error!(
                    "Failed to publish cross-crate event from sip-transport: {}",
                    e
                );
            }
        }
    ...
    fn convert_transport_to_cross_crate_event(
        ...
        match event {
            ...
            TransportEvent::Error { error } => Some(RvoipCrossCrateEvent::TransportToDialog(
            ...
            TransportEvent::Closed => Some(RvoipCrossCrateEvent::TransportToDialog(
                TransportToDialogEvent::TransportError {
    ...
    fn convert_cross_crate_to_transport_event(
        ...
        match event {
            RvoipCrossCrateEvent::DialogToTransport(dialog_event) => {
                match dialog_event {
                    DialogToTransportEvent::SendSipMessage {
                        destination,
                        method,
                        headers,
                        body,
                    ...
                    DialogToTransportEvent::SendSipResponse {
                        transaction_id,
                        status_code,
                        reason_phrase,
                        headers,
                    ...
                    DialogToTransportEvent::RegisterEndpoint {
                        uri,
                        expires,
                        contact,
                    } => {
...
impl CrossCrateEventHandler for TransportCrossCrateEventHandler {
    async fn handle(&self, event: Arc<dyn CrossCrateEvent>) -> Result<()> {
        debug!(
            "Handling cross-crate event in sip-transport: {}",
            event.event_type()
        );
