...
impl CsrcManager {
    ...
    pub fn add_mapping(&mut self, mapping: CsrcMapping) {
        debug!(
            "Adding CSRC mapping: {:08x} -> {:08x}",
            mapping.original_ssrc, mapping.csrc
        );
...
impl MediaCsrcService {
    ...
    /// Add a simple SSRC to CSRC mapping
    pub async fn add_simple_mapping(
        &self,
    ...
    pub async fn remove_mapping(
        &self,
        original_ssrc: RtpSsrc,
    ) -> Result<Option<CsrcMapping>, MediaError> {
        if !self.is_enabled().await {
            return Err(MediaError::ConfigError(
                "CSRC management is not enabled".to_string(),
            ));
