...
impl HeaderExtensionManager {
    ...
    pub fn configure_extension(&mut self, id: u8, uri: String) -> Result<(), MediaError> {
        if !self.enabled {
            return Err(MediaError::ConfigError(
                "Header extensions are not enabled".to_string(),
            ));
        }
    ...
    pub fn auto_configure_extension(&mut self, uri: String) -> Result<u8, MediaError> {
        ...
        if self.next_id > max_id {
            return Err(MediaError::ConfigError(
                "No available extension IDs".to_string(),
            ));
        }
    ...
    /// Create an audio level extension
    pub fn create_audio_level_extension(
        &self,
        voice_activity: bool,
        level: u8,
    ) -> Result<HeaderExtension, MediaError> {
        let id = self
            .get_id_for_uri("urn:ietf:params:rtp-hdrext:ssrc-audio-level")
...
impl MediaHeaderExtensionService {
    ...
    pub async fn configure_extensions(
        &self,
        mappings: HashMap<u8, String>,
    ) -> Result<(), MediaError> {
        let mut manager = self.manager.write().await;
    ...
    pub async fn configure_audio_level_extension(&self, id: Option<u8>) -> Result<u8, MediaError> {
        ...
        if let Some(id) = id {
            ...
            manager
                .auto_configure_extension("urn:ietf:params:rtp-hdrext:ssrc-audio-level".to_string())
        }
    }

    /// Add header extension for a client
    pub async fn add_extension_for_client(
        &self,
        client_id: &str,
    ...
    pub async fn add_audio_level_for_client(
        &self,
        client_id: &str,
        voice_activity: bool,
        level: u8,
    ...
    pub async fn get_pending_extensions(
        &self,
        client_id: &str,
    ) -> Result<Vec<HeaderExtension>, MediaError> {
    ...
    pub async fn record_received_extension(
        &self,
        client_id: &str,
        extension: HeaderExtension,
    ) -> Result<(), MediaError> {
        let mut received = self.received_extensions.write().await;
        let client_received = received
            .entry(client_id.to_string())
            .or_insert_with(Vec::new);
    ...
    pub async fn get_received_extensions(
        &self,
        client_id: &str,
    ) -> Result<Vec<HeaderExtension>, MediaError> {
    ...
    pub async fn get_received_audio_level(
        &self,
        client_id: &str,
    ) -> Result<Option<(bool, u8)>, MediaError> {
        let received = self.received_extensions.read().await;
...
mod tests {
    ...
    fn test_header_extension_manager() {
        ...
        // Configure extension
        manager
            .configure_extension(1, "urn:ietf:params:rtp-hdrext:ssrc-audio-level".to_string())
            .unwrap();
        assert_eq!(
            manager.get_id_for_uri("urn:ietf:params:rtp-hdrext:ssrc-audio-level"),
            Some(1)
        );
        assert_eq!(
            manager.get_uri_for_id(1),
            Some(&"urn:ietf:params:rtp-hdrext:ssrc-audio-level".to_string())
        );

        // Auto-configure extension
        let id = manager
            .auto_configure_extension("urn:ietf:params:rtp-hdrext:toffset".to_string())
            .unwrap();
        assert_eq!(id, 2);
        assert_eq!(
            manager.get_id_for_uri("urn:ietf:params:rtp-hdrext:toffset"),
            Some(2)
        );
    ...
    async fn test_media_header_extension_service() {
        ...
        // Configure audio level extension
        let id = service
            .configure_audio_level_extension(Some(1))
            .await
            .unwrap();
        assert_eq!(id, 1);
        ...
        service
            .add_audio_level_for_client("client1", true, 60)
            .await
            .unwrap();
        ...
        service
            .record_received_extension("client1", received_ext)
            .await
            .unwrap();
