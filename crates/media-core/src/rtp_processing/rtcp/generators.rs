...
use super::feedback::{
    ...
    FeedbackPriority, QualityDegradation, StreamStats,
};
...
impl FeedbackGenerator for LossFeedbackGenerator {
    fn generate_feedback(
    ...
    ) -> Result<FeedbackDecision, MediaError> {
        let loss_rate = self.current_loss_rate();
        ...
        if self.should_generate_pli(context, config) {
            ...
            return Ok(FeedbackDecision::Pli { priority, reason });
        }
...
impl QualityFeedbackGenerator {
    ...
    fn calculate_quality_score(
        ...
        let overall_score =
            (loss_score * 0.4 + jitter_score * 0.3 + bandwidth_score * 0.3).clamp(0.0, 1.0);
...
impl FeedbackGenerator for QualityFeedbackGenerator {
    ...
    fn update_statistics(&mut self, stats: &StreamStats) {
        ...
        let quality_score =
            self.calculate_quality_score(loss_rate, stats.jitter_ms as u32, bandwidth_ratio);
    ...
    fn combine_feedback_decisions(&self, decisions: Vec<FeedbackDecision>) -> FeedbackDecision {
        ...
        for decision in decisions {
            match decision {
                ...
                FeedbackDecision::Pli { priority, .. } | FeedbackDecision::Fir { priority, .. } => {
...
impl FeedbackGenerator for ComprehensiveFeedbackGenerator {
    fn generate_feedback(
        ...
        decisions.push(
            self.congestion_generator
                .generate_feedback(context, config)?,
        );
