...
impl RemoteRateController {
    ...
    fn update(&mut self, state: GccState) -> u32 {
        match state {
            GccState::Increase => {
                self.current_bitrate = ((self.current_bitrate as f64 * self.increase_factor)
                    as u32)
                    .min(self.max_bitrate);
            }
            GccState::Decrease => {
                self.current_bitrate = ((self.current_bitrate as f64 * self.decrease_factor)
                    as u32)
...
impl GoogleCongestionControl {
    ...
    pub fn on_packet_arrival(&mut self, timestamp: Instant, size_bytes: usize) -> u32 {
        if let Some(last) = self.last_update {
            ...
            // Detect over-use
            let overuse_state = self
                .overuse_detector
                .detect(self.arrival_filter.value(), self.arrival_filter.variance());
...
impl TransportCcProcessor {
    ...
    pub fn should_send_feedback(&self) -> bool {
        match self.last_feedback_time {
            ...
            Some(last) => {
                last.elapsed() >= self.feedback_interval && !self.arrival_times.is_empty()
            }
        }
    }
