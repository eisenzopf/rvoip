...
use super::feedback::{FeedbackPacketType, PayloadFeedbackFormat, RtpSsrc};
        ...
        let packet_type = match data[1] {
            ...
            _ => {
                return Err(MediaError::FormatError(
                    "Unknown RTCP feedback type".to_string(),
                ))
            }
        };
...
impl RembPacket {
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self, MediaError> {
        ...
        let exp = data[17] >> 2;
        let mantissa =
            (((data[17] & 0x3) as u32) << 16) | ((data[18] as u32) << 8) | (data[19] as u32);
...
mod tests {
    ...
    fn test_bitrate_encoding() {
        ...
        let test_cases = vec![0, 1000, 64000, 1_000_000, 10_000_000, 100_000_000];
