...
impl G711Codec {
    ...
    pub fn encode_to_buffer(&mut self, samples: &[i16], output: &mut [u8]) -> Result<usize> {
        ...
        self.inner.encode_to_buffer(samples, output).map_err(|e| {
            Error::Codec(crate::error::CodecError::EncodingFailed {
                reason: format!(
                    "G.711 {} zero-copy encoding failed: {}",
                    match self.variant {
    ...
    pub fn decode_to_buffer(&mut self, data: &[u8], output: &mut [i16]) -> Result<usize> {
        self.inner.decode_to_buffer(data, output).map_err(|e| {
            Error::Codec(crate::error::CodecError::DecodingFailed {
                reason: format!(
                    "G.711 {} zero-copy decoding failed: {}",
                    match self.variant {
                        G711Variant::MuLaw => "μ-law",
                        G711Variant::ALaw => "A-law",
                    },
    ...
    fn decode(&mut self, encoded_data: &[u8]) -> Result<AudioFrame> {
        if encoded_data.is_empty() {
            return Err(Error::Codec(crate::error::CodecError::DecodingFailed {
                reason: format!(
                    "G.711 {} decoding failed: empty buffer",
                    match self.variant {
                        G711Variant::MuLaw => "μ-law",
                        G711Variant::ALaw => "A-law",
                    }
        ...
        let samples = self.inner.decode(encoded_data).map_err(|e| {
            Error::Codec(crate::error::CodecError::DecodingFailed {
                reason: format!(
                    "G.711 {} decoding failed: {}",
                    match self.variant {
                        G711Variant::MuLaw => "μ-law",
                        G711Variant::ALaw => "A-law",
                    },
