...
impl FrameBuffer {
        ...
        // Check for overflow
        {
            ...
            if frames.len() >= self.config.max_frames {
                ...
                if let Some((&oldest_ts, _)) = frames.iter().next() {
                    ...
                    debug!(
                        "Frame buffer overflow, dropped frame at timestamp {}",
                        oldest_ts
                    );
                }
            }
    ...
    pub async fn get_frames_in_range(&self, start_ts: u32, end_ts: u32) -> Vec<(u32, AudioFrame)> {
        ...
        {
            let mut frames = self.frames.write().await;
            let timestamps_to_remove: Vec<u32> =
                frames.range(start_ts..=end_ts).map(|(&ts, _)| ts).collect();
    ...
    pub async fn get_frames_older_than(
        &self,
