...
impl MediaEventAdapter {
    ...
    pub async fn publish_integration_event(&self, event: IntegrationEventType) -> Result<()> {
        ...
        if let Some(cross_crate_event) = self.convert_integration_to_cross_crate_event(&event) {
            ...
            {
                error!(
                    "Failed to publish cross-crate integration event from media-core: {}",
                    e
                );
            }
        }
    ...
    fn convert_media_to_cross_crate_event(
        &self,
        event: &MediaSessionEventType,
        ...
        match event {
            ...
            MediaSessionEventType::SessionDestroyed => Some(RvoipCrossCrateEvent::MediaToSession(
    ...
    fn convert_integration_to_cross_crate_event(
        &self,
        event: &IntegrationEventType,
    ) -> Option<RvoipCrossCrateEvent> {
        match event {
            IntegrationEventType::MediaSessionReady { session_id, .. } => Some(
            ...
            IntegrationEventType::MediaSessionDestroyed { session_id } => Some(
                RvoipCrossCrateEvent::MediaToSession(MediaToSessionEvent::MediaStreamStopped {
                    session_id: session_id.to_string(),
            ...
            IntegrationEventType::RtpSessionUnregister { session_id } => Some(
                RvoipCrossCrateEvent::MediaToRtp(MediaToRtpEvent::StopRtpStream {
                    session_id: session_id.to_string(),
                }),
            ),
...
impl CrossCrateEventHandler for MediaCrossCrateEventHandler {
    async fn handle(&self, event: Arc<dyn CrossCrateEvent>) -> Result<()> {
        debug!(
            "Handling cross-crate event in media-core: {}",
            event.event_type()
        );
