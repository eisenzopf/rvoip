...
impl MediaSessionController {
    ...
    pub async fn process_rtp_packet_zero_copy(
        &self,
        packet: &rtp_core::RtpPacket,
    ) -> Result<rtp_core::RtpPacket> {
        ...
        self.simd_processor
            .apply_gain_in_place(pooled_frame.samples_mut(), 1.2);
    ...
    pub async fn process_rtp_packet_traditional(
        ...
        let audio_frame =
            crate::types::AudioFrame::new(decoded_samples, 8000, 1, packet.header.timestamp);

        // Step 4: Process → Vec<i16> (COPY)
        let mut processed_samples = audio_frame.samples.clone();
        self.simd_processor
            .apply_gain(&audio_frame.samples, 1.2, &mut processed_samples);
