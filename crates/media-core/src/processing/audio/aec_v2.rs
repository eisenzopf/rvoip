...
impl AdvancedAcousticEchoCanceller {
    ...
    pub fn new(config: AdvancedAecConfig) -> Result<Self> {
        debug!(
            "Creating AdvancedAcousticEchoCanceller: FFT size={}, partitions={}",
    ...
    pub fn process_frame(
        ...
        if self.config.residual_suppression > 0.0 {
            cancelled_fft = self.suppress_residual_echo(
                &cancelled_fft,
                &echo_estimate_fft,
        ...
        if double_talk_confidence < 0.5 {
            // Low confidence = not double-talk
    ...
    fn detect_double_talk_advanced(
        &mut self,
    ...
    fn update_multi_partition_filters(
        ...
        for partition in 0..self.config.num_partitions {
            ...
            let normalized_step =
                self.config.nlms_step_size / (reference_power + self.config.regularization);
    ...
    /// Echo cancellation in frequency domain
    fn cancel_echo_frequency_domain(
        &self,
        near_fft: &[Complex<f32>],
        echo_fft: &[Complex<f32>],
    ) -> Vec<Complex<f32>> {
        near_fft
            .iter()
            .zip(echo_fft.iter())
    ...
    fn suppress_residual_echo(
        &mut self,
        signal_fft: &[Complex<f32>],
        echo_fft: &[Complex<f32>],
        double_talk_confidence: f32,
    ) -> Result<Vec<Complex<f32>>> {
        self.wiener_filter
            .apply(signal_fft, echo_fft, double_talk_confidence)
    }
    ...
    fn calculate_partition_power(&self, partition: usize) -> f32 {
        self.reference_history[partition]
            .iter()
            .map(|c| c.norm_sqr())
            .sum::<f32>()
            / self.reference_history[partition].len() as f32
    }

    /// Calculate residual echo level
    fn calculate_residual_level(&self, signal_fft: &[Complex<f32>]) -> f32 {
        signal_fft.iter().map(|c| c.norm_sqr()).sum::<f32>() / signal_fft.len() as f32
    }
}
...
impl ERLETracker {
    ...
    fn calculate_erle(&mut self, input_fft: &[Complex<f32>], output_fft: &[Complex<f32>]) -> f32 {
        ...
        let avg_input: f32 =
            self.input_power_history.iter().sum::<f32>() / self.history_size as f32;
        let avg_output: f32 =
            self.output_power_history.iter().sum::<f32>() / self.history_size as f32;
...
mod tests {
    ...
    fn test_echo_cancellation_basic() {
        ...
        println!(
            "AEC Basic - ERLE: {:.1}dB, Coherence: {:.2}, Double-talk: {:.2}",
        ...
        assert!(
            aec_result.double_talk_confidence >= 0.0 && aec_result.double_talk_confidence <= 1.0
        );
    }
    ...
    fn test_multi_partition_adaptation() {
        ...
        println!(
            "ERLE improvement: {:.1}dB -> {:.1}dB",
            initial_erle, final_erle
        );
    ...
    fn test_double_talk_detection() {
        ...
        println!(
            "Echo-only double-talk confidence: {:.2}",
            result1.double_talk_confidence
        );
        println!(
            "Mixed signal double-talk confidence: {:.2}",
            result2.double_talk_confidence
        );
    ...
    fn test_residual_echo_suppression() {
        ...
        println!(
            "Residual suppression result - ERLE: {:.1}dB, Residual level: {:.4}",
    ...
    fn test_latency_measurement() {
        ...
        println!(
            "AEC latency: {} samples ({:.1}ms at 8kHz)",
            result.latency_samples,
            result.latency_samples as f32 / 80.0
        );
    }
}
