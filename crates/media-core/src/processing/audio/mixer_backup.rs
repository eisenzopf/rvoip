...
impl AudioMixer {
    ...
    pub fn new(config: ConferenceMixingConfig) -> Result<Self> {
    ...
    pub async fn set_event_sender(&self, sender: tokio::sync::mpsc::UnboundedSender<ConferenceMixingEvent>) {
        if let Ok(mut event_sender) = self.event_sender.lock().await {
    ...
    pub async fn add_audio_stream(&self, id: ParticipantId, stream: AudioStream) -> ConferenceResult<()> {
        ...
        let active_participants = self.stream_manager.get_active_participants().await?;
        ...
        self.stream_manager.add_stream(stream).await?;
    ...
    pub fn remove_audio_stream(&self, id: &ParticipantId) -> ConferenceResult<()> {
    ...
    pub fn process_audio_frame(&self, participant_id: &ParticipantId, frame: AudioFrame) -> ConferenceResult<()> {
    ...
    pub fn get_mixed_audio(&self, participant_id: &ParticipantId) -> ConferenceResult<Option<AudioFrame>> {
        // Check cache first
        if let Ok(cache) = self.output_cache.lock() {
    ...
    pub fn mix_participants(&self, inputs: &[AudioFrame]) -> ConferenceResult<HashMap<ParticipantId, AudioFrame>> {
    ...
    pub fn get_mixing_stats(&self) -> ConferenceResult<ConferenceMixingStats> {
        self.stats.lock()
            .map(|stats| stats.clone())
            .map_err(|_| ConferenceError::MixingFailed {
    ...
    pub fn cleanup_inactive_participants(&self) -> ConferenceResult<Vec<ParticipantId>> {
