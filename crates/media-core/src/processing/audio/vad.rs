...
impl VoiceActivityDetector {
    ...
    pub fn analyze_frame(&mut self, frame: &AudioFrame) -> Result<VadResult> {
        ...
        trace!(
            "VAD: energy={:.4}, zcr={:.4}, voice={}, confidence={:.2}",
            energy,
            zcr,
            is_voice,
            confidence
    ...
    fn detect_voice_activity(&mut self, energy: f32, zcr: f32) -> bool {
        ...
        let energy_above_threshold =
            energy > self.config.energy_threshold * (self.noise_energy + 0.001); // Add small constant to avoid division by zero
    ...
    fn calculate_confidence(&self, energy: f32, zcr: f32) -> f32 {
        ...
        // Combined confidence
        (energy_confidence * 0.7 + zcr_confidence * 0.3)
            .max(0.0)
            .min(1.0)
    }
}
