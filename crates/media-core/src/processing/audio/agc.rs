...
impl AutomaticGainControl {
    ...
    pub fn new(config: AgcConfig) -> Result<Self> {
        ...
        let attack_coeff =
            Self::calculate_time_constant(config.attack_time_ms, sample_rate, frame_size);
        let release_coeff =
            Self::calculate_time_constant(config.release_time_ms, sample_rate, frame_size);
    ...
    pub fn process_frame(&mut self, frame: &AudioFrame) -> Result<AgcResult> {
        ...
        let desired_gain = if input_level > 0.0 {
            (self.config.target_level / input_level)
        ...
        trace!(
            "AGC: input={:.4}, gain={:.2}, output={:.4}, limiter={}",
            input_level,
            final_gain,
            output_level,
            limiter_active
