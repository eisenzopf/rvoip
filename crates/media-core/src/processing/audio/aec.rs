...
impl AcousticEchoCanceller {
    ...
    pub fn process_frame(
        ...
        let near_samples: Vec<f32> = near_end
            .samples
            .iter()
            .map(|&s| s as f32 / 32768.0)
            .collect();

        let far_samples: Vec<f32> = far_end
            .samples
            .iter()
            .map(|&s| s as f32 / 32768.0)
            .collect();
        ...
        let (output_samples, suppression_applied) =
            self.cancel_echo(&near_samples, &echo_estimate, double_talk_detected);
        ...
        trace!(
            "AEC: near={:.4}, far={:.4}, echo={:.4}, suppression={:.2}, double_talk={}",
            near_level,
            far_level,
            echo_level,
            suppression_applied,
        ...
        for i in 0..frame_size {
            ...
            for j in 0..self.config.filter_length.min(self.reference_buffer.len()) {
                if i + j < self.reference_buffer.len() {
                    sum += self.filter_coeffs[j]
                        * self.reference_buffer[self.reference_buffer.len() - 1 - i - j];
                }
            }
    ...
    fn cancel_echo(
        ...
        for (i, (&near, &echo)) in near_samples.iter().zip(echo_estimate.iter()).enumerate() {
            ...
            let final_output =
                if echo_cancelled.abs() < self.config.min_echo_level && self.config.comfort_noise {
