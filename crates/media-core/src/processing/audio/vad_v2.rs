...
impl AdvancedVoiceActivityDetector {
    ...
    pub fn new(config: AdvancedVadConfig, sample_rate: f32) -> Result<Self> {
        debug!(
            "Creating AdvancedVoiceActivityDetector: FFT size={}, sample_rate={}",
        ...
        let window: Vec<f32> = hanning_iter(config.fft_size).map(|x| x as f32).collect();
    ...
    pub fn analyze_frame(&mut self, frame: &AudioFrame) -> Result<AdvancedVadResult> {
        ...
        trace!(
            "AdvancedVAD: voice={}, conf={:.2}, energy={:.4}, centroid={:.0}Hz",
            final_decision,
            confidence,
            energy,
            spectral_centroid
    ...
    fn calculate_spectrum(&mut self, samples: &[i16]) -> Result<Vec<f32>> {
        ...
        for (i, (&sample, &window_val)) in samples
            .iter()
    ...
    fn detect_fundamental_frequency(&self, spectrum: &[f32]) -> f32 {
        let min_bin =
            ((self.config.f0_range.0 / self.sample_rate) * 2.0 * spectrum.len() as f32) as usize;
        let max_bin =
            ((self.config.f0_range.1 / self.sample_rate) * 2.0 * spectrum.len() as f32) as usize;
    ...
    fn calculate_detector_scores(
        ...
        let spectral_score = if spectral_centroid > 500.0
            && spectral_centroid < self.config.spectral_centroid_threshold
        {
            1.0
        } else {
            0.0
...
mod tests {
    ...
    fn test_speech_detection() {
        ...
        println!(
            "Speech detection - Voice: {}, Confidence: {:.2}, F0: {:.1}Hz, Centroid: {:.1}Hz",
            vad_result.is_voice,
            vad_result.confidence,
            vad_result.fundamental_frequency,
            vad_result.spectral_centroid
        );
    ...
    fn test_noise_rejection() {
        ...
        for _ in 0..10 {
            ...
            println!(
                "Noise frame - Voice: {}, Confidence: {:.2}, Energy: {:.4}",
    ...
    fn test_spectral_features() {
        ...
        println!(
            "Spectral centroids - Low: {:.1}Hz, Mid: {:.1}Hz, High: {:.1}Hz",
            low_result.spectral_centroid,
            mid_result.spectral_centroid,
    ...
    fn test_ensemble_vs_simple_decision() {
        ...
        println!(
            "Ensemble confidence: {:.2}, Simple confidence: {:.2}",
