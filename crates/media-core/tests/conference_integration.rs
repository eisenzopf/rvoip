...
async fn test_participant_management() {
    ...
    for participant in &participants {
        ...
        // Start media session
        controller
            .start_media(DialogId::new(dialog_id.clone()), media_config)
            .await
            .expect("Failed to start media session");

        // Add to conference
        controller
            .add_to_conference(&dialog_id)
            .await
    ...
    controller
        .remove_from_conference("dialog_bob")
        .await
...
async fn test_conference_capacity_limits() {
    ...
    controller
        .start_media(DialogId::new(overflow_dialog.clone()), media_config)
        .await
        .expect("Failed to start media session");
    ...
    assert!(
        result.is_err(),
        "Should fail to add participant beyond capacity"
    );
...
async fn test_conference_audio_processing() {
    ...
    for frame_num in 0..5 {
        for (i, participant) in participants.iter().enumerate() {
            ...
            controller
                .process_conference_audio(&dialog_id, audio_frame)
                .await
        ...
        for participant in &participants {
            let dialog_id = format!("dialog_{}", participant);
            let mixed_audio = controller
                .get_conference_mixed_audio(&dialog_id)
                .await
    ...
    assert!(
        stats.total_mixes > 0,
        "Should have performed mixing operations"
    );
}
...
async fn test_conference_events() {
    ...
    let event_collector = tokio::spawn(async move {
        ...
        controller_for_task
            .add_to_conference(&dialog_id_for_task)
            .await
            .expect("Failed to add to conference");
        ...
        controller_for_task
            .remove_from_conference(&dialog_id_for_task)
            .await
            .expect("Failed to remove from conference");
        ...
        let collection_timeout = timeout(Duration::from_millis(500), async {
            while let Some(event) = events.recv().await {
                ...
                if collected_events.len() >= 2 {
                    // Expect 2 events (add + remove)
                    break;
                }
            }
        });
    ...
    let has_participant_added = collected_events
        .iter()
        .any(|event| matches!(event, ConferenceMixingEvent::ParticipantAdded { .. }));

    assert!(
        has_participant_added,
        "Should have received ParticipantAdded event"
    );
}
...
async fn test_error_handling() {
    ...
    let result = controller
        .remove_from_conference("nonexistent_dialog")
        .await;
    ...
    let result = controller
        .process_conference_audio("nonexistent_dialog", audio_frame)
        .await;
    assert!(result.is_err(), "Should fail for non-existent participant");

    // Try to get mixed audio for non-existent participant
    let result = controller
        .get_conference_mixed_audio("nonexistent_dialog")
        .await;
    assert!(result.is_err(), "Should fail for non-existent participant");
}
