...
fn test_performance_benchmark() {
    ...
    let encode_per_iter = encode_duration / iterations;
    
    println!("G.711 encoding: {:?} per frame ({}k frames/sec)", 
             encode_per_iter, 
             1_000_000 / encode_per_iter.as_micros());
    ...
    let decode_per_iter = decode_duration / iterations;
    
    println!("G.711 decoding: {:?} per frame ({}k frames/sec)", 
             decode_per_iter,
             1_000_000 / decode_per_iter.as_micros());
    
    // Benchmark zero-copy encoding
    let mut output_buffer = vec![0u8; 160];
    let start = Instant::now();
    for _ in 0..iterations {
        let _ = codec.encode_to_buffer(&samples, &mut output_buffer).unwrap();
    }
    let zero_copy_encode_duration = start.elapsed();
    let zero_copy_encode_per_iter = zero_copy_encode_duration / iterations;
    
    println!("G.711 zero-copy encoding: {:?} per frame ({}k frames/sec)", 
             zero_copy_encode_per_iter,
             1_000_000 / zero_copy_encode_per_iter.as_micros());
    
    // Zero-copy should be faster than regular encoding
    assert!(zero_copy_encode_per_iter < encode_per_iter);
}