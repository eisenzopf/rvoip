...
async fn test_long_running_rtp_streaming() {
    ...
    controller
        .update_media(dialog_a.clone(), updated_config_a)
        .await
        .expect("Failed to update media config for A");

    controller
        .update_media(dialog_b.clone(), updated_config_b)
        .await
    ...
    controller
        .set_audio_frame_callback(dialog_a.clone(), frame_tx_a)
        .await
        .expect("Failed to set callback for A");

    controller
        .set_audio_frame_callback(dialog_b.clone(), frame_tx_b)
        .await
    ...
    info!(
        "🎯 Target: {} seconds, expecting ~{} frames",
        TARGET_DURATION_SECS, EXPECTED_FRAMES
    );
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                for i in 0..SAMPLES_PER_FRAME {
                    let t = ((frame_count * SAMPLES_PER_FRAME as u64 + i as u64) as f32)
                        / SAMPLE_RATE as f32;
            ...
            info!(
                "✅ Dialog A: Finished sending {} frames in {:.1}s",
                frame_count,
    ...
    let send_task_b = {
        ...
        tokio::spawn(async move {
            ...
            let total_duration = start_time.elapsed();
            info!(
                "✅ Dialog B: Finished sending {} frames in {:.1}s",
                frame_count,
                total_duration.as_secs_f32()
            );
        })
    ...
    let receive_task_a = {
        ...
        tokio::spawn(async move {
            ...
            loop {
                match tokio::time::timeout(Duration::from_secs(5), frame_rx_a.recv()).await {
                    Ok(Some(frame)) => {
                        ...
                        if frame_count == 1 {
                            info!(
                                "🎵 Dialog A received first frame: {} samples",
                                frame.samples.len()
                            );
                        }
                    ...
                    Ok(None) => {
                        warn!(
                            "Dialog A: Receive channel closed after {} frames",
                            frame_count
                        );
                        break;
                    }
                    Err(_) => {
                        warn!(
                            "Dialog A: Timeout waiting for frames after {} frames",
                            frame_count
                        );
                        break;
                    }
            ...
            let total_duration = start_time.elapsed();
            info!(
                "🛑 Dialog A: Stopped receiving after {} frames in {:.1}s",
                frame_count,
                total_duration.as_secs_f32()
            );
        })
    ...
    let receive_task_b = {
        ...
        tokio::spawn(async move {
            ...
            loop {
                match tokio::time::timeout(Duration::from_secs(5), frame_rx_b.recv()).await {
                    Ok(Some(frame)) => {
                        ...
                        if frame_count == 1 {
                            info!(
                                "🎵 Dialog B received first frame: {} samples",
                                frame.samples.len()
                            );
                        }
                    ...
                    Ok(None) => {
                        warn!(
                            "Dialog B: Receive channel closed after {} frames",
                            frame_count
                        );
                        break;
                    }
                    Err(_) => {
                        warn!(
                            "Dialog B: Timeout waiting for frames after {} frames",
                            frame_count
                        );
                        break;
                    }
            ...
            let total_duration = start_time.elapsed();
            info!(
                "🛑 Dialog B: Stopped receiving after {} frames in {:.1}s",
                frame_count,
                total_duration.as_secs_f32()
            );
        })
    ...
    if let Some(stats_a) = controller.get_rtp_statistics(&dialog_a).await {
        info!(
            "📊 Dialog A RTP stats: packets_sent={}, packets_received={}, packets_lost={}",
    ...
    if let Some(stats_b) = controller.get_rtp_statistics(&dialog_b).await {
        info!(
            "📊 Dialog B RTP stats: packets_sent={}, packets_received={}, packets_lost={}",
    ...
    controller
        .stop_media(&dialog_a)
        .await
        .expect("Failed to stop media A");
    controller
        .stop_media(&dialog_b)
        .await
        .expect("Failed to stop media B");

    // Final report
    info!("📊 === FINAL REPORT ===");
    info!("📊 Expected frames: ~{}", EXPECTED_FRAMES);
    info!(
        "📊 Dialog A: Sent {} frames, Received {} frames",
        sent_a, received_a
    );
    info!(
        "📊 Dialog B: Sent {} frames, Received {} frames",
        sent_b, received_b
    );
    ...
    // Assertions
    assert!(
        sent_a >= EXPECTED_FRAMES * 95 / 100,
        "Dialog A should send at least 95% of expected frames, sent {}",
        sent_a
    );
    assert!(
        sent_b >= EXPECTED_FRAMES * 95 / 100,
        "Dialog B should send at least 95% of expected frames, sent {}",
        sent_b
    );
    ...
    assert!(
        received_a >= sent_b * 90 / 100,
        "Dialog A should receive at least 90% of frames sent by B, received {} of {}",
        received_a,
        sent_b
    );
    assert!(
        received_b >= sent_a * 90 / 100,
        "Dialog B should receive at least 90% of frames sent by A, received {} of {}",
        received_b,
        sent_a
    );
