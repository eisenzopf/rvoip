...
async fn test_session_controller_performance_integration() -> Result<()> {
    ...
    println!(
        "📊 Initial frame pool stats: {} frames available",
        pool_stats.pool_size
    );
    assert!(
        pool_stats.pool_size > 0,
        "Frame pool should have frames available"
    );
    ...
    println!(
        "   - AGC v2: {} (single-band)",
        processor_config.enable_advanced_agc
    );
    ...
    println!(
        "\n📞 Starting basic media session for dialog: {}",
        dialog_id
    );

    controller
        .start_media(dialog_id.clone(), media_config.clone())
        .await?;
    ...
    if let Some(session_info) = controller.get_session_info(&dialog_id).await {
        println!(
            "📊 Session info: port={:?}, status={:?}",
            session_info.rtp_port, session_info.status
        );
        assert!(
            session_info.rtp_port.is_some(),
            "RTP port should be allocated"
        );
    } else {
    ...
    println!(
        "\n🚀 Starting advanced media session for dialog: {}",
        dialog_id_advanced
    );

    controller
        .start_advanced_media(
    ...
    let has_processors = controller
        .has_advanced_processors(&dialog_id_advanced)
        .await;
    ...
    let processed_frame = controller
        .process_advanced_audio(&dialog_id_advanced, test_frame)
        .await?;
    println!("✅ Audio frame processed with advanced v2 processors");
    println!(
        "   - Input samples: 512, Output samples: {}",
        processed_frame.samples.len()
    );
    assert_eq!(
        processed_frame.samples.len(),
        512,
        "Output frame should maintain sample count"
    );

    // Get performance metrics
    if let Some(dialog_metrics) = controller
        .get_dialog_performance_metrics(&dialog_id_advanced)
        .await
    {
        ...
        assert!(
            dialog_metrics.operation_count > 0,
    ...
    assert!(
        global_metrics.operation_count > 0,
        "Should have recorded global operations"
    );
    ...
    let has_processors_after = controller
        .has_advanced_processors(&dialog_id_advanced)
        .await;
    println!(
        "🔍 Advanced processors after cleanup: {}",
        has_processors_after
    );
    assert!(
        !has_processors_after,
        "Advanced processors should be cleaned up"
    );
...
async fn test_performance_metrics_tracking() -> Result<()> {
    ...
    assert_eq!(
        initial_metrics.operation_count, 0,
...
async fn test_frame_pool_integration() -> Result<()> {
    ...
    println!(
        "Initial pool stats: size={}, hits={}, misses={}",
        initial_stats.pool_size, initial_stats.pool_hits, initial_stats.pool_misses
    );

    assert!(
        initial_stats.pool_size > 0,
        "Pool should have initial frames"
    );
