...
fn create_echo_frame(
    ...
    for (i, &sample) in original.samples.iter().enumerate() {
        if i >= delay_samples {
            let echo_contribution =
                (original.samples[i - delay_samples] as f32 * echo_strength) as i16;
...
async fn test_vad_accuracy_comparison() {
    ...
    let test_scenarios = vec![
        (
            "Speech (200Hz)",
            create_test_frame(200.0, 0.5, 8000, 64),
            true,
        ),
        (
            "Speech (400Hz)",
            create_test_frame(400.0, 0.4, 8000, 64),
            true,
        ),
        (
            "High Frequency (3000Hz)",
            create_test_frame(3000.0, 0.5, 8000, 64),
            false,
        ),
        ...
        (
            "Quiet Signal",
            create_test_frame(150.0, 0.1, 8000, 64),
            false,
        ),
    ];
    ...
    println!(
        "{:<20} {:<15} {:<15} {:<15} {:<10}",
        "Scenario", "Expected", "Basic VAD", "Advanced VAD", "Expected"
    );
    println!("{}", "-".repeat(80));
    ...
    let avg_advanced_time =
        advanced_times.iter().sum::<std::time::Duration>() / advanced_times.len() as u32;

    println!("\n📈 Performance Summary:");
    println!(
        "Basic VAD Accuracy:    {:.1}% ({}/{})",
        basic_accuracy, basic_correct, total_tests
    );
    println!(
        "Advanced VAD Accuracy: {:.1}% ({}/{})",
        advanced_accuracy, advanced_correct, total_tests
    );
    println!(
        "Accuracy Improvement:  {:.1} percentage points",
        advanced_accuracy - basic_accuracy
    );
    println!();
    ...
    println!(
        "Time Overhead:         {:.1}x",
        avg_advanced_time.as_nanos() as f32 / avg_basic_time.as_nanos() as f32
    );
...
async fn test_aec_erle_comparison() {
    ...
    println!(
        "{:<6} {:<15} {:<15} {:<15} {:<15}",
        "Frame", "Basic ERLE", "Advanced ERLE", "Basic Time", "Adv Time"
    );
    println!("{}", "-".repeat(75));
    ...
    for frame_num in 0..15 {
        // Test basic AEC
        let start = Instant::now();
        let basic_result = basic_aec
            .process_frame(&near_end_with_echo, &far_end)
            .unwrap();
        let basic_time = start.elapsed();
        basic_times.push(basic_time);
        ...
        // Test advanced AEC
        let start = Instant::now();
        let advanced_result = advanced_aec
            .process_frame(&near_end_with_echo, &far_end)
            .unwrap();
        let advanced_time = start.elapsed();
        advanced_times.push(advanced_time);
        ...
        if frame_num % 3 == 0 {
            println!(
                "{:<6} {:<15.1} {:<15.1} {:<15?} {:<15?}",
                frame_num, basic_erle, advanced_result.erle_db, basic_time, advanced_time
            );
        }
    }
    ...
    println!(
        "ERLE Improvement:        {:.1} dB",
        final_advanced_erle - final_basic_erle
    );
    println!();
    ...
    println!(
        "ERLE comparison: Advanced ({:.1}) vs Basic ({:.1})",
        final_advanced_erle, final_basic_erle
    );
}
...
async fn test_agc_consistency_comparison() {
    ...
    println!(
        "{:<6} {:<12} {:<15} {:<15} {:<15} {:<15}",
        "Frame", "Input Level", "Basic Output", "Adv Output", "Basic Gain", "Adv Gain"
    );
    println!("{}", "-".repeat(90));
    ...
    for (i, &input_level) in test_levels.iter().enumerate() {
        ...
        let advanced_result = advanced_agc
            .process_frame(&mut advanced_frame_copy)
            .unwrap();
        let advanced_time = start.elapsed();
        advanced_times.push(advanced_time);
    ...
    let basic_variance = basic_outputs
        .iter()
        .map(|x| (x - basic_mean).powi(2))
        .sum::<f32>()
        / basic_outputs.len() as f32;
    let advanced_variance = advanced_outputs
        .iter()
        .map(|x| (x - advanced_mean).powi(2))
        .sum::<f32>()
        / advanced_outputs.len() as f32;
    ...
    println!(
        "Consistency Improvement:    {:.1}x",
        basic_std_dev / advanced_std_dev
    );
    println!();
...
async fn test_comprehensive_performance_comparison() {
    ...
    println!(
        "{:<15} {:<15} {:<15} {:<15}",
        "Component", "Basic (μs)", "Advanced (μs)", "Overhead"
    );
    println!("{}", "-".repeat(65));
    ...
    for (signal_name, signal) in test_signals {
        ...
        let mut advanced_vad =
            AdvancedVoiceActivityDetector::new(advanced_vad_config, sample_rate as f32).unwrap();
    ...
    println!(
        "• AEC: Advanced version uses frequency-domain processing and multi-partition filtering"
    );
