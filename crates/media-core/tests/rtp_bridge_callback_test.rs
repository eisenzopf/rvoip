...
use rvoip_media_core::types::{MediaPacket, MediaSessionId};
...
async fn test_rtp_bridge_event_forwarding() {
    ...
    bridge
        .process_incoming_packet(&session_id, test_packet.clone())
        .await
    ...
    match received_event {
        RtpEvent::MediaReceived {
            ...
            assert_eq!(
                *payload_type, test_packet.payload_type,
                "Payload type should match"
            );
            assert_eq!(
                payload,
                &test_packet.payload.to_vec(),
                "Payload should match"
            );
            assert_eq!(*timestamp, test_packet.timestamp, "Timestamp should match");
            assert_eq!(
                *sequence_number, test_packet.sequence_number,
...
async fn test_multiple_callbacks() {
    ...
    bridge
        .process_incoming_packet(&session_id, test_packet)
        .await
    ...
    assert_eq!(
        received_events_1.lock().unwrap().len(),
        1,
        "Callback 1 should receive event"
    );
    assert_eq!(
        received_events_2.lock().unwrap().len(),
        1,
        "Callback 2 should receive event"
    );
    assert_eq!(
        received_events_3.lock().unwrap().len(),
        1,
        "Callback 3 should receive event"
    );
...
async fn test_callback_isolation_between_sessions() {
    ...
    bridge
        .register_session(session_1.clone(), rtp_params)
        .await
        .unwrap();
    ...
    bridge
        .register_session(session_2.clone(), rtp_params_2)
        .await
        .unwrap();
    ...
    bridge
        .process_incoming_packet(&session_1, packet_1)
        .await
        .unwrap();
    bridge
        .process_incoming_packet(&session_2, packet_2)
        .await
        .unwrap();
    ...
    let session_ids: std::collections::HashSet<String> =
        sessions.iter().map(|s| s.as_str().to_string()).collect();

    assert!(
        session_ids.contains(session_1.as_str()),
        "Should contain session 1"
    );
    assert!(
        session_ids.contains(session_2.as_str()),
...
async fn create_test_rtp_bridge() -> Arc<RtpBridge> {
    let (integration_event_tx, _integration_event_rx) =
        mpsc::unbounded_channel::<IntegrationEvent>();
