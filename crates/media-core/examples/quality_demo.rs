...
use rvoip_media_core::quality::{
    AdaptationStrategy, OverallMetrics, QualityAdjustment, QualityMetrics, QualityMonitor,
    QualityMonitorConfig, SessionMetrics,
};
...
async fn main() -> Result<()> {
    ...
    for (scenario_name, metrics) in scenarios {
        ...
        let _ = quality_monitor
            .analyze_media_packet(&session1, &test_packet)
            .await;
    ...
    for i in 0..3 {
        ...
        let _ = quality_monitor
            .analyze_media_packet(&session_id, &packet)
            .await;
    }
    ...
    let quality_sequence = vec![4.2, 4.1, 3.8, 3.5, 3.2, 2.8, 2.5, 2.7, 3.0, 3.4, 3.8, 4.1];
    ...
    for (i, &mos_score) in quality_sequence.iter().enumerate() {
        ...
        let packet = create_test_packet();
        let _ = quality_monitor
            .analyze_media_packet(&trend_session, &packet)
            .await;
        ...
        if let Some(session_metrics) = quality_monitor.get_session_metrics(&trend_session).await {
            ...
            if i > 2 && (trend != QualityTrend::Stable) {
                ...
                if !adjustments.is_empty() {
                    println!(
                        "     💡 Adaptation suggested: {} adjustments",
                        adjustments.len()
                    );
                    for adj in &adjustments[..1] {
                        // Show first adjustment
                        println!(
                            "       - {} (confidence: {:.0}%)",
                            adj.reason,
                            adj.confidence * 100.0
                        );
                    }
                }
            }
    ...
    println!(
        "     Average MOS: {:.2}",
        final_overall.avg_quality.mos_score
    );
    println!("     System CPU: {:.1}%", final_overall.cpu_usage);
    println!(
        "     Memory usage: {:.1} MB",
        final_overall.memory_usage as f64 / 1024.0 / 1024.0
    );
    println!(
        "     Bandwidth: {:.1} kbps",
        final_overall.bandwidth_usage as f64 / 1000.0
    );
...
fn display_overall_metrics(metrics: &OverallMetrics) {
    ...
    println!(
        "     Bandwidth: {:.1} kbps",
        metrics.bandwidth_usage as f64 / 1000.0
    );
}
