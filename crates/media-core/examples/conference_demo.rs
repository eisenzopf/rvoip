...
/// Create MediaSessionController with conference mixing enabled
async fn create_conference_controller(
) -> Result<Arc<MediaSessionController>, Box<dyn std::error::Error>> {
...
async fn demo_conference_operations(
    ...
    info!(
        "📊 Conference stats: {} active participants",
...
async fn simulate_conference_call(
    controller: &Arc<MediaSessionController>,
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(mut events) = event_receiver {
        tokio::spawn(async move {
            while let Some(event) = events.recv().await {
                match event {
                    ConferenceMixingEvent::ParticipantAdded {
                        participant_id,
                        participant_count,
                    } => {
                        info!(
                            "🎤 Conference event: {} joined (total: {})",
                            participant_id, participant_count
                        );
                    }
                    ConferenceMixingEvent::ParticipantRemoved {
                        participant_id,
                        participant_count,
                    } => {
                        info!(
                            "👋 Conference event: {} left (total: {})",
                            participant_id, participant_count
                        );
                    }
                    ConferenceMixingEvent::VoiceActivityChanged {
                        participant_id,
                        is_talking,
                    } => {
                        info!(
                    ...
                    ConferenceMixingEvent::QualityChanged {
                        old_score,
                        new_score,
                        reason,
                    } => {
                        info!(
                            "📊 Conference event: Quality changed {:.2} -> {:.2} ({})",
                            old_score, new_score, reason
                        );
                    }
                    ConferenceMixingEvent::PerformanceWarning {
                        latency_us,
                        cpu_usage,
                        reason,
                    } => {
    ...
    for frame_count in 0..50 {
        // Simulate 1 second of audio (50 * 20ms)
        interval.tick().await;
        ...
        for (i, participant) in participants.iter().enumerate() {
            ...
            if let Err(e) = controller
                .process_conference_audio(participant, audio_frame)
                .await
            {
        ...
        if frame_count % 25 == 0 {
            if let Ok(stats) = controller.get_conference_stats().await {
                info!(
                    "📊 Conference stats: {} participants, {:.2}μs avg latency, {:.1}% CPU",
                    stats.active_participants,
...
async fn demo_advanced_features(
    ...
    for i in 0..7 {
        // Try to add more than max_participants (5)
