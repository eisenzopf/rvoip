...
async fn main() -> Result<()> {
    ...
    // Create AEC instance
    println!(
        "🏗️ Creating AEC with {} taps filter...",
    ...
    println!(
        "   Echo suppression: {:.2}%",
        aec_result.echo_suppression * 100.0
    );
    println!(
        "   Double-talk detected: {}",
        aec_result.double_talk_detected
    );
    ...
    let mixed_near_end = mix_signals(
        &near_end_speech,
        &create_delayed_echo(&far_end_speech, 0.2, 15),
    );
    ...
    println!(
        "   Echo suppression: {:.2}%",
        double_talk_result.echo_suppression * 100.0
    );
    println!(
        "   Double-talk detected: {}",
        double_talk_result.double_talk_detected
    );
    ...
    println!(
        "   Processing {} frames to show adaptation...",
        adaptation_frames
    );
    ...
    for i in 0..adaptation_frames {
        ...
        if i % 3 == 0 {
            // Print every 3rd frame
            println!(
                "   Frame {}: echo_level={:.4}, suppression={:.2}%",
                i,
                result.echo_estimate,
                result.echo_suppression * 100.0
            );
        }
    }
    ...
    println!(
        "   Processed {} frames in {:.2}ms",
        performance_frames,
        total_time.as_millis()
    );
    println!("   Average time per frame: {} μs", avg_time_per_frame);
    println!(
        "   Real-time factor: {:.1}x (20ms frames)",
...
fn mix_signals(signal1: &AudioFrame, signal2: &AudioFrame) -> AudioFrame {
    ...
    let mixed_samples: Vec<i16> = signal1
        .samples
        .iter()
