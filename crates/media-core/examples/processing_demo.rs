...
async fn main() -> Result<()> {
    ...
    if let Some(audio_result) = &quiet_result.audio_result {
        if let Some(vad_result) = &audio_result.vad_result {
            println!(
                "   VAD Result: {} (confidence: {:.2}, energy: {:.4})",
        ...
        if let Some(agc_result) = &audio_result.agc_result {
            println!(
                "   AGC Result: gain={:.2}x, input={:.4}, output={:.4}, limiter={}",
                agc_result.applied_gain,
                agc_result.input_level,
                agc_result.output_level,
                agc_result.limiter_active
            );
        }
    }
    ...
    println!(
        "📈 Original: {}Hz, {}ch, {} samples",
        quiet_speech_frame.sample_rate,
        quiet_speech_frame.channels,
        quiet_speech_frame.samples.len()
    );
    println!(
        "📈 Converted: {}Hz, {}ch, {} samples",
        quiet_result.frame.sample_rate,
        quiet_result.frame.channels,
        quiet_result.frame.samples.len()
    );
    ...
    println!(
        ...
        stats.audio_processing_operations
    );
    ...
    println!(
        ...
        final_stats.frames_processed
    );
    println!(
        "   Voice frames detected: {}%",
        (final_stats.audio_processing_operations as f64 / final_stats.frames_processed as f64)
            * 100.0
    );
...
fn create_synthetic_speech_frame(
    sample_rate: u32,
    channels: u8,
    ...
    for i in 0..samples_per_channel {
        ...
        let signal = (t * 2.0 * std::f32::consts::PI * 200.0).sin() * 0.3 +  // 200 Hz fundamental
...
fn create_synthetic_silence_frame(
    sample_rate: u32,
    channels: u8,
    samples_per_channel: usize,
) -> AudioFrame {
