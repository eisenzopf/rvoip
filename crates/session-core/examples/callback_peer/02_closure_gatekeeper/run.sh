...
cargo run -p rvoip-session-core --example callback_peer_closure_gatekeeper_server --quiet \
  2>&1 | sed "s/^/$(printf '\033[0;32m')[SERVER]$(printf '\033[0m') /" &
sleep 2

echo -e "${CYAN}[CLIENT]${NC} Starting"
cargo run -p rvoip-session-core --example callback_peer_closure_gatekeeper_client --quiet \
