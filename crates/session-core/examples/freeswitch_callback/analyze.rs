//! Generic analyzer for FreeSWITCH CallbackPeer example WAV files.
...
fn analyze_hold(tls: bool) -> ExampleResult<()> {
    let cfg = if tls {
        endpoint_config("1001", 15070, 16000, 16100)?
    } else {
        endpoint_config("2001", 15080, 17000, 17100)?
    };
    ...
    println!("=== FreeSWITCH {} audio analysis ===", label);
    print_analysis(
...
fn analyze_dtmf() -> ExampleResult<()> {
    ...
    println!("=== FreeSWITCH TLS/SRTP callback DTMF audio analysis ===");
...
fn analyze_transfer() -> ExampleResult<()> {
    ...
    println!("=== FreeSWITCH TLS/SRTP callback blind transfer audio analysis ===");
