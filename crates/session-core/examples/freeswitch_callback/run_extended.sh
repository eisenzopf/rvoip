#!/usr/bin/env sh
# Extended CallbackPeer FreeSWITCH validation sequence.
set -eu

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
FREESWITCH_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/../freeswitch" && pwd)
...
run_endpoint() {
  ...
  AUDIO_OUTPUT_DIR="$out_dir" CALLBACK_SCENARIO="$scenario" \
    cargo run -p rvoip-session-core --features dev-insecure-tls \
      --example freeswitch_callback_endpoint --quiet >"$log" 2>&1
}
...
run_analyze() {
  ...
  AUDIO_OUTPUT_DIR="$out_dir" CALLBACK_ANALYZE="$scenario" \
    cargo run -p rvoip-session-core --features dev-insecure-tls \
      --example freeswitch_callback_analyze --quiet >"$log" 2>&1
}
...
echo "Building CallbackPeer extended FreeSWITCH examples..."
cargo build -p rvoip-session-core --features dev-insecure-tls \
  --example freeswitch_callback_endpoint \
  --example freeswitch_callback_analyze
...
echo
echo "========================================================================"
echo "== CallbackPeer extended FreeSWITCH sequence complete"
echo "========================================================================"
