//! FreeSWITCH TLS/SRTP ring/cancel target: 1003 registers and lets 1001 ring
//! without answering so 1001 can validate CANCEL through FreeSWITCH.
...
async fn main() -> ExampleResult<()> {
    ...
    let cfg = endpoint_config("1003", 15074, 16240, 16340)?;
        ...
        .await
    {
        ...
        Err(e) => {
            println!(
                "[1003] No target-side cancellation event observed before timeout ({e}); caller-side cancellation remains the required FreeSWITCH assertion."
            );
            guard.abandon();
        }
    }
