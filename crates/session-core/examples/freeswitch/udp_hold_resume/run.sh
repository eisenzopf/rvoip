#!/usr/bin/env sh
# FreeSWITCH UDP hold/resume example: register 2001 and 2002, call through
# FreeSWITCH, exercise a mid-call hold/resume, and verify audio before and
# after resume.
set -eu
...
echo "Building FreeSWITCH UDP hold/resume endpoint examples..."
cargo build -p rvoip-session-core \
  --example freeswitch_udp_hold_resume_2001 \
  --example freeswitch_udp_hold_resume_2002 \
  --example freeswitch_udp_hold_resume_analyze

echo "[2002] Starting"
AUDIO_OUTPUT_DIR="$OUT_DIR" cargo run -p rvoip-session-core --example freeswitch_udp_hold_resume_2002 --quiet \
  >"$LOG_2002" 2>&1 &
PID_2002=$!
...
echo "[2001] Starting"
AUDIO_OUTPUT_DIR="$OUT_DIR" cargo run -p rvoip-session-core --example freeswitch_udp_hold_resume_2001 --quiet \
  >"$LOG_2001" 2>&1 &
PID_2001=$!
...
AUDIO_OUTPUT_DIR="$OUT_DIR" cargo run -p rvoip-session-core --example freeswitch_udp_hold_resume_analyze --quiet \
  >"$LOG_ANALYZE" 2>&1 &
PID_ANALYZE=$!
...
echo
echo "=== FreeSWITCH UDP hold/resume example complete ==="
