...
freeswitch_tls_uses_reachable_contact() {
  case "$(freeswitch_tls_contact_mode)" in
...
freeswitch_generate_tls_cert() {
  cert_dir=$1

  if ! command -v openssl >/dev/null 2>&1; then
    echo "openssl not found; install openssl or set TLS_CERT_PATH/TLS_KEY_PATH in examples/freeswitch/.env" >&2
    return 1
  fi

  mkdir -p "$cert_dir"
  cert_conf="$cert_dir/openssl.cnf"
  export TLS_CERT_PATH="$cert_dir/rvoip-freeswitch-listener.pem"
  export TLS_KEY_PATH="$cert_dir/rvoip-freeswitch-listener-key.pem"

  cert_cn=${TLS_CERT_CN:-rvoip-freeswitch-example.local}
  dns_i=1
  ip_i=1

  {
    ...
    if [ -n "$advertised_ip" ]; then
      if freeswitch_is_ip_literal "$advertised_ip"; then
        printf 'IP.%s = %s\n' "$ip_i" "$advertised_ip"
        ip_i=$((ip_i + 1))
      else
        printf 'DNS.%s = %s\n' "$dns_i" "$advertised_ip"
    ...
    if [ -n "$local_ip" ] && freeswitch_is_ip_literal "$local_ip"; then
      printf 'IP.%s = %s\n' "$ip_i" "$local_ip"
    fi
  } >"$cert_conf"
...
ensure_freeswitch_tls_listener_cert() {
  cert_dir=$1

  if ! freeswitch_tls_uses_reachable_contact; then
    return 0
  fi
  ...
  freeswitch_generate_tls_cert "$cert_dir"
}
