//! FreeSWITCH TLS/SRTP hold/resume endpoint 1002: register over SIP TLS,
...
async fn main() -> ExampleResult<()> {
    ...
    let cfg = endpoint_config("1002", 15072, 16120, 16220)?;
    ...
    if expect_remote_hold_events()? {
        ...
        println!(
            "[1002] Remote hold/resume event assertion disabled; set FREESWITCH_EXPECT_REMOTE_HOLD_EVENTS=1 for PBX profiles that forward hold re-INVITEs."
        );
    }
