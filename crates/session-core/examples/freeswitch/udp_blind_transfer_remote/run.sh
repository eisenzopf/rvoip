...
cargo build -p rvoip-session-core \
  --example freeswitch_udp_blind_transfer_remote_transferor \
  --example freeswitch_udp_blind_transfer_remote_transferee \
  --example freeswitch_udp_blind_transfer_remote_target

echo "[2003] Starting transfer target"
AUDIO_OUTPUT_DIR="$OUT_DIR" cargo run -p rvoip-session-core \
  --example freeswitch_udp_blind_transfer_remote_target --quiet \
  >"$LOG_TARGET" 2>&1 &
PID_TARGET=$!
...
AUDIO_OUTPUT_DIR="$OUT_DIR" cargo run -p rvoip-session-core \
  --example freeswitch_udp_blind_transfer_remote_transferee --quiet \
  >"$LOG_TRANSFEREE" 2>&1 &
PID_TRANSFEREE=$!
...
AUDIO_OUTPUT_DIR="$OUT_DIR" cargo run -p rvoip-session-core \
  --example freeswitch_udp_blind_transfer_remote_transferor --quiet \
  >"$LOG_TRANSFEROR" 2>&1
