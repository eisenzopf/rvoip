...
cargo build -p rvoip-session-core --features dev-insecure-tls \
  --example freeswitch_tls_srtp_blind_transfer_remote_transferor \
  --example freeswitch_tls_srtp_blind_transfer_remote_transferee \
  --example freeswitch_tls_srtp_blind_transfer_remote_target \
  --example freeswitch_tls_srtp_blind_transfer_remote_analyze

echo "[1003] Starting transfer target"
AUDIO_OUTPUT_DIR="$OUT_DIR" cargo run -p rvoip-session-core --features dev-insecure-tls \
  --example freeswitch_tls_srtp_blind_transfer_remote_target --quiet \
  >"$LOG_TARGET" 2>&1 &
PID_TARGET=$!
...
AUDIO_OUTPUT_DIR="$OUT_DIR" cargo run -p rvoip-session-core --features dev-insecure-tls \
  --example freeswitch_tls_srtp_blind_transfer_remote_transferee --quiet \
  >"$LOG_TRANSFEREE" 2>&1 &
PID_TRANSFEREE=$!
...
AUDIO_OUTPUT_DIR="$OUT_DIR" cargo run -p rvoip-session-core --features dev-insecure-tls \
  --example freeswitch_tls_srtp_blind_transfer_remote_transferor --quiet \
  >"$LOG_TRANSFEROR" 2>&1
...
AUDIO_OUTPUT_DIR="$OUT_DIR" cargo run -p rvoip-session-core --features dev-insecure-tls \
  --example freeswitch_tls_srtp_blind_transfer_remote_analyze --quiet \
  >"$LOG_ANALYZE" 2>&1
