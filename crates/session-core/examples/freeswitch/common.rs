...
use rvoip_session_core::{
    ...
    SipContactMode, SrtpSuitePolicy, StreamPeer,
};
...
pub const DEFAULT_POST_REGISTER_SETTLE_SECS: u64 = 2;
...
impl TlsContactMode {
    fn from_env() -> ExampleResult<Self> {
        match env_string("FREESWITCH_TLS_CONTACT_MODE", "reachable-contact")
            .trim()
            .to_ascii_lowercase()
            .as_str()
        {
            ...
            other => Err(format!(
                "FREESWITCH_TLS_CONTACT_MODE must be reachable-contact, registered-flow-rfc5626, or registered-flow-symmetric, got '{}'",
                other
            )
            .into()),
        }
...
impl EndpointConfig {
    ...
    pub fn stream_config(&self) -> Config {
        let mut config = Config::freeswitch_internal(
            &self.username,
    ...
    pub fn tls_srtp_stream_config(&self) -> ExampleResult<Config> {
        if !self.is_tls() {
            return Err("tls_srtp examples require SIP_TRANSPORT=TLS".into());
        }
        ...
        config.srtp_required = env_bool("FREESWITCH_TLS_SRTP_REQUIRED", true)?;
...
pub fn load_env() {
    let env_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("examples/freeswitch/.env");
    let _ = dotenvy::from_filename(env_path);
    let _ = dotenvy::dotenv();
    if let Ok(home) = std::env::var("HOME") {
        let freeswitch_local = Path::new(&home)
            .join("Developer")
            .join("freeswitch")
            .join("freeswitch-local.env");
        let _ = dotenvy::from_filename(freeswitch_local);
    }
}
...
pub fn endpoint_config(
    ...
    let transport = std::env::var("FREESWITCH_TRANSPORT")
        .or_else(|_| std::env::var("SIP_TRANSPORT"))
        .unwrap_or_else(|_| "UDP".to_string())
        .to_lowercase();
    let default_addr = if transport == "tls" {
        "127.0.0.1:5063"
    } else {
        "127.0.0.1:5062"
    };
    let addr_key = if transport == "tls" {
        "FREESWITCH_TLS_ADDR"
    } else {
        "FREESWITCH_UDP_ADDR"
    };
    let profile_addr = env_string(addr_key, default_addr);
    let (sip_server, sip_port) = split_host_port(&profile_addr)?;
    ...
    let local_ip: IpAddr = std::env::var("RVOIP_LOCAL_IP")
        .or_else(|_| std::env::var("LOCAL_IP"))
        .unwrap_or_else(|_| "127.0.0.1".to_string())
        .parse()?;
    let advertised_ip =
        match std::env::var("RVOIP_ADVERTISED_IP").or_else(|_| std::env::var("ADVERTISED_IP")) {
            Ok(value) => value.parse()?,
            Err(_) if !local_ip.is_unspecified() => local_ip,
            Err(_) => {
                return Err(
                    "RVOIP_ADVERTISED_IP is required when RVOIP_LOCAL_IP is 0.0.0.0 or ::".into(),
                );
            }
        };
    let media_advertised_ip = match std::env::var("RVOIP_MEDIA_ADVERTISED_IP")
    ...
    let output_dir = std::env::var("AUDIO_OUTPUT_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| {
            Path::new(env!("CARGO_MANIFEST_DIR")).join("examples/freeswitch/output")
        });
...
pub fn post_register_settle_duration() -> ExampleResult<Duration> {
    let secs = std::env::var("FREESWITCH_POST_REGISTER_SETTLE_SECS")
...
pub fn remote_test_timeout() -> ExampleResult<Duration> {
    let secs = std::env::var("FREESWITCH_TEST_TIMEOUT_SECS")
...
pub fn call_retry_attempts() -> ExampleResult<usize> {
    let attempts = std::env::var("FREESWITCH_CALL_RETRY_ATTEMPTS")
...
pub fn remote_test_digits() -> Vec<char> {
    std::env::var("FREESWITCH_TEST_DIGITS")
...
pub fn expect_remote_hold_events() -> ExampleResult<bool> {
    env_bool("FREESWITCH_EXPECT_REMOTE_HOLD_EVENTS", false)
}
...
pub async fn call_with_ringing_retry(
    ...
    for attempt in 1..=attempts {
            ...
            .map(|_| ())
        {
            ...
            Err(e) => {
                println!(
                    "[call] Attempt {}/{} to {} did not report provisional progress: {}",
...
fn split_host_port(value: &str) -> ExampleResult<(String, u16)> {
    ...
    let (host, port) = value
        .rsplit_once(':')
        .ok_or_else(|| format!("expected host:port for FreeSWITCH address, got '{}'", value))?;
    if host.trim().is_empty() {
        return Err(format!("missing host in FreeSWITCH address '{}'", value).into());
    }
...
mod tests {
    ...
    fn generated_440hz_wav_passes_440hz_detection() {
        let samples = (0..TONE_FRAMES)
            ...
            .collect::<Vec<_>>();
        let tmp =
            std::env::temp_dir().join(format!("rvoip_freeswitch_440_{}.wav", uuid::Uuid::new_v4()));
    ...
    fn generated_880hz_wav_passes_880hz_detection() {
        let samples = (0..TONE_FRAMES)
            ...
            .collect::<Vec<_>>();
        let tmp =
            std::env::temp_dir().join(format!("rvoip_freeswitch_880_{}.wav", uuid::Uuid::new_v4()));
        let out_dir = tmp.parent().unwrap();
        let name = tmp.file_name().unwrap().to_string_lossy().to_string();
        save_wav(out_dir, &name, &samples).unwrap();
