...
impl Endpoint {
    ...
    ///
    /// ```rust,no_run
    /// # async fn example(config: rvoip_session_core::EndpointConfig) -> rvoip_session_core::Result<()> {
    /// let endpoint = rvoip_session_core::Endpoint::from_config(config).await?;
    /// endpoint.shutdown().await?;
    /// # Ok(())
    /// # }
    /// ```
    ...
    /// ```rust,no_run
    /// # async fn example() -> rvoip_session_core::Result<()> {
    /// let endpoint = rvoip_session_core::Endpoint::from_json_file("alice.json").await?;
    /// endpoint.shutdown().await?;
    /// # Ok(())
    /// # }
    /// ```
