...
use crate::state_table::{CallState, FailureReason};
...
impl SessionStore {
    ...
    pub async fn export_session_history(&self, session_id: &SessionId) -> crate::errors_v2::Result<String> {
        let session = self.get_session(session_id).await
            .map_err(|e| crate::errors_v2::SessionError::Other(e.to_string()))?;
        Ok(session.history
            .map(|h| h.export_json())
    ...
    pub async fn export_session_history_csv(&self, session_id: &SessionId) -> crate::errors_v2::Result<String> {
        let session = self.get_session(session_id).await
            .map_err(|e| crate::errors_v2::SessionError::Other(e.to_string()))?;
        Ok(session.history
            .map(|h| h.export_csv())
