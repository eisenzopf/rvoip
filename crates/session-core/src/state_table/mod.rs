...
fn build_master_table() -> MasterStateTable {
    ...
    let table = YamlTableLoader::load_embedded_default()
        .expect("BUG: Embedded default state table must be valid");

    // Validate the table - embedded default must be valid at compile time
    if let Err(errors) = table.validate() {
        tracing::error!("BUG: Embedded default state table failed validation: {:?}", errors);
        // This is a programming error, not a runtime condition, but we avoid panic
        // and return the table anyway since it was successfully parsed
    }
...
/// 3. Embedded default
pub fn load_state_table_with_config(config_path: Option<&str>) -> Result<MasterStateTable, Vec<String>> {
    ...
    // Validate the embedded default table
    if let Err(errors) = table.validate() {
        tracing::error!("BUG: Embedded default state table failed validation: {:?}", errors);
        return Err(errors);
    }
