...
async fn test_bridge_with_audio() {
    ...
    let (bob_tx, _bob_rx) = call_to_bob.audio_channels().expect("Failed to get audio channels");
    ...
    let (charlie_tx, _charlie_rx) = call_to_charlie.audio_channels().expect("Failed to get audio channels");
