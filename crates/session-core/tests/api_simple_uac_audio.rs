...
async fn test_audio_channel_setup() {
    ...
    // This would panic: let (_tx2, _rx2) = call.audio_channels().expect("Failed to get audio channels");
...
async fn test_audio_sending() {
    ...
    let (tx, _rx) = call.audio_channels().expect("Failed to get audio channels");
