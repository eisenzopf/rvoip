...
side receiving a re-INVITE has no observable behavior in session-core —
...
### Phase 1 — Production wiring (dialog-core + session-core)
...
#### P1.3 Extend session-core's `EventType::ReinviteReceived` with method

**File**: `crates/session-core/src/state_table/types.rs:181`
...
**File**: `crates/session-core/src/adapters/session_event_handler.rs`
...
**File**: `crates/session-core/state_tables/default.yaml`
...
**File**: `crates/session-core/src/state_table/types.rs` (Guard enum)
...
**Files**:
- `crates/session-core/examples/streampeer/glare_retry/alice.rs`
- `crates/session-core/examples/streampeer/glare_retry/bob.rs`
- `crates/session-core/tests/glare_retry_integration.rs`
...
**Files**:
- `crates/session-core/examples/streampeer/session_timer_failure/alice.rs`
- `crates/session-core/examples/streampeer/session_timer_failure/bob.rs`
- `crates/session-core/tests/session_timer_failure_integration.rs`
...
| `crates/session-core/src/state_table/types.rs` | 1.3, 2.1 | Split into `ReinviteReceived`/`UpdateReceived`; add `HasPendingReinvite` guard |
| `crates/session-core/src/state_machine/guards.rs` | 2.1 | Implement `HasPendingReinvite` match arm |
| `crates/session-core/src/state_machine/executor.rs` | 1.6 | Wire SDP from incoming re-INVITE into `session.remote_sdp` |
| `crates/session-core/src/adapters/session_event_handler.rs` | 1.4 | Split handler by method, dispatch correct `EventType` |
| `crates/session-core/src/state_table/yaml_loader.rs` | 1.3 | Parse `ReinviteReceived`, `UpdateReceived` event names |
| `crates/session-core/state_tables/default.yaml` | 1.5, 2.2 | 4 new transitions (normal) + 2 glare transitions |
| `crates/session-core/examples/streampeer/glare_retry/{alice,bob}.rs` | 3.1 | New |
| `crates/session-core/examples/streampeer/session_timer_failure/{alice,bob}.rs` | 3.2 | New |
| `crates/session-core/tests/glare_retry_integration.rs` | 3.1 | New |
| `crates/session-core/tests/session_timer_failure_integration.rs` | 3.2 | New |
| `crates/session-core/tests/glare_retry.rs` | 4.3 | Delete (superseded) |
| `crates/session-core/Cargo.toml` | 3.1, 3.2 | Register 4 new example binaries |
| `crates/session-core/docs/RFC_COMPLIANCE_STATUS.md` | 4.1 | Flip rows, add UAS re-INVITE row |
| `crates/session-core/docs/HARDENING_BEFORE_B2BUA.md` | 4.2 | Mark T2.2, T2.3 done |
...
2. `cargo check -p rvoip-session-core` — state table loader, event
   dispatch, YAML transitions all compile and load.
3. `cargo test -p rvoip-session-core --tests -- --test-threads=1` — all
   existing tests still pass, especially `hold_resume` (which now drives
   real state-machine re-INVITE responses).
4. `cargo test -p rvoip-session-core --test glare_retry_integration`
   — passes in under 20s; Alice observes either her own retry or Bob's
   success, lands in `CallOnHold`.
5. `cargo test -p rvoip-session-core --test session_timer_failure_integration`
