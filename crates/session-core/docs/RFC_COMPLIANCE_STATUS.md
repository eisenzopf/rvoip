...
| BYE | ✅ | ✅ | `SessionHandle::hangup()` / `hangup_and_wait()` drive state-machine teardown. `hangup_with_reason(SipReason, timeout)` attaches an RFC 3326 `Reason:` header to the BYE. BYE `200`, `481`, `408`, and transaction termination are terminal for local state. |
| CANCEL | ✅ | ✅ | |
...
| SUBSCRIBE | ✅ | ✅ | RFC 6665 event framework, including public RFC 4235 dialog-package subscriptions via `StreamPeer::subscribe_dialogs` / `UnifiedCoordinator::subscribe_dialogs`. |
...
| `DialogPackageNotify`, `DialogStateChanged` | RFC 4235 dialog-package notifications parsed from `application/dialog-info+xml`. |
...
| RFC 3515 §2.4.5 progress-NOTIFY wiring | ✅ (`tests/transfer_notify_wiring_tests.rs` — structural tests on the default state table: `SendRefer100Trying` fires before `SendReferAccepted` on REFER accept, `SendTransferNotifyRinging` on UAC `Dialog180Ringing`, `SendTransferNotifySuccess` on UAC `Dialog200OK` ordered after media-commit, and `next_state` invariance so non-transfer calls are unaffected.) |
