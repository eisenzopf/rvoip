...
| INFO | ⚠️ | ⚠️ | dialog-core has the plumbing; no session-core helper yet |
...
| PUBLISH | ⚠️ | ⚠️ | Partial — dialog-core plumbing exists; presence scenarios not tested in session-core |
...
4. **INFO method** — dialog-core supports it; no public session-core helper API.
