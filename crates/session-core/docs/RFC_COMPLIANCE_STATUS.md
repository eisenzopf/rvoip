...
| `ReferReceived`, `TransferAccepted`, `TransferCompleted`, `TransferFailed`, `TransferProgress` | REFER / transfer flow. `TransferProgress` / `Completed` / `Failed` are emitted **both** on the send side (transferee's UAC progress driving `SendTransferNotify*` actions) and the receive side (transferor observing inbound sipfrag NOTIFYs on the REFER subscription), so b2bua wrappers can listen uniformly regardless of which end they're playing. |
| `NotifyReceived { event_package, subscription_state, content_type, body }` | Inbound NOTIFY on any event package (RFC 6665). Raw strings — session-core does not parse `subscription_state` beyond forwarding it. For `event_package == "refer"` + `message/sipfrag` body, the sipfrag status line is additionally parsed into `TransferProgress` / `TransferCompleted` / `TransferFailed`. |
...
| Generic NOTIFY send + `Event::NotifyReceived` | ✅ (`tests/notify_send_integration.rs` — multi-binary; Alice calls Bob, sends a NOTIFY for `event_package = "dialog"` with `Subscription-State: active;expires=3600` via `SessionHandle::send_notify`; Bob's event stream asserts `Event::NotifyReceived` carries the expected package + subscription-state. Exercises `SessionHandle::send_notify` → `DialogAdapter::send_notify` → dialog-core NOTIFY handler → new `DialogToSessionEvent::NotifyReceived` cross-crate event → `SessionCrossCrateEventHandler::handle_notify_received` → public `Event::NotifyReceived`.) |
| RFC 3515 §2.4.5 progress-NOTIFY wiring | ✅ (`tests/transfer_notify_wiring_tests.rs` — 5 structural tests on the default state table: `SendRefer100Trying` fires before `SendReferAccepted` on REFER accept, `SendTransferNotifyRinging` on UAC `Dialog180Ringing`, `SendTransferNotifySuccess` on UAC `Dialog200OK` ordered after media-commit, `next_state` invariance so non-transfer calls are unaffected. Unit tests for sipfrag status-line parsing live in `src/adapters/session_event_handler.rs::tests`.) |

Tests to add would fill the "no test" gaps above. 423 REGISTER retry is covered by an in-process raw-UDP mock registrar in `tests/register_423_retry.rs`. 491 glare was previously covered by a state-table unit test; it's now a real multi-binary integration test (`tests/glare_retry_integration.rs`) using the `cancel_integration.rs` two-subprocess pattern. End-to-end RFC 3515 §2.4.5 progress-NOTIFY send + inbound sipfrag observation tests (three-peer fixtures) belong with the upcoming b2bua crate and are deferred there.
