...
`crates/session-core/state_tables/default.yaml` — `CancelCall`
...
| B | `crates/session-core/state_tables/default.yaml` | Add three HangupCall transitions (Initiating, Ringing, EarlyMedia) |
| A+B | `crates/session-core/tests/cancel_integration.rs` | Remove `#[ignore]`; verify end-to-end |
| A+B | `crates/session-core/docs/RFC_COMPLIANCE_STATUS.md` | Mark the 487 row ✅ |

## Verification

1. `cargo test -p rvoip-dialog-core` — 317 tests stay green; new UAS
   CANCEL unit test passes.
2. `cargo test -p rvoip-session-core` — 168 tests stay green;
