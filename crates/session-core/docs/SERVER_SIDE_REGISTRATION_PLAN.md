...
### What Works ✅
- **Client-side:** session-core can send REGISTER, handle 401, compute digest auth
...
### What's Missing ❌
- **Event definitions:** No event for incoming REGISTER from dialog-core → session-core
- **Event definitions:** No event for session-core → dialog-core to send 401/200 response  
- **Handler:** session-core doesn't subscribe to or handle incoming REGISTER
...
### Step 4: Implement session-core Server-Side REGISTER Handler (3-4 hours)

**File:** `crates/session-core/src/adapters/registration_handler.rs` (NEW)
...
**File:** `crates/session-core/tests/registration_test.rs`
...
async fn start_registrar_server(port: u16, realm: &str) -> (Arc<RegistrarService>, Arc<DialogServer>) {
    ...
    // Wire up registration handler in session-core pattern
    let registration_handler = RegistrationHandler::new(
        registrar.clone(),
        global_coordinator.clone(),
    );
...
| session-core | Implement server-side handler | 3-4 |
...
✅ **session-core** subscribes to event, coordinates with registrar-core  
✅ **session-core** publishes response event back to bus  
