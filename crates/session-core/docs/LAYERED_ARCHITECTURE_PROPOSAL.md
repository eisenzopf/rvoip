# Layered Architecture: session-core → b2bua → call-center
...
│  - Routing logic                        │
├─────────────────────────────────────────┤
│      rvoip-session-core              │  ← Single session management
...
**session-core handles ONE session at a time.** Multi-session orchestration
(B2BUA, call queues, bridges) belongs in higher-layer crates.

---

## session-core Interfaces
...
b2bua
    ↓ uses
session-core (StreamPeer / CallbackPeer / SessionHandle)
    ↓ uses
