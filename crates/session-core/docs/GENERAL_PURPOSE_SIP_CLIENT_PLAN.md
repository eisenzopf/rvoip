...
| A5 | SIP Outbound — Contact `+sip.instance`/`reg-id`/`;ob` helpers landed; flow-token state machine + CRLF keep-alive pending | RFC 5626 | ✅ Phase 1 | `crates/sip-core/src/types/outbound.rs` — `OutboundContactParams { instance_urn, reg_id }` + `set/read_outbound_contact_params(Address)` helpers with RFC 5626 §4.1 quoting (`+sip.instance="<urn:...>"`) and §4.2 reg-id. `mark_uri_as_outbound(Address)` adds the §5.4 `;ob` URI parameter (idempotent). 10 unit tests covering write, round-trip, idempotent re-write, missing-angle-brackets tolerance, non-numeric reg-id rejection, and `;ob` idempotency. **Phase 2+ (pending)**: `OutboundFlow` state machine in new `crates/dialog-core/src/outbound/mod.rs` (25 s CRLFCRLF ping per §5.1, flow-failure detection, re-registration trigger); per-connection keep-alive hook in TCP/TLS transports; `Config::sip_outbound_enabled` / `sip_instance` / `outbound_keepalive_interval_secs` knobs in session-core; `DialogAdapter::send_register` rewrite to attach outbound Contact params + `;ob`. |
