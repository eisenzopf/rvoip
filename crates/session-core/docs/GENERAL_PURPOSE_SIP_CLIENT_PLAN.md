...
| **UAC behind NAT, long-duration registration** | ✅ | A3 — RFC 3581 NAT discovery + RFC 5626 §5 Contact rewrite. Long-duration NAT-traversal via RFC 5626 §3.5.1 CRLFCRLF keep-alive on TCP/TLS landed in A5 Phase 2b-min (stateless ping); failure-driven re-REGISTER (Phase 2c) still pending |
...
| B4 | DTMF RTP events — core codec (transmitter / receiver / API wiring follow-up) | RFC 4733 | ✅ codec | `crates/media-core/src/codec/audio/dtmf.rs` — pure `DtmfEvent` + `TelephoneEvent` types implementing RFC 4733 §2.3 wire format (4-byte payload: event/E/R/volume/duration). `DtmfEvent::from_digit` covers 0-9, `*`, `#`, A-D (case-insensitive). `TelephoneEvent::encode/decode` round-trip every DTMF digit; decoder tolerates oversized payloads per spec forward-compatibility clause, ignores the reserved R bit. Volume saturates at 63 (RFC 4733 6-bit field). Re-exported through `codec::audio`. 10 unit tests. **Follow-up** (B4 wiring): extend `crates/rtp-core/src/transport/udp.rs` receiver loop to dispatch PT 101 frames through the new decoder; expose `UnifiedCoordinator::send_dtmf(session_id, digit, duration)` + `SessionEvent::DtmfReceived` on the session-core event bus; add `a=rtpmap:101 telephone-event/8000` + `a=fmtp:101 0-15` to the default SDP offer in `crates/sip-core/src/sdp/builder.rs` when PCMU/PCMA is present; extend `audio_roundtrip_integration` with a mid-call DTMF send+receive. |
