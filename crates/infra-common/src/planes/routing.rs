...
use super::{FederatedPlane, PlaneConfig};
...
impl PlaneRouter {
    ...
    async fn send_to_plane(
        &self,
        plane: &Arc<dyn FederatedPlane>,
        event: Arc<dyn RoutableEvent>,
    ) -> Result<()> {
    ...
    async fn deliver_local(
        &self,
        _plane: &Arc<dyn FederatedPlane>,
        _event: Arc<dyn RoutableEvent>,
    ) -> Result<()> {
...
impl std::fmt::Debug for RoutingCondition {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            RoutingCondition::EventProperty { key, value } => {
                write!(f, "EventProperty {{ key: {}, value: {} }}", key, value)
            }
