...
impl LayerTaskManager {
        ...
        F: Future<Output = ()> + Send + 'static,
    {
        ...
        self.spawn_tracked(task_name, priority, timeout_future)
            .await
    }
    ...
    pub fn cancel_all(&self) {
        debug!("Cancelling all tasks in layer {}", self.layer_name);
    ...
    pub async fn shutdown_all(&self) -> Result<()> {
        ...
        let shutdown_result =
            tokio::time::timeout(self.shutdown_timeout, self.wait_for_completion()).await;
        ...
        match shutdown_result {
            ...
            Err(_) => {
                warn!(
                    ...
                    self.layer_name, self.shutdown_timeout
                );
...
impl GlobalTaskRegistry {
    ...
    pub async fn shutdown_all(&self, timeout: Duration) -> Result<()> {
        ...
        let results =
            tokio::time::timeout(timeout, futures::future::join_all(shutdown_futures)).await;
...
mod tests {
    ...
    async fn test_shutdown_with_timeout() {
        let manager = LayerTaskManager::with_config("test", 100, Duration::from_millis(100));

        // Spawn tasks that won't respond to cancellation
        for i in 0..3 {
            let _task_id = manager
                .spawn_tracked(format!("stubborn_task_{}", i), TaskPriority::Low, async {
