...
use crate::config::provider::{BasicConfigProvider, ConfigSource};
...
impl ConfigLoader {
        ...
        T: DeserializeOwned + Send + Sync + Debug + 'static,
    {
        ...
        if base_path.exists() {
            config
                .merge(File::from(base_path.clone()))
