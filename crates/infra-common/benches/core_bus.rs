...
fn bench_event_bus_single_publisher(c: &mut Criterion) {
    ...
    for &num_subscribers in &SUBSCRIBER_COUNTS[0..3] {
        // Using first 3 elements: 1, 10, 100
                ...
                b.iter(|| {
                    rt.block_on(async {
                        ...
                        for _ in 0..n {
                            ...
                            let _ = event_bus
                                .subscribe::<TestEvent, _>(None, handler)
                                .await
                                .unwrap();
                        }
                        ...
                        for i in 0..100 {
                            let _ = event_bus
                                .publish(create_test_event(i, EventPriority::Normal))
                                .await;
                        }
...
fn bench_event_bus_channel(c: &mut Criterion) {
    ...
    for &num_subscribers in &SUBSCRIBER_COUNTS[1..] {
        // Using last 3 elements: 10, 100, 1000
            ...
            |b, &n| {
                b.iter(|| {
                    rt.block_on(async {
                        ...
                        let dummy_handler = TestHandler {
                            counter: counter.clone(),
                        };
                        let _ = event_bus
                            .subscribe::<TestEvent, _>(None, dummy_handler)
                            .await
                            .unwrap();

                        // Publish one event to ensure the broadcast channel is created
                        let _ = event_bus
                            .publish(create_test_event(0, EventPriority::Normal))
                            .await;
                        ...
                        let counter_tasks: Vec<_> = receivers
                            .into_iter()
                            .map(|mut rx| {
                        ...
                        if counter.load(std::sync::atomic::Ordering::Relaxed) < expected_count {
                            println!(
                                "Warning: Not all events were processed. Got {} out of {}",
                                counter.load(std::sync::atomic::Ordering::Relaxed),
                                expected_count
                            );
                        }
                ...
                b.iter(|| {
                    rt.block_on(async {
                        ...
                        for i in 0..EVENT_COUNT {
                            let _ = event_bus
                                .publish(create_test_event(i as u64, EventPriority::Normal))
                                .await;
                        }
...
mod tests {
    ...
    fn test_event_bus_broadcast() {
        ...
        rt.block_on(async {
            ...
            let _ = event_bus
                .publish(create_test_event(1, EventPriority::Normal))
                .await;
            ...
            assert_eq!(
                channel_counter.load(std::sync::atomic::Ordering::Relaxed),
                1,
