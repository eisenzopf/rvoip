...
use crate::common::{uri, addr, param_tag, param_expires, param_q, param_transport, param_method, param_user, param_other, param_received, param_ttl, param_lr, assert_parses_ok, assert_parse_fails};
...
fn test_parse_simple_uri_adapted() {
    ...
    assert_parses_ok(
        "sip:example.com", 
        Uri { scheme: Scheme::Sip, host: Host::Domain("example.com".to_string()), ..Default::default() }
    );
    assert_parses_ok(
        "sips:secure.example.com:5061", 
        Uri { scheme: Scheme::Sips, host: Host::Domain("secure.example.com".to_string()), port: Some(5061), ..Default::default() }
    );
}
...
fn test_tel_uri_adapted() {
    ...
    assert_parses_ok(
        "tel:+1-212-555-0123", 
        Uri { scheme: Scheme::Tel, host: Host::Domain("+1-212-555-0123".to_string()), ..Default::default() }
    );
}
...
fn test_parse_uri_edge_cases() {
    ...
    assert_parses_ok("sip:example.com", Uri { scheme: Scheme::Sip, host: Host::Domain("example.com".to_string()), ..Default::default() });
    // User only
    assert_parses_ok("sip:alice@atlanta.com", Uri { scheme: Scheme::Sip, user: Some("alice".to_string()), host: Host::Domain("atlanta.com".to_string()), ..Default::default() });
    // User and password (discouraged)
    assert_parses_ok("sip:alice:secret@atlanta.com", Uri { scheme: Scheme::Sip, user: Some("alice".to_string()), password: Some("secret".to_string()), host: Host::Domain("atlanta.com".to_string()), ..Default::default() });
    // Host is IPv4
    assert_parses_ok("sip:192.168.0.1", Uri { scheme: Scheme::Sip, host: Host::IPv4("192.168.0.1".to_string()), ..Default::default() });
    // Host is IPv6
    assert_parses_ok("sips:[2001:db8::1]:5061", Uri { scheme: Scheme::Sips, host: Host::IPv6("2001:db8::1".to_string()), port: Some(5061), ..Default::default() });
    // Params but no user
    assert_parses_ok("sip:host.com;transport=udp", Uri { scheme: Scheme::Sip, host: Host::Domain("host.com".to_string()), parameters: vec![param_transport("udp")], ..Default::default() });
    ...
    assert_parses_ok("sip:host.com;foo=", Uri { scheme: Scheme::Sip, host: Host::Domain("host.com".to_string()), parameters: vec![param_other("foo", Some(""))], ..Default::default() });
}
