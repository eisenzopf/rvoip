...
use crate::parser::uri::host::{host, hostport, port, ipv4_address, ipv6_reference}; // Need host parts too
...
use crate::types::via::{SentProtocol, ViaParam, ViaParamInfo}; // Assuming these exist
use crate::types::via::Via as ViaHeader; // Import the final header type

// protocol-name = "SIP" / token
fn protocol_name(input: &[u8]) -> ParseResult<&[u8]> {
    alt((tag("SIP"), token))(input)
}
...
fn transport(input: &[u8]) -> ParseResult<&[u8]> {
    alt((
        tag_no_case("UDP"), tag_no_case("TCP"), tag_no_case("TLS"),
        tag_no_case("SCTP"), 
        token // Fallback for other transports
    ))(input)
}

// sent-protocol = protocol-name SLASH protocol-version SLASH transport
fn sent_protocol(input: &[u8]) -> ParseResult<(String, String, String)> {
    map_res(
        tuple((
            protocol_name,
            preceded(slash, protocol_version),
            preceded(slash, transport)
        )),
        |(name_b, version_b, transport_b)| {
            let name = str::from_utf8(name_b)?.to_string();
            let version = str::from_utf8(version_b)?.to_string();
            let transport = str::from_utf8(transport_b)?.to_string();
            Ok((name, version, transport))
        }
    )(input)
}

// sent-by = host [ COLON port ]
// Uses hostport parser directly
...
fn ip_address_only(input: &[u8]) -> ParseResult<std::net::IpAddr> {
    alt(( 
...
fn via_params_item(input: &[u8]) -> ParseResult<Param> {
    alt((
        via_ttl,
        via_maddr,
        via_received,
...
fn via_parm(input: &[u8]) -> ParseResult<ViaHeader> {
    map(
        tuple((
            ...
            many0(preceded(semi, via_params_item))
        )),
