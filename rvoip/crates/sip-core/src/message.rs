...
impl Request {
    ...
    pub fn via_headers(&self) -> Vec<crate::parser::Via> {
        let mut result = Vec::new();
        
        for header in &self.headers {
            if header.name == HeaderName::Via {
                if let Some(text) = header.value.as_text() {
                    if let Ok(vias) = crate::parser::parse_multiple_vias(text) {
                        result.extend(vias);
                    }
                }
            }
    ...
    pub fn first_via(&self) -> Option<crate::parser::Via> {
...
impl Message {
    ...
    pub fn parse(data: &[u8]) -> Result<Self> {
        crate::parser::parse_message_bytes(data)
    }
}
