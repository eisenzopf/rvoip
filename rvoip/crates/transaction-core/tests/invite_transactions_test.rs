    ...
    let test_result = timeout(Duration::from_secs(5), async {
        ...
        if let Message::Request(request) = message {
            ...
            if let Message::Response(trying) = message {
                ...
                // 16. Get the sent 180 Ringing (may need to search through multiple messages)
                sleep(Duration::from_millis(30)).await;
                
                // In auto-processing mode, server might send multiple 100 Trying responses
                // So we need to search for the 180 Ringing response
                let mut ringing_response = None;
                for _ in 0..5 {  // Try up to 5 times to find the 180 Ringing
                    if let Some((message, _)) = env.server_transport.get_sent_message().await {
                        if let Message::Response(response) = message {
                            println!("Server sent response with status: {}", response.status_code());
                            if response.status_code() == StatusCode::Ringing.as_u16() {
                                ringing_response = Some(response);
                                break;
                            }
                            // If it's another 100 Trying, continue searching
                            else if response.status_code() == StatusCode::Trying.as_u16() {
                                println!("Found another 100 Trying response, continuing search for 180 Ringing");
                                continue;
                            }
                        }
                    }
                ...
                let ringing = ringing_response.expect("Could not find 180 Ringing response");
                ...
                // 21. Get the sent 200 OK (may need to search through multiple messages)
                sleep(Duration::from_millis(30)).await;
                
                // Search for the 200 OK response
                let mut ok_response = None;
                for _ in 0..5 {  // Try up to 5 times to find the 200 OK
                    if let Some((message, _)) = env.server_transport.get_sent_message().await {
                        if let Message::Response(response) = message {
                            println!("Server sent response with status: {}", response.status_code());
                            if response.status_code() == StatusCode::Ok.as_u16() {
                                ok_response = Some(response);
                                break;
                            }
                            // If it's another provisional response, continue searching
                            else if response.status_code() < 200 {
                                println!("Found another provisional response, continuing search for 200 OK");
                                continue;
                            }
                        }
                    }
                ...
                let ok = ok_response.expect("Could not find 200 OK response");
...
async fn test_invite_failure_flow() {
    ...
    let test_result = timeout(Duration::from_secs(5), async {
        ...
        // 4. Wait for the server to receive the INVITE or detect auto-created transaction
        println!("Waiting for server to receive INVITE request");
        
        // Give time for the INVITE message to be processed
        sleep(Duration::from_millis(100)).await;
        
        // Check if server auto-processed the INVITE by looking for any responses
        let messages_before = env.server_transport.sent_message_count().await;
        let server_auto_processed = messages_before > 0;
        
        let (server_tx_id, received_invite);
        
        if server_auto_processed {
            ...
            server_tx_id = server_tx.id().clone();
            received_invite = invite_request.clone();
            println!("Server transaction created with ID: {:?}", server_tx_id);
        } else {
            ...
            match env.wait_for_server_event(
                ...
                Some((orig_server_tx_id, recv_invite, _)) => {
                    println!("Server received INVITE request via NewRequest event, transaction ID: {:?}", orig_server_tx_id);
                    ...
                    server_tx_id = server_tx.id().clone();
                    received_invite = recv_invite;
                    println!("Server transaction created with ID: {:?}", server_tx_id);
                },
                None => {
                    println!("No NewRequest event received for INVITE, creating server transaction directly");
        ...
        // 5. Verify client is in Calling or Proceeding state (may have received 100 Trying already)
        let client_state = env.client_manager.transaction_state(&client_tx_id).await
            .expect("Failed to get client state");
        println!("Client state after sending INVITE: {:?}", client_state);
        assert!(client_state == TransactionState::Calling || client_state == TransactionState::Proceeding,
                "Client should be in Calling or Proceeding state after sending INVITE (may have received 100 Trying already)");
        
        // 6. Server sends 100 Trying response
        println!("Server sending 100 Trying");
        ...
        // 7. Wait for client to receive 100 Trying
        ...
        // 10. Wait for client to receive 486 Busy Here
        ...
        // 11. Verify the response status code is 486 Busy Here
        assert_eq!(busy_response.status_code(), StatusCode::BusyHere.as_u16(),
                 "Response should be 486 Busy Here");
        
        // 12. Wait for client to transition to Completed state and send ACK
        ...
        // 13. Wait for client to send ACK for 486
        ...
        // 14. Wait for server to transition to Confirmed state
        ...
        // 15. Wait for transaction termination
