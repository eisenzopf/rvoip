...
vCon (the IETF Virtualized Conversations envelope) is on a credible standardization path. Compliance-driven voice deployments — healthcare, financial services, lawful intercept, regulated contact centers — need an addressable, verifiable, redaction-aware conversation envelope, and vCon is the IETF answer. rvoip ships the first Rust implementation as `rvoip-vcon`: a schema-aligned container and builder with explicit JWS General JSON signing and redaction/amendment lineage fields. Core Session finalization emits validated **unsigned** vCon JSON. Signing is an opt-in application operation, JWE is not implemented, and the lineage fields do not themselves perform redaction. For compliance-bound voice AI deployments, consumers must apply the signing, encryption-at-rest, access-control, and redaction policy their audit regime requires.
...
- **vCon emission: ALWAYS, per Session.** Every Session produces a [vCon](https://datatracker.ietf.org/doc/draft-ietf-vcon-vcon-core/) — the IETF Virtualized Conversations envelope — at end-of-Session, regardless of whether audio recording was enabled. The vCon is the durable JSON record of the Session: who joined (`parties[]` from Participants), what happened (`dialog[]` per Connection / Stream / Message / transfer event), what analyses were attached (`analysis[]` for transcripts, sentiment, summaries), and what artifacts were captured (`attachments[]` for SIP signaling, STIR certs, consent records). Audio is one optional `dialog[].body` entry, not a precondition for emission.
- **vCon signing and encryption.** The automatic core path persists unsigned canonical JSON. `rvoip-vcon` offers explicit JWS General JSON signing and signature appending for consumers that need one or more signers. JWE is not implemented; deployments provide encryption at rest or an external envelope. STIR/SHAKEN PASSporTs ride in `parties[].stir`. See §7 for the security model.
- **`VconReady` carries the finalized vCon handle.** Consumers retrieve the exact bytes via the configured `VconStore` (see `INTERFACE_DESIGN.md` §11). `RecordingComplete.vcon_ref` remains unwired in 0.3.3; recording completion and Session-level vCon finalization are separate events.
...
- **Vcon storage**: every Session emits a vCon to a separate `VconStore` (per `INTERFACE_DESIGN.md` §11.4); independent of `ConversationStore` so deployments may use different backends (Postgres for Conversations, S3/object-store for vCons). The store index carries tenant, Conversation, and Session relationships without emitting the reserved vCon `group` parameter.
...
### 7.3 vCon signing, confidentiality, and lineage

- **Explicit signing**: `rvoip-vcon` signs with JWS General JSON Serialization when the caller supplies a key and certificate reference. Callers can append signatures from the AI provider, recording pipeline, or consumer. The core Session finalizer does not select tenant keys or sign automatically.
- **Confidentiality boundary**: JWE is not implemented. Deployments that require confidentiality use storage encryption or wrap the vCon outside rvoip.
- **Lineage model**: the container represents the spec's mutually exclusive `redacted` and `amended` references. rvoip does not redact content, encrypt predecessors, or enforce lineage access policy; those remain consumer responsibilities.
- **Lawful intercept**: rvoip exposes the vCon container, explicit signing APIs, and extension fields. The consumer's compliance layer applies jurisdiction-specific capture, consent, access, and redaction policy.
...
| `VconReady` | A Session's canonical unsigned vCon was validated and persisted to `VconStore` (per `INTERFACE_DESIGN.md` §11.4); carries handle for retrieval |
| `VconRedacted` | A consumer-produced redacted vCon was recorded; the model can carry its lineage reference but rvoip does not perform the redaction |
| **Messaging** | |
...
7. **vCons as the canonical conversation envelope.** rvoip emits a validated unsigned [vCon](https://datatracker.ietf.org/doc/draft-ietf-vcon-vcon-core/) at end-of-Session for every Session, regardless of audio recording. This replaces ad-hoc "audio file + sidecar JSON + DB metadata" patterns with an addressable container in the IETF standard direction for conversational data. Consumers opt into JWS General JSON signing; JWE and automated redaction are not shipped. See §4 (recording), §7.3 (signing/confidentiality), §11 (harness integration). rvoip ships the first Rust implementation of the vCon spec as `rvoip-vcon` (per `INTERFACE_DESIGN.md` §2).
...
7. **vCon assembly cadence.** Immediate-on-BYE (commit synchronously, may delay the BYE response) vs. async-batched. Lean: async-batched with a 5-second SLA, surfaced as a separate `VconReady` event when committed. `RecordingComplete.vcon_ref` is not wired in 0.3.3. Explicit signing, when requested by the consumer, is outside the automatic finalization path.
...
9. **vCon redaction policy boundary.** `rvoip-vcon` represents `redacted` / `amended` lineage but does not transform content. Open: should rvoip add a redaction operation or remain model-only? Lean: remain redaction-agnostic; consumers integrate their existing PII detection and policy.
