...
**Scope:** `rvoip-sip` (beta-ready within its documented envelope) and `rtp-core` (media transport). Both ship on the unified `0.3.9` train; SIP is beta-qualified by product scope rather than by a version suffix.
